...
async function fetchHelloAOBible(reference){
    ...
    const text=verses.map(v=>String(v.text||"").replace(/\s+/g," ").trim()).filter(Boolean).join(" ").trim();
...
function fitMultiLyricsToScreen(output, section, settings){
    ...
    // Lyrics Only automatically selects the largest size that fits the real screen.
    let low=14, high=Math.max(48,Math.min(180,Math.floor(Math.min(width*.11,height*.18))));
    applySize(low);
    const fits=()=>section.scrollHeight<=height*.92+2 && section.scrollWidth<=width*.98+2;
    for(let i=0;i<12;i++){
        const mid=Math.floor((low+high)/2);
        applySize(mid);
        if(fits()) low=mid+1; else high=mid-1;
    }
    const finalSize=Math.max(14,Math.min(180,high));
    applySize(finalSize);
    const freeSpace=Math.max(0,height-section.scrollHeight);
    ...
    section.dataset.fittedFontSize=String(Math.round(finalSize));
}
