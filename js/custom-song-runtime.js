...
import { collection, deleteDoc, doc, getDoc, getDocs, setDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
const CHORDIO_UPLOADED_BG_COLLECTION="backgrounds";
const CHORDIO_UPLOADED_BG_MAX_BYTES=100*1024*1024;

function chordioIsVideoType(type){return String(type||"").toLowerCase().startsWith("video/");}
function chordioUploadedBackgroundUrl(id){return "fb://"+String(id||"");}

async function chordioGetUploadedBackgroundMeta(){
    try{
        const snap=await getDocs(collection(db,CHORDIO_UPLOADED_BG_COLLECTION));
        return snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>Number(b.createdAt?.toMillis?.()||b.createdAt||0)-Number(a.createdAt?.toMillis?.()||a.createdAt||0));
    }catch(error){
        console.error("Unable to load shared background library:",error);
        return [];
    }
}
...
async function chordioSaveUploadedBackground(file){
    ...
    if(!file)throw new Error("No background file selected.");
    const mime=String(file.type||"").toLowerCase();
    const ext=String(file.name||"").toLowerCase().match(/\.([a-z0-9]+)$/)?.[1]||"";
    const isImage=/^image\//.test(mime)||/^(jpg|jpeg|png|gif|webp|bmp|svg)$/.test(ext);
    const isVideo=/^video\//.test(mime)||/^(mp4|webm|ogg|mov|m4v|avi|mkv)$/.test(ext);
    if(!isImage&&!isVideo)throw new Error("Only image and video files are supported.");
    if(Number(file.size||0)>CHORDIO_UPLOADED_BG_MAX_BYTES)throw new Error("The background file must be 100 MB or smaller.");

    const id="bg-"+Date.now()+"-"+Math.random().toString(36).slice(2,10);
    const safeName=String(file.name||"background").replace(/[^a-zA-Z0-9._-]+/g,"_");
    const path=`backgrounds/${id}-${safeName}`;
    const objectRef=storageRef(storage,path);
    try{
        await uploadBytes(objectRef,file,{contentType:mime || (isVideo ? "video/mp4" : "image/jpeg"),customMetadata:{ownerUid:auth.currentUser.uid}});
    }catch(error){
        console.error("Firebase Storage background upload failed:",error);
        throw new Error(`Firebase Storage upload failed: ${error?.message||error}`);
    }
    let downloadURL="";
    try{downloadURL=await getDownloadURL(objectRef);}
    catch(error){
        try{await deleteObject(objectRef);}catch(_){}
        console.error("Firebase Storage download URL failed:",error);
        throw new Error(`Firebase Storage URL creation failed: ${error?.message||error}`);
    }
    const data={
        name:String(file.name||"Uploaded Background"),
        type:mime || (isVideo ? "video/mp4" : "image/jpeg"),
        size:Number(file.size||0),
        downloadURL,
        storagePath:path,
        ownerUid:auth.currentUser.uid,
        ownerEmail:String(auth.currentUser.email||""),
        createdAt:serverTimestamp(),
        updatedAt:serverTimestamp()
    };
    try{
        await setDoc(doc(db,CHORDIO_UPLOADED_BG_COLLECTION,id),data);
    }catch(error){
        try{await deleteObject(objectRef);}catch(_){}
        console.error("Firestore background metadata save failed:",error);
        throw new Error(`Firebase database save failed: ${error?.message||error}`);
    }
...
async function chordioGetUploadedBackground(id){
    try{
        const snap=await getDoc(doc(db,CHORDIO_UPLOADED_BG_COLLECTION,String(id||"")));
        return snap.exists()?{id:snap.id,...snap.data()}:null;
    }catch(error){
        console.error("Unable to read shared background:",error);
        return null;
    }
}

async function chordioDeleteUploadedBackground(id){
    if(!auth.currentUser)throw new Error("Please login first.");
    const item=await chordioGetUploadedBackground(id);
    if(!item)throw new Error("Background not found.");
    const isOwner=item.ownerUid===auth.currentUser.uid;
    const isAdmin=String(auth.currentUser.email||"").trim().toLowerCase()==="cmcaragon@gmail.com";
    if(!isOwner&&!isAdmin)throw new Error("You do not have permission to delete this background.");
    if(item.storagePath){try{await deleteObject(storageRef(storage,item.storagePath));}catch(error){if(error?.code!=="storage/object-not-found")throw error;}}
    await deleteDoc(doc(db,CHORDIO_UPLOADED_BG_COLLECTION,String(id)));
    return true;
}

async function chordioResolveBackgroundUrl(url){
    const raw=String(url||"");
    if(raw.startsWith("fb://")){const item=await chordioGetUploadedBackground(raw.slice(5));return String(item?.downloadURL||"");}
    return raw;
}
...
function chordioIsUploadedBackgroundValue(value){return String(value||"").startsWith("fb://");}
...
async function initCmgBackgroundLibrary(){
    ...
    if(!select)return;
    const metas=await chordioGetUploadedBackgroundMeta();
    ...
    const uploaded=metas.map(item=>`<option value="${esc(chordioUploadedBackgroundUrl(item.id))}" data-uploaded="1" data-type="${esc(item.type||"")}">MY UPLOAD — ${esc(item.name)}</option>`).join("");
...
async function updateCmgBackgroundPreview(){
    ...
    preview.innerHTML="";
    if(chordioIsUploadedBackgroundValue(value)){
        const item=await chordioGetUploadedBackground(value.slice(5));
        ...
        if(!url){preview.innerHTML='<div class="multi-cmg-preview-empty">Saved background URL is unavailable.</div>';return;}
        if(chordioIsVideoType(item.type)){const v=document.createElement("video");v.src=url;v.autoplay=true;v.muted=true;v.loop=true;v.playsInline=true;v.controls=false;v.style.cssText="width:100%;height:100%;object-fit:cover;display:block";preview.appendChild(v);void v.play().catch(()=>{});}
...
async function openChordioBackgroundLibrary(){
    ...
    const addCard=async(item,isUpload)=>{
        const wrap=document.createElement("div");wrap.className="chordio-bg-card-wrap";
        ...
        if(isUpload){
            const url=String(item.downloadURL||"");
            if(!url)return null;
            if(chordioIsVideoType(item.type)){const v=document.createElement("video");v.src=url;v.muted=true;v.loop=true;v.playsInline=true;v.autoplay=true;mediaWrap.appendChild(v);void v.play().catch(()=>{});}
            else{const img=document.createElement("img");img.src=url;img.alt=item.name;mediaWrap.appendChild(img);}
            card.dataset.value=chordioUploadedBackgroundUrl(item.id);
            const deleteBtn=document.createElement("button");deleteBtn.type="button";deleteBtn.className="chordio-bg-delete";deleteBtn.title="Delete uploaded background";deleteBtn.innerHTML="<i class=\"fa-solid fa-trash\"></i> Delete";
            deleteBtn.onclick=async event=>{
                event.preventDefault();event.stopPropagation();
                if(!confirm(`Delete uploaded background \"${item.name.replace(/\\/g,"\\\\").replace(/`/g,"\\`")}\"?`))return;
                deleteBtn.disabled=true;
                try{
                    await chordioDeleteUploadedBackground(item.id);
                    const current=String(document.getElementById("multiCmgBackground")?.value||"");
                    if(current===chordioUploadedBackgroundUrl(item.id)){
                        const select=document.getElementById("multiCmgBackground");if(select)select.value="";
                        const typeEl=document.getElementById("multiBackgroundType");if(typeEl)typeEl.value="none";
                        const input=document.getElementById("multiBackgroundUrl");if(input)input.value="";
                    }
                    await initCmgBackgroundLibrary();
                    await openChordioBackgroundLibrary();
                }catch(error){console.error("Unable to delete uploaded background:",error);alert(error?.message||"Unable to delete this background.");deleteBtn.disabled=false;}
            };
            wrap.append(card,deleteBtn);
        }else{
            const img=document.createElement("img");img.src=chordioCmgBackgroundUrl(item.name);img.alt=item.name;mediaWrap.appendChild(img);
            card.dataset.value=item.name;
            wrap.append(card);
        }
        card.append(mediaWrap,label);
        card.onclick=async()=>{
            const value=String(card.dataset.value||"");
            const select=document.getElementById("multiCmgBackground");if(select)select.value=value;
            let type="image",inputUrl="";
            if(chordioIsUploadedBackgroundValue(value)){
                const saved=await chordioGetUploadedBackground(value.slice(5));if(!saved)return;
                type=chordioIsVideoType(saved.type)?"video":"image";inputUrl=chordioUploadedBackgroundUrl(saved.id);
            }else{type=/\.(mp4|webm|ogg|mov|m4v)$/i.test(value)?"video":"image";inputUrl=chordioCmgBackgroundUrl(value);}
            const typeEl=document.getElementById("multiBackgroundType");if(typeEl)typeEl.value=type;
            const input=document.getElementById("multiBackgroundUrl");if(input)input.value=inputUrl;
...
async function useSelectedCmgBackground(){
    ...
    if(chordioIsUploadedBackgroundValue(value)){const item=await chordioGetUploadedBackground(value.slice(5));if(!item)return;type=chordioIsVideoType(item.type)?"video":"image";url=chordioUploadedBackgroundUrl(item.id);}
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        document.getElementById("multiBackgroundApply")?.addEventListener("click",async()=>{const scope=document.getElementById("multiBackgroundScopeSelect")?.value||"all";let type=document.getElementById("multiBackgroundType")?.value||"none";let url=document.getElementById("multiBackgroundUrl")?.value.trim()||"";const selected=String(document.getElementById("multiCmgBackground")?.value||"").trim();if(selected&&chordioIsUploadedBackgroundValue(selected)){const item=await chordioGetUploadedBackground(selected.slice(5));if(item){type=chordioIsVideoType(item.type)?"video":"image";url=chordioUploadedBackgroundUrl(item.id);}}else if(!url&&selected){url=chordioCmgBackgroundUrl(selected);type=/\.(mp4|webm|ogg|mov|m4v)$/i.test(selected)?"video":"image";}if(!url&&type!=="none"){alert("Please choose or upload an image/video background first.");return;}await setMultiScreenBackground(type,url,scope==="song");const st=document.getElementById("multiBackgroundSongName");if(st){const old=st.textContent;st.textContent="✓ Background applied";setTimeout(()=>{st.textContent=old;},1500);}});
        ...
        document.getElementById("multiCmgBackground")?.addEventListener("change",async()=>{const value=String(document.getElementById("multiCmgBackground")?.value||"");if(!value)return;const typeEl=document.getElementById("multiBackgroundType");const input=document.getElementById("multiBackgroundUrl");let type="image",url=value;if(chordioIsUploadedBackgroundValue(value)){const item=await chordioGetUploadedBackground(value.slice(5));if(item){type=chordioIsVideoType(item.type)?"video":"image";url=chordioUploadedBackgroundUrl(item.id);}}else{url=chordioCmgBackgroundUrl(value);type=/\.(mp4|webm|ogg|mov|m4v)$/i.test(value)?"video":"image";}if(typeEl)typeEl.value=type;if(input)input.value=url;await updateCmgBackgroundPreview();});
