...
async function loadFromService(){
    ...
    try {
        ...
        // Give every occurrence in a Service Planner its own stable instance ID.
        // This is essential when the same song is intentionally added twice: each
        // copy can then have its own Verse/Chorus/Bridge arrangement.
        let serviceSongsChanged=false;
        let presentationLayoutsChanged=false;
        const seenInstanceIds=new Set();
        const seenSongIds=new Set();
        const migratedLayouts={...(service.presentationLayouts||{})};
        service.songs=service.songs.map(item=>{
            const copy={...item};
            const oldSongId=String(copy.id||copy.file||copy.title||"");
            let instance=copy.serviceInstanceId || copy._serviceInstanceId || copy.serviceSongId;
            if(!instance || seenInstanceIds.has(String(instance))){
                instance=`service-song-${Date.now()}-${Math.random().toString(36).slice(2,10)}`;
                serviceSongsChanged=true;
            }
            copy.serviceInstanceId=String(instance);
            delete copy._serviceInstanceId; delete copy.serviceSongId;
            seenInstanceIds.add(String(instance));
            // Preserve a legacy arrangement only for the first occurrence.
            // Later duplicate occurrences intentionally start from the normal
            // song structure so they can be arranged independently.
            if(oldSongId && !seenSongIds.has(oldSongId) && migratedLayouts[oldSongId] && !migratedLayouts[String(instance)]){
                migratedLayouts[String(instance)]=migratedLayouts[oldSongId];
                presentationLayoutsChanged=true;
            }
            seenSongIds.add(oldSongId);
            return copy;
        });
        if(serviceSongsChanged || presentationLayoutsChanged){
            service={...service,songs:service.songs,presentationLayouts:migratedLayouts};
            try{localStorage.setItem("currentServiceSnapshot",JSON.stringify(service));}catch(_){}
            // Never block song rendering on the one-time migration write. The
            // service snapshot is already corrected locally; persist it in the
            // background so a slow Firestore connection cannot leave the page
            // stuck on "Loading song…".
            if(auth.currentUser){
                void setDoc(doc(db,"users",auth.currentUser.uid,"services",String(serviceId)),{songs:service.songs,presentationLayouts:migratedLayouts,updatedAt:serverTimestamp()},{merge:true}).catch(error=>console.warn("Unable to migrate Service Planner song instances:",error));
            }
        }
...
async function load(){
    ...
    bootPromise=(async()=>{
        ...
        if(hasService && resumePresentation){
            // Service Planner launches always cache the selected service locally.
            // Load that snapshot immediately; Firebase Auth is allowed to finish
            // in the background instead of blocking the song screen.
...
function currentServiceSongInstanceKey(data=song){
    const explicit=data?.serviceInstanceId || data?._serviceInstanceId || data?.serviceSongId;
    if(explicit) return String(explicit);
    // Legacy service entries did not have an instance key. The loader assigns
    // one before this is used, so this fallback is only for standalone songs.
    return String(data?.id || new URLSearchParams(location.search).get("id") || data?.title || "song");
}
function presentationLayoutStorageKey(){
    const serviceId=localStorage.getItem("currentServiceId") || "standalone";
    return PRESENTATION_LAYOUT_PREFIX + String(serviceId) + ":" + currentServiceSongInstanceKey(song);
}
...
function getPresentationLayout(){
    ...
    const serviceId=localStorage.getItem("currentServiceId");
    const instanceKey=currentServiceSongInstanceKey(song);
    const serviceSaved=serviceId && song?.id ? serviceLayouts[instanceKey] : null;
    const hasExplicitInstance=!!(song?.serviceInstanceId || song?._serviceInstanceId || song?.serviceSongId);
    const legacySaved=serviceId && song?.id && !serviceSaved && !hasExplicitInstance ? serviceLayouts[String(song.id)] : null;
    try{
...
async function savePresentationLayout(layout){
    ...
    const presentationLayouts={...(service.presentationLayouts||{}),[currentServiceSongInstanceKey(song)]:layout};
...
function defaultMultiSlideFontSettings(){return{fontPx:56,lineSpacing:1.15,align:"center",fontFamily:"Arial",color:"#FFFFFF",bold:false,italic:false};}
function readMultiSlideFontSettings(){return{fontPx:Math.max(12,Math.min(240,Number(document.getElementById("multiSlideFontPx")?.value||56)),lineSpacing:Math.max(.7,Math.min(2.2,Number(document.getElementById("multiSlideLineSpacing")?.value||1.15))),align:document.getElementById("multiSlideAlign")?.value||"center",fontFamily:document.getElementById("multiSlideFontFamily")?.value||"Arial",color:document.getElementById("multiSlideColor")?.value||"#FFFFFF",bold:document.getElementById("multiSlideBold")?.getAttribute("aria-pressed")==="true",italic:document.getElementById("multiSlideItalic")?.getAttribute("aria-pressed")==="true"};}
function syncMultiSlideFontControls(slide){const st={...defaultMultiSlideFontSettings(),...(slide?.fontSettings||{})};[["multiSlideFontPx",st.fontPx],["multiSlideLineSpacing",st.lineSpacing],["multiSlideAlign",st.align],["multiSlideFontFamily",st.fontFamily],["multiSlideColor",st.color]].forEach(([id,v])=>{const el=document.getElementById(id);if(el)el.value=String(v);});[["multiSlideBold",!!st.bold],["multiSlideItalic",!!st.italic]].forEach(([id,on])=>{const el=document.getElementById(id);if(el){el.setAttribute("aria-pressed",String(on));el.classList.toggle("active",on);}});}
...
function castSelectedPageSlide(){const slides=currentServiceSongSlides(),slide=slides[multiSelectedPageSlideIndex];if(!slide){alert("Select a page slide first.");return;}multiActivePageSlide={...slide,index:multiSelectedPageSlideIndex,fontSettings:readMultiSlideFontSettings()};multiScreenBroadcast({activePageSlide:multiActivePageSlide});setMultiSlideStatus("Slide cast to enabled screens");}
...
function multiScreenOpenDisplay(displayNumber){
    ...
    const url=new URL(location.href);url.pathname=url.pathname.replace(/[^/]+$/, "multi-screen-output.html");url.search="";url.searchParams.set("display",String(n));url.searchParams.set("id",String(song.id||""));url.searchParams.set("mode",mode);url.searchParams.set("fullscreen","1");url.searchParams.set("ts",String(Date.now()));
    const sw=Math.max(800,window.screen?.availWidth||1280),sh=Math.max(450,window.screen?.availHeight||720);
    const features=`popup=yes,resizable=yes,scrollbars=no,width=${sw},height=${sh},left=0,top=0`;let w=multiScreenWindows[n];if(!w||w.closed)w=window.open(url.href,`CHORDIO_SCREEN_${n}`,features);else{try{w.location.replace(url.href);w.focus();w.moveTo?.(0,0);w.resizeTo?.(sw,sh);}catch(_){} }multiScreenWindows[n]=w;
    try{w?.moveTo?.(0,0);w?.resizeTo?.(sw,sh);w?.focus?.();}catch(_){}
...
function fitMultiPageSlideText(output,body,settings){
    ...
    const requested=Math.max(12,Math.min(240,Number(settings?.fontPx)||56));
    const min=12;let size=requested;
    body.style.fontSize=`${size}px`;body.style.lineHeight=String(Math.max(.7,Math.min(2.2,Number(settings?.lineSpacing)||1.15)));
    body.style.maxHeight="86vh";body.style.overflow="hidden";body.style.width="92vw";body.style.boxSizing="border-box";body.style.whiteSpace="pre-wrap";body.style.overflowWrap="anywhere";
    const fits=()=>body.scrollHeight<=body.clientHeight+2&&body.scrollWidth<=body.clientWidth+2;
    for(let i=0;i<24&&!fits()&&size>min;i++){size=Math.max(min,Math.floor(size*0.9));body.style.fontSize=`${size}px`;}
    body.dataset.fittedFontSize=String(size);
}
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        ["multiSlideBold","multiSlideItalic"].forEach(id=>document.getElementById(id)?.addEventListener("click",e=>{const el=e.currentTarget;const on=el.getAttribute("aria-pressed")==="true";el.setAttribute("aria-pressed",String(!on));el.classList.toggle("active",!on);}));
        ["multiSlideFontPx","multiSlideLineSpacing","multiSlideAlign","multiSlideFontFamily","multiSlideColor"].forEach(id=>document.getElementById(id)?.addEventListener("change",()=>{if(multiActivePageSlide){multiActivePageSlide={...multiActivePageSlide,fontSettings:readMultiSlideFontSettings()};multiScreenBroadcast({activePageSlide:multiActivePageSlide});}}));
...
document.addEventListener("DOMContentLoaded",()=>{
    bindControls();
    // Always start the loader immediately. Service Planner launches carry a
    // local snapshot, so they no longer need to wait for Firebase Auth before
    // the selected song can render. The auth callback below can refresh it later.
    void load();
});
