...
async function redirectSong(nextSong, targetIndex=index){
    ...
    void saveMultiScreenServiceConfig({activeSongIndex:Number(targetIndex),activeSectionIndex:0});
...
async function loadFromService(){
    ...
    try {
        ...
        const configuredIndex=Number(service.multiScreenConfig?.activeSongIndex);
        if(index < 0 || index >= service.songs.length){
            index=(Number.isInteger(configuredIndex)&&configuredIndex>=0&&configuredIndex<service.songs.length)?configuredIndex:0;
        }
...
function goCustomPresentationSection(delta){
    ...
    void saveMultiScreenServiceConfig({activeSectionIndex:next});
...
async function loadServiceIndex(targetIndex){
    ...
    customPresentationSectionIndex=0;
    localStorage.setItem("currentSongIndex",String(index));
    void saveMultiScreenServiceConfig({activeSongIndex:index});
...
// Multi-Screen configuration is stored inside the selected Service Planner.
// This makes screen assignments, output modes, backgrounds and preview layout
// belong to the service rather than to the browser/global user session.
function getDefaultMultiScreenServiceConfig(){return {
    modes:{1:"lyrics",2:"lyrics",3:"lyrics",4:"chords"},
    enabled:{1:true,2:true,3:true,4:true},
    backgroundModes:{1:"common",2:"common",3:"common",4:"common"},
    previewCount:4,
...
function getMultiScreenServiceConfig(){
    const d=getDefaultMultiScreenServiceConfig(), c=service?.multiScreenConfig||{};
    return {
        ...d,...c,
        modes:{...d.modes,...(c.modes||{})},
        enabled:{...d.enabled,...(c.enabled||{})},
        backgroundModes:{...d.backgroundModes,...(c.backgroundModes||{})},
        previewSlots:Array.isArray(c.previewSlots)&&c.previewSlots.length?c.previewSlots.map(Number).filter(n=>n>=1&&n<=4).slice(0,4):d.previewSlots,
        previewCount:Math.max(1,Math.min(4,Number(c.previewCount)||d.previewCount)),
        background:{...d.background,...(c.background||{})},
        songBackgrounds:{...(c.songBackgrounds||{})}
    };
}
async function saveMultiScreenServiceConfig(patch={}){
    if(!service)return false;
    const current=getMultiScreenServiceConfig();
    const merged={...current,...patch,
        modes:{...current.modes,...(patch.modes||{})},
        enabled:{...current.enabled,...(patch.enabled||{})},
        backgroundModes:{...current.backgroundModes,...(patch.backgroundModes||{})},
        background:{...current.background,...(patch.background||{})},
        songBackgrounds:{...current.songBackgrounds,...(patch.songBackgrounds||{})}
    };
    service={...service,multiScreenConfig:merged};
    try{localStorage.setItem("currentServiceSnapshot",JSON.stringify(service));}catch(_){}
    const serviceId=localStorage.getItem("currentServiceId");
    if(serviceId&&auth.currentUser){try{await setDoc(doc(db,"users",auth.currentUser.uid,"services",String(serviceId)),{multiScreenConfig:merged,updatedAt:serverTimestamp()},{merge:true});}catch(error){console.warn("Unable to save Multi-Screen service configuration:",error);return false;}}
    return true;
}
function loadMultiScreenServiceConfig(){
    const c=getMultiScreenServiceConfig();
    multiScreenModes={...multiScreenModes,...c.modes};
    multiScreenEnabled={...multiScreenEnabled,...c.enabled};
    multiScreenBackgroundModes={...multiScreenBackgroundModes,...c.backgroundModes};
    multiScreenPreviewCount=c.previewCount;
    multiScreenPreviewSlots=c.previewSlots;
    multiScreenBackground={...c.background};
    multiScreenSongBackgrounds={...c.songBackgrounds};
    if(Number.isInteger(Number(c.activeSectionIndex))) multiScreenCurrentSection=Math.max(0,Number(c.activeSectionIndex));
}
}

function multiScreenSongBackgroundKey(data=song){return String(data?.id||data?.title||"song");}
function getMultiScreenSongBackground(data=song){const key=multiScreenSongBackgroundKey(data);return multiScreenSongBackgrounds[key]||((getMultiScreenServiceConfig().songBackgrounds||{})[key]||((service?.multiScreenSongBackgrounds||{})[key]||null))||null;}
...
let multiServiceSongPickerCache=null;
async function loadMultiServiceSongLibrary(){
    if(Array.isArray(multiServiceSongPickerCache)) return multiServiceSongPickerCache;
    try{
        const snap=await getDocs(collection(db,"songs"));
        multiServiceSongPickerCache=snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>Array.isArray(x.sections)&&x.sections.length);
    }catch(error){console.error("Unable to load song library:",error);multiServiceSongPickerCache=[];}
    return multiServiceSongPickerCache;
}
function closeMultiServiceSongPicker(){document.getElementById("multiServiceSongPicker")?.remove();}
function serviceSongIdentityLocal(item){return String(item?.id||item?.file||item?.title||"").trim().toLowerCase();}
async function addSongToCurrentServiceFromPicker(songToAdd){
    if(!service||!Array.isArray(service.songs)||!songToAdd)return;
    const key=serviceSongIdentityLocal(songToAdd), count=service.songs.filter(x=>serviceSongIdentityLocal(x)===key).length;
    if(count>=3){alert(`"${songToAdd.title||"This song"}" can only be added a maximum of 3 times to the same Service Planner.`);return;}
    const copy={
        id:songToAdd.id||String(Date.now()),title:songToAdd.title||"",artist:songToAdd.artist||"",file:songToAdd.file||"",
        category:songToAdd.category||songToAdd.genre||"",language:songToAdd.language||"",key:songToAdd.key||"",
        originalKey:songToAdd.originalKey||songToAdd.key||"",serviceKey:songToAdd.serviceKey||songToAdd.key||songToAdd.originalKey||"",
        transpose:0,youtube:songToAdd.youtube||"",customSong:songToAdd.customSong===true,createdByUid:songToAdd.createdByUid||"",
        createdByEmail:songToAdd.createdByEmail||"",sections:normalizeSections(songToAdd.sections),contentVersion:songToAdd.contentVersion||null,
        createdAt:songToAdd.createdAt||null,updatedAt:songToAdd.updatedAt||null
    };
    const updated=[...service.songs,copy];
    const ok=await saveServiceSongs(updated);
    if(!ok){alert("Unable to save the song to this Service Planner.");return;}
    closeMultiServiceSongPicker();
    index=updated.length-1; song={...copy,sections:normalizeSections(copy.sections),serviceKey:copy.serviceKey,transpose:0}; transposeSteps=0;
    localStorage.setItem("currentSongIndex",String(index));
    render();renderCustomPresentation();renderMultiScreenControl();multiScreenBroadcast({sectionIndex:0});
}
async function openMultiServiceSongPicker(){
    closeMultiServiceSongPicker();
    const modal=document.createElement("div");modal.id="multiServiceSongPicker";modal.className="multi-service-song-picker-modal";
    modal.innerHTML=`<div class="multi-service-song-picker-card"><div class="multi-service-song-picker-head"><strong>ADD SONG TO SERVICE</strong><button type="button" id="multiServiceSongPickerClose">✕</button></div><input id="multiServiceSongPickerSearch" type="search" placeholder="Search song, artist, category or language..."><div id="multiServiceSongPickerList" class="multi-service-song-picker-list"><div class="multi-picker-loading">Loading song list…</div></div></div>`;
    document.body.appendChild(modal); modal.addEventListener("click",e=>{if(e.target===modal)closeMultiServiceSongPicker();});
    document.getElementById("multiServiceSongPickerClose")?.addEventListener("click",closeMultiServiceSongPicker);
    const library=await loadMultiServiceSongLibrary();
    const listEl=document.getElementById("multiServiceSongPickerList"),search=document.getElementById("multiServiceSongPickerSearch");
    const draw=()=>{const q=String(search?.value||"").trim().toLowerCase();const matches=library.filter(x=>!q||[x.title,x.artist,x.category,x.genre,x.language].some(v=>String(v||"").toLowerCase().includes(q))).sort((a,b)=>String(a.title||"").localeCompare(String(b.title||""),undefined,{numeric:true,sensitivity:"base"}));listEl.innerHTML="";if(!matches.length){listEl.innerHTML='<div class="multi-picker-loading">No songs found.</div>';return;}matches.forEach(x=>{const count=(service?.songs||[]).filter(y=>serviceSongIdentityLocal(y)===serviceSongIdentityLocal(x)).length;const row=document.createElement("div");row.className="multi-picker-song";row.innerHTML=`<div><strong>${esc(x.title||"Untitled Song")}</strong><small>${esc(x.artist||"")} ${x.category?`• ${esc(x.category)}`:""} ${count?`• Added ${count}/3`:""}</small></div><button type="button" ${count>=3?"disabled":""}>${count>=3?"✓ 3 Added":"ADD"}</button>`;row.querySelector("button").onclick=()=>void addSongToCurrentServiceFromPicker(x);listEl.appendChild(row);});};
...
function renderMultiServiceSongs(){
    ...
    addButton.type="button"; addButton.className="multi-add-service-song-btn";
    addButton.textContent="＋ ADD SONG TO SERVICE";
    addButton.onclick=()=>openMultiServiceSongPicker();
    box.appendChild(addButton);
    if(!songs.length){const empty=document.createElement("div");empty.className="multi-preview-empty";empty.style.height="120px";empty.textContent="No songs in this Service Planner.";box.appendChild(empty);return;}
...
function initMultiFeatureAccordions(){
    document.querySelectorAll("#multiScreenControl .multi-feature-toggle").forEach(toggle=>{
        const panel=toggle.closest(".multi-feature-panel")||toggle.parentElement;
        // Every Multi-Screen opening starts collapsed. The user can open it
        // after the control screen is visible.
...
function renderMultiScreenPreviews(){
    ...
    select.onchange=()=>{multiScreenPreviewCount=Math.max(1,Math.min(4,Number(select.value)||4));
        ...
        try{localStorage.setItem("chordioMultiScreenPreviewCount",String(multiScreenPreviewCount));localStorage.setItem("chordioMultiScreenPreviewSlots",JSON.stringify(multiScreenPreviewSlots));}catch(_){} void saveMultiScreenServiceConfig({previewCount:multiScreenPreviewCount,previewSlots:multiScreenPreviewSlots}); renderMultiScreenPreviews();};
    ...
    [1,2,3,4].forEach(n=>{const b=document.createElement("button");b.type="button";b.textContent=String(n);b.className=slots.includes(n)?"active":"";b.title=slots.includes(n)?`Hide Screen ${n}`:`Show Screen ${n}`;b.onclick=()=>{
        ...
        void saveMultiScreenServiceConfig({previewCount:multiScreenPreviewCount,previewSlots:multiScreenPreviewSlots});
...
function multiScreenSelectSection(index,label=""){
    ...
    void saveMultiScreenServiceConfig({activeSectionIndex:multiScreenCurrentSection});
...
function saveMultiScreenBackground(){try{localStorage.setItem("chordioMultiScreenBackground",JSON.stringify(multiScreenBackground));}catch(_){}void saveMultiScreenServiceConfig({background:multiScreenBackground});multiScreenBroadcast({background:{...multiScreenBackground},songBackground:getMultiScreenSongBackground(song)});}
async function setMultiScreenBackground(type,url,forSong=false){
    ...
    if(forSong&&song){
        ...
        void saveMultiScreenServiceConfig({songBackgrounds:{[multiScreenSongBackgroundKey(song)]:bg}});
        const serviceId=localStorage.getItem("currentServiceId");
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        [1,2,3,4].forEach(n=>{document.getElementById(`multiMode${n}`)?.addEventListener("change",e=>{multiScreenModes[n]=e.target.value;try{localStorage.setItem("chordioMultiScreenModes",JSON.stringify(multiScreenModes));}catch(_){}void saveMultiScreenServiceConfig({modes:{[n]:multiScreenModes[n]}});renderMultiScreenPreviews();multiScreenBroadcast({});});document.getElementById(`multiBackgroundMode${n}`)?.addEventListener("change",e=>{multiScreenBackgroundModes[n]=e.target.value||"common";try{localStorage.setItem("chordioMultiScreenBackgroundModes",JSON.stringify(multiScreenBackgroundModes));}catch(_){}void saveMultiScreenServiceConfig({backgroundModes:{[n]:multiScreenBackgroundModes[n]}});renderMultiScreenPreviews();multiScreenBroadcast({});});document.getElementById(`multiEnabled${n}`)?.addEventListener("change",e=>{multiScreenEnabled[n]=e.target.checked;try{localStorage.setItem("chordioMultiScreenEnabled",JSON.stringify(multiScreenEnabled));}catch(_){}void saveMultiScreenServiceConfig({enabled:{[n]:multiScreenEnabled[n]}});renderMultiScreenPreviews();if(!multiScreenEnabled[n]){try{multiScreenWindows[n]?.close();}catch(_){}}multiScreenBroadcast({});});document.querySelector(`[data-open-screen="${n}"]`)?.addEventListener("click",()=>multiScreenOpenDisplay(n));});
