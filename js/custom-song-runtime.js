                ...
                <div class="line" data-section-index="${si}" data-line-index="${li}">
                    <div class="chords" aria-label="Chords">
                        ${line.chords.map((c, ci) => {
                            ...
                            return `<span class="chord" data-chord-id="${esc(c.id)}" data-original-chord="${esc(original)}" data-position="${c.position}" style="left:calc(${Number(c.position) || 0} * 0.6em)">${esc(displayed)}</span>`;
                        }).join("")}
                    </div>
