...
function offlineBibleNormalizeRef(value){return String(value||"").trim().toLowerCase().replace(/[–—]/g,"-").replace(/\s+/g," ");}
function getOfflineBibleEntries(){
    const sources=[window.CHORDIO_OFFLINE_BIBLE,window.CHORDIO_BIBLE_LOCAL];
    for(const src of sources){if(src&&typeof src==="object")return src;}
    return {};
}
function normalizeOfflineBibleParsed(parsed, fallbackVersion="WEB"){
    if(Array.isArray(parsed))return {[fallbackVersion]:parsed};
    if(parsed&&typeof parsed==="object"&&Array.isArray(parsed.books)){
        const out=[];
        parsed.books.forEach(book=>{(book.chapters||[]).forEach(ch=>{(ch.verses||[]).forEach(v=>{const ref=`${book.name} ${ch.chapter}:${v.verse}`;out.push({reference:ref,book:book.name,chapter:Number(ch.chapter),verse:Number(v.verse),text:String(v.text||v.value||"")});});});});
        return {[parsed.version||fallbackVersion]:out};
    }
    return parsed&&typeof parsed==="object"?parsed:{};
}
function parseOfflineVpl(text,version="WEB"){
    const out=[];
    String(text||"").split(/\r?\n/).forEach(line=>{
        const raw=line.trim();if(!raw||raw.startsWith("#"))return;
        const m=raw.match(/^(.+?\s+\d{1,3}:\d{1,3})[\t| ]+(.*)$/);
        if(!m)return;
        const reference=m[1].trim(), verseText=m[2].trim();
        const bm=reference.match(/^(.+?)\s+\d{1,3}:\d{1,3}$/);
        out.push({reference,book:bm?bm[1]:reference,text:verseText});
    });
    return {[version]:out};
}
function loadOfflineBibleFile(file){
    if(!file)return;
    ...
    reader.onload=()=>{
        try{
            ...
            let parsed;
            if(/\.(vpl|txt)$/i.test(file.name)) parsed=parseOfflineVpl(raw,"WEB");
            else parsed=normalizeOfflineBibleParsed(JSON.parse(raw),"WEB");
            window.CHORDIO_OFFLINE_BIBLE=parsed;
            try{localStorage.setItem("chordioOfflineBible",JSON.stringify(parsed));}catch(_){}
            const version=document.getElementById("multiBibleVersion");if(version&&parsed.WEB)version.value="WEB";
            searchOfflineBible();
            const r=document.getElementById("multiBibleResults");if(r)r.insertAdjacentHTML("afterbegin",'<span class="multi-bible-loaded">Offline Bible loaded</span>');
        }catch(_){alert("The local Bible file could not be read. Use JSON or a verse-per-line VPL/TXT file.");}
    };
    reader.readAsText(file);
}
function initOfflineBibleStore(){try{const saved=JSON.parse(localStorage.getItem("chordioOfflineBible")||"null");if(saved&&typeof saved==="object")window.CHORDIO_OFFLINE_BIBLE=saved;}catch(_){} }
function searchOfflineBible(){
    const version=document.getElementById("multiBibleVersion")?.value||"MBBTAG";
    const ref=String(document.getElementById("multiBibleReference")?.value||"").trim();
    const results=document.getElementById("multiBibleResults"),textBox=document.getElementById("multiBibleText");
    if(!ref){if(results)results.textContent="Enter a Bible reference first.";return;}
    const db=getOfflineBibleEntries(),list=Array.isArray(db?.[version])?db[version]:[];
    const q=offlineBibleNormalizeRef(ref);
    const matches=list.filter(x=>offlineBibleNormalizeRef(x.reference).includes(q)||offlineBibleNormalizeRef(x.book).includes(q));
    if(results){results.innerHTML="";if(!matches.length){results.textContent="No matching offline scripture was found. Add/import your licensed local MBB/NIV Bible data, or enter the scripture text below.";}else matches.slice(0,30).forEach(item=>{const b=document.createElement("button");b.type="button";b.className="multi-bible-result";b.textContent=`${item.reference} — ${version==="NIV"?"NIV":"MBB"}`;b.onclick=()=>{if(textBox)textBox.value=String(item.text||"");if(document.getElementById("multiSlideTitle"))document.getElementById("multiSlideTitle").value=`${item.reference} — ${version==="NIV"?"NIV":"MBB"}`;if(document.getElementById("multiSlideText"))document.getElementById("multiSlideText").value=String(item.text||"");};results.appendChild(b);});}
    if(matches.length===1&&textBox)textBox.value=String(matches[0].text||"");
}
async function addBibleReferenceSlide(){const version=document.getElementById("multiBibleVersion")?.value||"MBBTAG";const ref=String(document.getElementById("multiBibleReference")?.value||"").trim();const text=String(document.getElementById("multiBibleText")?.value||"").trim();if(!ref)return alert("Enter a Bible reference first.");if(!text){searchOfflineBible();return alert("No offline scripture text is available for this reference. Please import your licensed local Bible data or enter the verse text.");}const label=version==="NIV"?"NIV":version==="WEB"?"WEB":"MBB";const slide={id:`scripture-${Date.now()}`,title:`${ref} — ${label}`,text,scriptureReference:ref,scriptureVersion:version};const slides=[...currentServiceSongSlides(),slide];await saveServiceSongs(service.songs.map((x,i)=>i===index?{...x,pageSlides:slides}:x));multiSelectedPageSlideIndex=slides.length-1;renderMultiSlides();document.getElementById("multiSlideTitle").value=slide.title;document.getElementById("multiSlideText").value=slide.text;setMultiSlideStatus("Offline scripture added as slide");}
...
function multiScreenOpenDisplay(displayNumber){
    ...
    const selectedSongId=String(song.id||"");
    const url=new URL(location.href);url.searchParams.set("multiScreen","1");url.searchParams.set("display",String(n));url.searchParams.set("id",selectedSongId);url.searchParams.set("mode",mode);url.searchParams.set("ts",String(Date.now()));
    const features="popup=yes,resizable=yes,scrollbars=no,width=1280,height=720";let w=multiScreenWindows[n];
    if(!w||w.closed)w=window.open(url.href,`CHORDIO_SCREEN_${n}`,features);else{try{w.location.replace(url.href);w.focus();}catch(_){} }
    multiScreenWindows[n]=w;
    const fresh=multiScreenMessage();try{w?.postMessage(fresh,"*");}catch(_){}
    setTimeout(()=>{try{w?.postMessage(multiScreenMessage(),"*");}catch(_){}},500);
}
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        try{const cached=JSON.parse(localStorage.getItem("chordioMultiScreenState")||"null");if(cached&&(!expectedSongId||String(cached?.song?.id||"")===expectedSongId))acceptState(cached);}catch(_){}
        ...
        document.querySelectorAll("[data-screen-settings]").forEach(btn=>btn.addEventListener("click",()=>{const n=btn.getAttribute("data-screen-settings");const panel=document.getElementById(`multiScreenSettings${n}`);if(panel)panel.classList.toggle("open");}));
        ...
        document.getElementById("multiBibleSearch")?.addEventListener("click",searchOfflineBible);
        document.getElementById("multiBibleFile")?.addEventListener("change",e=>loadOfflineBibleFile(e.target.files?.[0]));
