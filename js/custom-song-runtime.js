...
async function updateCmgBackgroundPreview(){
    ...
    if(value.startsWith("idb://")){
        ...
        if(String(item.type).startsWith("video/")){const v=document.createElement("video");v.src=url;v.autoplay=true;v.muted=true;v.loop=true;v.playsInline=true;v.style.cssText="width:100%;height:100%;object-fit:cover";preview.appendChild(v);}
        else{const img=document.createElement("img");img.alt=item.name;img.src=url;preview.appendChild(img);}
        return;
    }
    const img=document.createElement("img");img.alt=value;img.src=chordioCmgBackgroundUrl(value);img.onerror=()=>{preview.innerHTML='<div class="multi-cmg-preview-empty">Unable to load this background.</div>';};preview.appendChild(img);
}
async function useSelectedCmgBackground(){
    ...
    if(value.startsWith("idb://")){const item=await chordioGetUploadedBackground(value.slice(6));if(!item)return;type=String(item.type).startsWith("video/")?"video":"image";}else url=chordioCmgBackgroundUrl(value);
    ...
    void setMultiScreenBackground(type,url,scope==="song");
}
