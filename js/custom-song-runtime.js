...
async function redirectSong(nextSong, targetIndex=index){
    ...
    if(presentation?.classList.contains("show") && service?.songs?.[targetIndex]){
        ...
        try{
            let master=null;
            if(candidate?.id){
                const snap=await getDoc(doc(db,"songs",String(candidate.id)));
                if(snap.exists()) master={id:snap.id,...snap.data()};
            }
            song={
                ...candidate,
                ...(master||{}),
                sections:normalizeSections((master||candidate)?.sections),
            ...
            console.warn("Fast presentation navigation failed; using normal navigation.",error);
        }
    }
...
async function loadFromService(){
    ...
    try {
        ...
        if(candidate.customSong===true || String(candidate.file||"").startsWith("custom-song.html")){
            void getDoc(doc(db,"songs",String(candidate.id||""))).then(masterSnap=>{
