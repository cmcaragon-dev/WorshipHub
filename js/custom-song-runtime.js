...
function renderMultiScreenPreviews(){
    ...
    [1,2,3,4].forEach(n=>{
        ...
        const head=document.createElement("div");head.className="multi-screen-preview-card-head";head.innerHTML=`<strong>SCREEN ${n}</strong><span>${multiScreenEnabled[n]===false?"HIDDEN":multiScreenModeLabel(multiScreenModes[n])}</span>`;const settingsBtn=document.createElement("button");settingsBtn.type="button";settingsBtn.className="multi-preview-settings-button";settingsBtn.textContent="⚙ SETTINGS";settingsBtn.setAttribute("data-preview-settings",String(n));settingsBtn.onclick=()=>{const panel=document.getElementById(`multiScreenSettings${n}`);if(panel){document.querySelectorAll("#multiScreenControl .multi-screen-settings-panel.open").forEach(p=>{if(p!==panel)p.classList.remove("open");});panel.classList.toggle("open");}};head.appendChild(settingsBtn);card.appendChild(head);
        ...
        if(multiScreenEnabled[n]===false) stage.innerHTML='<span class="multi-preview-empty">Screen disabled</span>';
            ...
            if(bg.type==="image"&&bg.url){stage.style.setProperty("background-image",`linear-gradient(rgba(0,0,0,.32),rgba(0,0,0,.32)),url("${String(bg.url).replace(/"/g,'\"')}")`,"important");stage.style.setProperty("background-size","cover","important");stage.style.setProperty("background-position","center","important");}
            ...
            if(multiScreenModes[n]==="lyrics"){
                ...
                const l=document.createElement("div");l.className="preview-selected-lyrics";l.style.lineHeight=String(settings.spacing||1.15);l.style.fontSize=`${Math.max(.65,Number(settings.fontPx||42)/42)}em`;l.style.paddingTop=`${Math.max(0,Number(settings.vertical||50)-50)/4}%`;
...
function renderMultiScreenOutput(message){
    ...
    if(mode==="lyrics"){
        ...
        // Lyrics + Chords uses the SAME presentation structure/classes as the
        // normal CHORDIO Presentation. The whole song is rendered; section
        // selection only changes the navigation/scroll target.
        output.className="multi-output-chords-stage";
        ...
        visible.forEach((entry,i)=>{
            ...
            el.className="custom-presentation-section"+(i===idx?" current-part":"");
            el.dataset.sectionIndex=String(i);
            el.id=`multiPresentationSection-${i}`;
            const title=document.createElement("div");
            title.className="custom-presentation-section-title";
            title.textContent=presentationSectionLabel(sec,i);
        ...
        const typ=getMultiLyricsSettings();
        output.querySelectorAll(".custom-presentation-lyric").forEach(ly=>{ly.style.fontFamily=typ.fontFamily||"Arial";ly.style.fontSize=`${Math.max(10,Number(typ.fontPx||42))}px`;ly.style.letterSpacing=`${Number(typ.letterSpacing||0)}px`;});
        output.querySelectorAll(".custom-presentation-lyric").forEach(ly=>{ly.style.color=typ.color||"#FFFFFF";ly.style.fontWeight=typ.bold?"800":"400";ly.style.fontStyle=typ.italic?"italic":"normal";ly.style.textDecoration=typ.underline?"underline":"none";ly.style.lineHeight=String(Math.max(.7,Math.min(2.4,Number(typ.spacing)||1.15)));});
        document.getElementById("multiOutputSection").textContent=visible[idx]?presentationSectionLabel(visible[idx].section,idx):"";
        requestAnimationFrame(()=>{
            const target=output.querySelector(`[data-section-index="${idx}"]`);
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        const acceptState=state=>{const incomingId=String(state?.song?.id||"");
            // Before the first valid handshake, ignore stale BroadcastChannel/localStorage data.
            // After that handshake, allow song changes from the controller normally.
