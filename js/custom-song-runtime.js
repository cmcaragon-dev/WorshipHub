...
async function loadFromService(){
    ...
    try {
        ...
        if(serviceSongsChanged || presentationLayoutsChanged){
            ...
            if(auth.currentUser){
                try{await setDoc(doc(db,"users",auth.currentUser.uid,"services",String(serviceId)),{songs:service.songs,presentationLayouts:migratedLayouts,updatedAt:serverTimestamp()},{merge:true});}catch(error){console.warn("Unable to migrate Service Planner song instances:",error);}
            }
        }
