...
function renderMultiScreenOutput(message){
    ...
    if(mode==="lyrics"){
        ...
        // Lyrics + Chords: mirror the PRESENTATION view. Render the WHOLE song
        // in the same section order/layout, then scroll to the section selected
        // by the service controls. This lets Verse 1 / Chorus / Bridge buttons
        // navigate the output without rebuilding it as a single-section screen.
        if(!visible.length){output.innerHTML='<div class="multi-output-empty-state">No lyrics or chords available for this song.</div>';return;}
        output.className="multi-output-chords-stage";output.style.textAlign="left";output.style.paddingTop="0";
        visible.forEach((entry,sectionIndex)=>{
            const sec=entry.section;
            const sectionEl=document.createElement("section");
            sectionEl.className="custom-presentation-section"+(sectionIndex===idx?" current-part":"");
            ...
            (sec.lines||[]).forEach((line,lineIndex)=>{
                ...
                const ly=document.createElement("div");
                ly.className="custom-presentation-lyric";
                ly.textContent=lyricLines?String(lyricLines[lineIndex]??""):String(line.lyrics||"");
                pair.appendChild(ly);
                sectionEl.appendChild(pair);
            });
            output.appendChild(sectionEl);
        });
        const selectedSection=output.querySelector(`#multiOutputPresentationSection-${idx}`)||output.firstElementChild;
        document.getElementById("multiOutputSection").textContent=presentationSectionLabel(visible[idx]?.section||visible[0].section,idx);
