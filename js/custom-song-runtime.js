...
function chordioCmgBackgroundUrl(name){
    return "backgrounds/cmg/"+encodeURIComponent(String(name||"")).replace(/%2F/g,"/");
}
function initCmgBackgroundLibrary(){
    const select=document.getElementById("multiCmgBackground");
    if(!select||select.dataset.ready==="1")return;
    select.innerHTML='<option value="">— Select a CMG background —</option>'+
        CHORDIO_CMG_BACKGROUNDS.map(name=>`<option value="${esc(name)}">${esc(name.replace(/^CMG\s*-\s*/i,""))}</option>`).join("");
    select.dataset.ready="1";
    updateCmgBackgroundPreview();
}
function updateCmgBackgroundPreview(){
    ...
    if(!name){preview.innerHTML='<div class="multi-cmg-preview-empty">Select a CMG background to preview it.</div>';return;}
    preview.innerHTML="";
    const img=document.createElement("img");img.alt=name;img.src=chordioCmgBackgroundUrl(name);
    img.onerror=()=>{preview.innerHTML='<div class="multi-cmg-preview-empty">Unable to load this background.</div>';};
    preview.appendChild(img);
}
function useSelectedCmgBackground(){
    const name=String(document.getElementById("multiCmgBackground")?.value||"").trim();
    if(!name)return;
    const scope=document.getElementById("multiBackgroundScopeSelect")?.value||"all";
    const url=chordioCmgBackgroundUrl(name);
    const type=document.getElementById("multiBackgroundType");if(type)type.value="image";
    const input=document.getElementById("multiBackgroundUrl");if(input)input.value=url;
    void setMultiScreenBackground("image",url,scope==="song");
}
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        document.getElementById("multiCmgBackground")?.addEventListener("change",updateCmgBackgroundPreview);
        document.getElementById("multiCmgBackgroundApply")?.addEventListener("click",useSelectedCmgBackground);
        initCmgBackgroundLibrary();
