...
function multiScreenMessage(){
    return {type:"chordio-multiscreen-state",song:song?{...song,sections:normalizeSections(song.sections)}:null,serviceName:service?.name||"",serviceIndex:index,serviceSongs:service?.songs||[],sectionIndex:multiScreenCurrentSection,modes:{...multiScreenModes},enabled:{...multiScreenEnabled},backgroundModes:{...multiScreenBackgroundModes},background:{...multiScreenBackground},songBackground:getMultiScreenSongBackground(song),layout:getPresentationLayout(),lyricsSettings:getMultiLyricsSettings(),lyricsOverrides:getMultiLyricsOverrides()};
}
...
function fitMultiLyricsToScreen(output, section, settings){
    ...
    const initial=Math.max(16,Math.min(140,base*userScale));
    let size=initial;
...
function renderMultiScreenOutput(message){
    ...
    if(mode==="lyrics"){
        ...
        // Lyrics + Chords always renders the FULL SONG. The selected Verse/Chorus/Bridge
        // is only the navigation target; it is scrolled into view instead of replacing the song.
        output.className="multi-output-chords-stage";output.style.textAlign="left";output.style.paddingTop="30px";
        visible.forEach((entry,i)=>{
            const sec=entry.section;
            const el=document.createElement("section");
            el.className="multi-output-section"+(i===idx?" current-part":"");
            el.dataset.sectionIndex=String(i);
            const title=document.createElement("div");
            title.className="multi-output-section-title";
            title.textContent=presentationSectionLabel(sec,i);
            el.appendChild(title);
            (sec.lines||[]).forEach(line=>{
                const row=document.createElement("div");
                row.className="multi-output-line";
                row.style.fontSize=`${Math.max(16,Math.min(140,58*(Number(settings.font||100)/100)))}px`;
                row.style.lineHeight=String(settings.spacing||1.15);
                const ch=document.createElement("div");
                ch.className="multi-output-chord";
                ch.textContent=chordRowFromPositions(line);
                if(ch.textContent)row.appendChild(ch);
                const ly=document.createElement("div");
                ly.className="multi-output-lyric";
                ly.textContent=String(line.lyrics||"");
                row.appendChild(ly);
                el.appendChild(row);
            });
        ...
        // Scroll only after the complete song has been laid out.
