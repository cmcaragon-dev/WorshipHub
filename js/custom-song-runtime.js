...
function multiScreenMessage(){
    return {type:"chordio-multiscreen-state",song:song?{...song,sections:normalizeSections(song.sections)}:null,serviceName:service?.name||"",serviceIndex:index,serviceSongs:service?.songs||[],sectionIndex:multiScreenCurrentSection,modes:{...multiScreenModes},enabled:{...multiScreenEnabled},backgroundModes:{...multiScreenBackgroundModes},background:{...multiScreenBackground},songBackground:getMultiScreenSongBackground(song),layout:getPresentationLayout(),lyricsSettings:getMultiLyricsSettings(),lyricsOverrides:getMultiLyricsOverrides(),activePageSlide:multiActivePageSlide};
}
...
const CHORDIO_BIBLE_TRANSLATIONS={NIV:{label:"English — NIV",gateway:"NIV"},MBBTAG:{label:"Tagalog — Magandang Balita Biblia",gateway:"MBBTAG"},ADB1905:{label:"Tagalog — Ang Dating Biblia (1905)",gateway:"ADB1905"}};
function initOnlineBibleControls(){const list=document.getElementById("multiBibleBookList");if(list&&!list.dataset.ready){list.innerHTML=CHORDIO_BIBLE_BOOKS.map(book=>`<option value="${esc(book)}"></option>`).join("");list.dataset.ready="1";}const version=document.getElementById("multiBibleVersion");if(version&&!version.dataset.ready){version.innerHTML=Object.entries(CHORDIO_BIBLE_TRANSLATIONS).map(([key,v])=>`<option value="${key}">${esc(v.label)}</option>`).join("");version.value=localStorage.getItem("chordioOnlineBibleVersion")||"NIV";version.dataset.ready="1";}updateOnlineBibleReferencePreview();}
function getOnlineBibleReference(){const book=String(document.getElementById("multiBibleBook")?.value||"").trim(),chapter=String(document.getElementById("multiBibleChapter")?.value||"").trim(),verse=String(document.getElementById("multiBibleVerse")?.value||"").trim().replace(/[–—]/g,"-").replace(/\s+/g,"");if(!book||!chapter||!verse||!/^\d+$/.test(chapter)||!/^\d+(?:-\d+)?$/.test(verse))return "";return `${book} ${chapter}:${verse}`;}
function getOnlineBibleVersion(){return String(document.getElementById("multiBibleVersion")?.value||"NIV").trim()||"NIV";}
function updateOnlineBibleReferencePreview(){const el=document.getElementById("multiBibleReferencePreview"),version=getOnlineBibleVersion(),label=CHORDIO_BIBLE_TRANSLATIONS[version]?.label||"NIV";if(el)el.textContent=`Reference: ${getOnlineBibleReference()||"—"}  •  ${label}`;}
function cleanBibleGatewayText(text){return String(text||"").replace(/\^\{[^}]*\}/g,"").replace(/\^\[[^\]]*\]/g,"").replace(/\^\([^)]*\)/g,"").replace(/\[[A-Z]{1,3}\]/g,"").replace(/\s+/g," ").trim();}
function parseBibleGatewayReader(text,reference){
    const raw=String(text||"").replace(/\r/g,""),m=reference.match(/^(.*?)\s+(\d+):(\d+(?:-\d+)?)$/);
    if(!m)return "";
    const chapter=m[2],verse=m[3],verseStart=verse.split("-")[0];
    const marker=new RegExp(String.raw`\b${chapter}\s*:\s*${verse.replace("-",String.raw`\s*-\s*`)}\b`,'i');
    const pos=raw.search(marker);if(pos<0)return "";
    let body=raw.slice(pos+raw.slice(pos).match(marker)[0].length);
    body=body.split(/\n(?:Read full chapter|Cross references|Footnotes)\b/i)[0];
    body=body.replace(/^\s*(?:New International Version|Magandang Balita Biblia|Ang Dating Biblia(?: \(1905\))?)\s*/i,"");
    body=body.replace(/(?:Holy Bible, New International Version|NIV®? Copyright|Copyright ©)[\s\S]*$/i,"");
    body=body.replace(/\^\{[^}]*\}/g,"").replace(/\^\[[^\]]*\]/g,"").replace(/\^\([^)]*\)/g,"").replace(/\[[A-Z]{1,3}\]/g,"");
    body=body.replace(/\s+/g," ").trim();
    if(!body)return "";
    // Preserve verse numbers for ranges, but remove the first heading marker if the source repeats it.
    body=body.replace(new RegExp(`^${chapter}\s*:\s*${verseStart}\s*`,'i'),"").trim();
    return body;
}
async function fetchBibleGateway(reference,version){const target=`https://www.biblegateway.com/passage/?search=${encodeURIComponent(reference)}&version=${encodeURIComponent(version)}`,response=await fetch(`https://r.jina.ai/${target}`,{headers:{Accept:"text/plain"}});if(!response.ok)throw new Error(`HTTP ${response.status}`);const text=parseBibleGatewayReader(await response.text(),reference);if(!text)throw new Error("Verse text not found");return {reference,text,version};}
function populateOnlineBibleResult(reference,data){const results=document.getElementById("multiBibleResults"),textBox=document.getElementById("multiBibleText"),version=data?.version||getOnlineBibleVersion(),label=CHORDIO_BIBLE_TRANSLATIONS[version]?.label||version,text=String(data?.text||"").trim();if(textBox)textBox.value=text;if(results){results.innerHTML="";const button=document.createElement("button");button.type="button";button.className="multi-bible-result";button.textContent=`${reference} — ${label}`;button.onclick=()=>{if(textBox)textBox.value=text;};results.appendChild(button);}const title=document.getElementById("multiSlideTitle"),slideText=document.getElementById("multiSlideText");if(title)title.value=`${reference} — ${label}`;if(slideText)slideText.value=text;return text;}
async function fetchOnlineBibleVerse(){const reference=getOnlineBibleReference(),version=getOnlineBibleVersion(),results=document.getElementById("multiBibleResults"),textBox=document.getElementById("multiBibleText"),label=CHORDIO_BIBLE_TRANSLATIONS[version]?.label||version;if(!reference){if(results)results.textContent="Enter Book, Chapter and Verse. Verse can be a single number or a range such as 1-5.";return null;}try{localStorage.setItem("chordioOnlineBibleVersion",version);}catch(_){}if(results)results.textContent=`Searching ${label} online…`;try{return populateOnlineBibleResult(reference,{...(await fetchBibleGateway(reference,CHORDIO_BIBLE_TRANSLATIONS[version]?.gateway||"NIV")),version});}catch(error){console.warn("Online Bible lookup failed:",error);if(results)results.textContent=`Unable to retrieve ${label}. Check the reference and make sure internet access is available.`;if(textBox)textBox.value="";return null;}}
async function addBibleReferenceSlide(){const reference=getOnlineBibleReference(),version=getOnlineBibleVersion(),label=CHORDIO_BIBLE_TRANSLATIONS[version]?.label||version;if(!reference){alert("Enter Book, Chapter and Verse first. Example: John → 3 → 16 or John → 3 → 1-5.");return;}let text=String(document.getElementById("multiBibleText")?.value||"").trim();if(!text)text=await fetchOnlineBibleVerse();if(!text)return;if(!service?.songs?.[index]){alert("Open a Service Planner song first.");return;}const slide={id:`scripture-${Date.now()}`,title:`${reference} — ${label}`,text,scriptureReference:reference,scriptureVersion:version},slides=[...currentServiceSongSlides(),slide];await saveServiceSongs(service.songs.map((x,i)=>i===index?{...x,pageSlides:slides}:x));multiSelectedPageSlideIndex=slides.length-1;renderMultiSlides();const title=document.getElementById("multiSlideTitle"),slideText=document.getElementById("multiSlideText");if(title)title.value=slide.title;if(slideText)slideText.value=slide.text;setMultiSlideStatus(`${label} scripture added as slide`);}
...
function renderMultiScreenPreviews(){
    ...
    select.onchange=()=>{multiScreenPreviewCount=Math.max(1,Math.min(4,Number(select.value)||4));multiScreenPreviewExpanded=0;try{localStorage.setItem("chordioMultiScreenPreviewCount",String(multiScreenPreviewCount));}catch(_){}renderMultiScreenPreviews();};
    const hint=document.createElement("span");hint.textContent="Double-click a screen to enlarge it.";
    toolbar.append(label,select,hint);box.appendChild(toolbar);
    ...
    screens.forEach(n=>{
        ...
        if(multiScreenEnabled[n]===false) stage.innerHTML='<span class="multi-preview-empty">Screen disabled</span>';
            ...
            if(bg.type==="image"&&bg.url){stage.style.setProperty("background-image",`linear-gradient(rgba(0,0,0,.32),rgba(0,0,0,.32)),url(${JSON.stringify(String(bg.url))})`,"important");stage.style.setProperty("background-size","cover","important");stage.style.setProperty("background-position","center","important");}
            ...
            if(multiScreenModes[n]==="lyrics"){
                ...
                const l=document.createElement("div");l.className="preview-selected-lyrics";l.style.lineHeight=String(settings.spacing||1.15);l.style.fontFamily=settings.fontFamily||"Arial";l.style.fontWeight=settings.bold?"800":"400";l.style.fontStyle=settings.italic?"italic":"normal";l.style.textDecoration=settings.underline?"underline":"none";l.style.color=settings.color||"#fff";l.style.letterSpacing=`${Number(settings.letterSpacing||0)}px`;l.style.fontSize=`${Math.max(10,Math.min(80,Number(settings.fontPx||42)))}px`;l.style.paddingTop=`${Math.max(0,Number(settings.vertical||50)-50)/4}%`;
...
function saveMultiScreenBackground(){try{localStorage.setItem("chordioMultiScreenBackground",JSON.stringify(multiScreenBackground));}catch(_){}multiScreenBroadcast({background:{...multiScreenBackground},songBackground:getMultiScreenSongBackground(song)});}
async function setMultiScreenBackground(type,url,forSong=false){
    ...
    }else{multiScreenBackground=bg;saveMultiScreenBackground();}
...
function multiScreenOpenDisplay(displayNumber){
    ...
    const url=new URL(location.href);url.searchParams.set("multiScreen","1");url.searchParams.set("display",String(n));url.searchParams.set("id",String(song.id||""));url.searchParams.set("mode",mode);url.searchParams.set("ts",String(Date.now()));
    const features="popup=yes,resizable=yes,scrollbars=no,width=1280,height=720";let w=multiScreenWindows[n];if(!w||w.closed)w=window.open(url.href,`CHORDIO_SCREEN_${n}`,features);else{try{w.location.replace(url.href);w.focus();}catch(_){}}multiScreenWindows[n]=w;
    const send=()=>{try{w?.postMessage({...multiScreenMessage(),display:String(n)},"*");}catch(_){} };send();setTimeout(send,250);setTimeout(send,800);setTimeout(send,1600);
}
...
function fitMultiLyricsToScreen(output, section, settings){
    ...
    const requestedFont=Math.max(10,Math.min(140,Number(settings.fontPx||42)));
    ...
    const base=Math.min(width*0.072,height*0.105);
    const initial=Math.max(10,Math.min(140,requestedFont));
    let size=initial;
    ...
    // 50% is centered by default. The Vertical Position control distributes
    // the remaining free space from top (0%) to bottom (100%).
    const contentHeight=section.scrollHeight;
    const freeSpace=Math.max(0,height-contentHeight);
    const topSpace=Math.round(freeSpace*(vertical/100));
    section.style.justifyContent="flex-start";
    section.style.paddingTop=`${topSpace}px`;
    section.style.paddingBottom=`${Math.max(0,freeSpace-topSpace)}px`;
...
function renderMultiScreenOutput(message){
    ...
    if(mode==="lyrics"){
        ...
        // Lyrics + Chords: show the SAME currently selected section used by
        // the Lyrics Only output. Chords stay directly above their lyrics.
        const entry=visible[idx]||visible[0];
        if(!entry){output.innerHTML='<div class="multi-output-empty-state">No lyrics or chords available for this song.</div>';return;}
        const sec=entry.section;
        output.className="multi-output-chords-stage";output.style.textAlign="left";output.style.paddingTop="0";
        const sectionEl=document.createElement("section");sectionEl.className="custom-presentation-section current-part";sectionEl.dataset.sectionIndex=String(idx);
        const title=document.createElement("div");title.className="custom-presentation-section-title";title.textContent=presentationSectionLabel(sec,idx);sectionEl.appendChild(title);
        const lyricLines=Array.isArray(outputLyricsOverrides[String(entry.item.id)])?outputLyricsOverrides[String(entry.item.id)]:null;
        (sec.lines||[]).forEach((line,lineIndex)=>{
            ...
            const ly=document.createElement("div");ly.className="custom-presentation-lyric";ly.textContent=lyricLines?String(lyricLines[lineIndex]??""):String(line.lyrics||"");
            ly.style.fontFamily=settings.fontFamily||"Arial";ly.style.fontSize=`${Math.max(10,Number(settings.fontPx||42))}px`;ly.style.letterSpacing=`${Number(settings.letterSpacing||0)}px`;ly.style.color=settings.color||"#FFFFFF";ly.style.fontWeight=settings.bold?"800":"400";ly.style.fontStyle=settings.italic?"italic":"normal";ly.style.textDecoration=settings.underline?"underline":"none";ly.style.lineHeight=String(Math.max(.7,Math.min(2.4,Number(settings.spacing)||1.15)));
            pair.appendChild(ly);sectionEl.appendChild(pair);
        });
        output.appendChild(sectionEl);
        document.getElementById("multiOutputSection").textContent=presentationSectionLabel(sec,idx);
        requestAnimationFrame(()=>sectionEl.scrollIntoView({block:"start",behavior:"auto"}));
    }
}
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        setTimeout(()=>{try{window.opener?.postMessage({type:"chordio-multiscreen-request",display:multiScreenDisplayId},"*");}catch(_){}},150);
        ...
        document.getElementById("multiBibleVersion")?.addEventListener("change",()=>{try{localStorage.setItem("chordioOnlineBibleVersion",getOnlineBibleVersion());}catch(_){}updateOnlineBibleReferencePreview();});
        ...
        document.getElementById("multiBackgroundApply")?.addEventListener("click",()=>{const type=document.getElementById("multiBackgroundType")?.value||"none";const url=document.getElementById("multiBackgroundUrl")?.value.trim()||"";const scope=document.getElementById("multiBackgroundScopeSelect")?.value||"all";if(scope==="song")void setMultiScreenBackground(type,url,true);else void setMultiScreenBackground(type,url,false);});
        document.getElementById("multiBackgroundClear")?.addEventListener("click",()=>{const scope=document.getElementById("multiBackgroundScopeSelect")?.value||"all";if(scope==="song")clearMultiScreenSongBackground();else{multiScreenBackground={type:"none",url:""};saveMultiScreenBackground();renderMultiBackgroundControls();renderMultiScreenPreviews();}});
        ...
        initOnlineBibleControls();
        document.getElementById("multiBackgroundFile")?.addEventListener("change",e=>{const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const scope=document.getElementById("multiBackgroundScopeSelect")?.value||"all";void setMultiScreenBackground(file.type.startsWith("video/")?"video":"image",String(reader.result||""),scope==="song");};reader.readAsDataURL(file);});
        document.getElementById("multiBackgroundSongFile")?.addEventListener("change",e=>{const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>void setMultiScreenBackground(file.type.startsWith("video/")?"video":"image",String(reader.result||""),true);reader.readAsDataURL(file);});
    }
}
