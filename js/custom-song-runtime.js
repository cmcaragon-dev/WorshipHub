...
async function chordioSaveUploadedBackground(file){
    ...
    if(!/^(image|video)\//i.test(String(file.type||"")))throw new Error("Only image and video files are supported.");
    ...
    await uploadBytes(objectRef,file,{contentType:file.type||"application/octet-stream",customMetadata:{ownerUid:auth.currentUser.uid}});
    ...
    const addCard=async(item,isUpload)=>{
        ...
        card.onclick=async()=>{
            ...
            if(chordioIsUploadedBackgroundValue(value)){
                ...
                type=chordioIsVideoType(saved.type)?"video":"image";inputUrl=String(saved.downloadURL||"");
...
async function useSelectedCmgBackground(){
    ...
    if(chordioIsUploadedBackgroundValue(value)){const item=await chordioGetUploadedBackground(value.slice(5));if(!item)return;type=chordioIsVideoType(item.type)?"video":"image";url=String(item.downloadURL||"");}
