...
async function openChordioBackgroundLibrary(){
    ...
    const addCard=async(item,isUpload)=>{
        ...
        card.append(mediaWrap,label);card.onclick=async()=>{const select=document.getElementById("multiCmgBackground");if(select){select.value=card.dataset.value;await updateCmgBackgroundPreview();}modal.classList.remove("show");modal.setAttribute("aria-hidden","true");};
        return card;
    };
