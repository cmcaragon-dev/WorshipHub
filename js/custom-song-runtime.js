...
const CHORDIO_BIBLE_BOOKS=[
    "Genesis","Exodus","Leviticus","Numbers","Deuteronomy","Joshua","Judges","Ruth","1 Samuel","2 Samuel","1 Kings","2 Kings","1 Chronicles","2 Chronicles","Ezra","Nehemiah","Esther","Job","Psalms","Proverbs","Ecclesiastes","Song of Solomon","Isaiah","Jeremiah","Lamentations","Ezekiel","Daniel","Hosea","Joel","Amos","Obadiah","Jonah","Micah","Nahum","Habakkuk","Zephaniah","Haggai","Zechariah","Malachi","Matthew","Mark","Luke","John","Acts","Romans","1 Corinthians","2 Corinthians","Galatians","Ephesians","Philippians","Colossians","1 Thessalonians","2 Thessalonians","1 Timothy","2 Timothy","Titus","Philemon","Hebrews","James","1 Peter","2 Peter","1 John","2 John","3 John","Jude","Revelation"
];
function initOnlineBibleControls(){
    const list=document.getElementById("multiBibleBookList");
    if(list&&!list.dataset.ready){
        list.innerHTML=CHORDIO_BIBLE_BOOKS.map(book=>`<option value="${esc(book)}"></option>`).join("");
        list.dataset.ready="1";
    }
    updateOnlineBibleReferencePreview();
}
function getOnlineBibleReference(){
    const book=String(document.getElementById("multiBibleBook")?.value||"").trim();
    const chapter=String(document.getElementById("multiBibleChapter")?.value||"").trim();
    const verse=String(document.getElementById("multiBibleVerse")?.value||"").trim().replace(/[–—]/g,"-").replace(/\s+/g,"");
    if(!book||!chapter||!verse)return "";
    if(!/^\d+$/.test(chapter)||!/^\d+(?:-\d+)?$/.test(verse))return "";
    return `${book} ${chapter}:${verse}`;
}
function updateOnlineBibleReferencePreview(){
    const el=document.getElementById("multiBibleReferencePreview");
    if(el)el.textContent=`Reference: ${getOnlineBibleReference()||"—"}`;
}
function populateOnlineBibleResult(reference,data){
    const results=document.getElementById("multiBibleResults"),textBox=document.getElementById("multiBibleText");
    const verses=Array.isArray(data?.verses)?data.verses:[];
    const text=String(data?.text||verses.map(v=>`[${v.verse}] ${String(v.text||"").trim()}`).join(" ")||"").trim();
    if(textBox)textBox.value=text;
    if(results){
        results.innerHTML="";
        const button=document.createElement("button");
        button.type="button";button.className="multi-bible-result";
        button.textContent=`${data?.reference||reference} — WEB`;
        button.onclick=()=>{if(textBox)textBox.value=text;};
        results.appendChild(button);
    }
    const title=document.getElementById("multiSlideTitle"),slideText=document.getElementById("multiSlideText");
    if(title)title.value=`${data?.reference||reference} — WEB`;
    if(slideText)slideText.value=text;
    return text;
}
async function fetchOnlineBibleVerse(){
    const reference=getOnlineBibleReference();
    const results=document.getElementById("multiBibleResults"),textBox=document.getElementById("multiBibleText");
    if(!reference){
        if(results)results.textContent="Enter Book, Chapter and Verse. Verse can be a single number or a range such as 1-5.";
        return null;
    }
    if(results)results.textContent="Searching online WEB Bible…";
    try{
        const url=`https://bible-api.com/${encodeURIComponent(reference)}?translation=web&verse_numbers=true`;
        const response=await fetch(url,{headers:{Accept:"application/json"}});
        if(!response.ok)throw new Error(`HTTP ${response.status}`);
        const data=await response.json();
        if(!Array.isArray(data?.verses)||!data.verses.length)throw new Error("Verse not found");
        return populateOnlineBibleResult(reference,data);
    }catch(error){
        console.warn("Online WEB Bible lookup failed:",error);
        if(results)results.textContent="Unable to retrieve that verse online. Check the book, chapter and verse and make sure internet access is available.";
        if(textBox)textBox.value="";
        return null;
    }
}
async function addBibleReferenceSlide(){
    const reference=getOnlineBibleReference();
    if(!reference){alert("Enter Book, Chapter and Verse first. Example: John → 3 → 16 or John → 3 → 1-5.");return;}
    let text=String(document.getElementById("multiBibleText")?.value||"").trim();
    if(!text)text=await fetchOnlineBibleVerse();
    if(!text)return;
    if(!service?.songs?.[index]){alert("Open a Service Planner song first.");return;}
    const slide={id:`scripture-${Date.now()}`,title:`${reference} — WEB`,text,scriptureReference:reference,scriptureVersion:"WEB"};
    const slides=[...currentServiceSongSlides(),slide];
    await saveServiceSongs(service.songs.map((x,i)=>i===index?{...x,pageSlides:slides}:x));
    multiSelectedPageSlideIndex=slides.length-1;
    renderMultiSlides();
    const title=document.getElementById("multiSlideTitle"),slideText=document.getElementById("multiSlideText");
    if(title)title.value=slide.title;
    if(slideText)slideText.value=slide.text;
    setMultiSlideStatus("Online WEB scripture added as slide");
}
