...
function multiScreenOpenControl(){
    ...
    const existingSongQueueIndex=multiScreenQueueIndexForSong(song?.id);if(existingSongQueueIndex>=0)multiScreenQueueIndex=existingSongQueueIndex;
...
function multiScreenQueue(){
    ...
    const byPresentation=new Map(presentations.map(x=>[String(x?.id||""),x]));
    const raw=Array.isArray(service.multiScreenQueue)?service.multiScreenQueue:[];
    const queue=[];
    raw.forEach(ref=>{
        const type=String(ref?.type||"song");
        if(type==="presentation"){
            const id=String(ref?.id||"");const item=byPresentation.get(id);if(item)queue.push({type,id,item,sourceIndex:-1});
        }else{
            const sourceIndex=Number.isInteger(Number(ref?.songIndex))?Number(ref.songIndex):-1;
            if(sourceIndex>=0&&sourceIndex<songs.length&&songs[sourceIndex])queue.push({type:"song",id:String(songs[sourceIndex]?.id||""),item:songs[sourceIndex],sourceIndex});
            else{
                const id=String(ref?.id||"");const sourceIndex2=songs.findIndex(x=>String(x?.id||"")===id);
                if(sourceIndex2>=0)queue.push({type:"song",id,item:songs[sourceIndex2],sourceIndex:sourceIndex2});
            }
        }
    });
    // Backward-compatible migration: keep every Service Planner song occurrence,
    // including duplicate songs, and append newly-added songs to the queue.
    songs.forEach((x,i)=>{
        if(!queue.some(q=>q.type==="song"&&q.sourceIndex===i))
            queue.push({type:"song",id:String(x?.id||""),item:x,sourceIndex:i});
    });
    presentations.forEach(x=>{
        if(!queue.some(q=>q.type==="presentation"&&String(q.id)===String(x.id)))
            queue.push({type:"presentation",id:String(x.id||""),item:x,sourceIndex:-1});
    });
    return queue;
}
function multiScreenQueueIndexForSong(songId){
    const id=String(songId||"");
    return multiScreenQueue().findIndex(x=>x.type==="song"&&String(x.id)===id);
}
...
async function addPresentationEntryForSlide(slide){
    ...
    const presentation={id:`presentation-${slide.id}`,title:slide.title||"Presentation",artist:"Presentation",presentation:true,presentationForSongId:String(service.songs[index].id||""),presentationSourceSongTitle:String(service.songs[index].title||""),presentationSlide:{...slide},presentationSlideIndex:0};
    const presentations=[...(service.multiScreenPresentations||[]),presentation];
