...
function multiScreenMessage(){
    return {type:"chordio-multiscreen-state",song:song?{...song,sections:normalizeSections(song.sections)}:null,serviceName:service?.name||"",serviceIndex:index,serviceSongs:service?.songs||[],sectionIndex:multiScreenCurrentSection,modes:{...multiScreenModes},enabled:{...multiScreenEnabled},background:{...multiScreenBackground},songBackground:getMultiScreenSongBackground(song),layout:getPresentationLayout(),lyricsSettings:getMultiLyricsSettings(),lyricsOverrides:getMultiLyricsOverrides()};
}
...
function renderMultiScreenPreviews(){
    ...
    [1,2,3,4].forEach(n=>{
        ...
        if(multiScreenEnabled[n]===false) stage.innerHTML='<span class="multi-preview-empty">Screen disabled</span>';
            ...
            const bg=getMultiScreenActiveBackground(song);
...
function renderMultiScreenOutput(message){
    ...
    const bg=message?.songBackground||message?.background||{type:"none",url:""};
