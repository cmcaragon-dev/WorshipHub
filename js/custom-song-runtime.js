    ...
    stage.innerHTML = sections.map((section, si) => `
        <section class="section" data-section-index="${si}">
            ...
            ${section.lines.map((line, li) => {
                ...
                const fallback = Array.isArray(line.chords) ? line.chords.map(c => c.originalChord || c.chord || "").join(" ") : "";
                const chordLayout = layout || fallback;
                const displayedLayout = transposeChordLayout(chordLayout, transposeSteps);
                return `
                <div class="line" data-section-index="${si}" data-line-index="${li}">
                    <div class="chords" aria-label="Chords above lyrics"><span class="chord" data-original-layout="${esc(chordLayout)}">${esc(displayedLayout)}</span></div>
                    <div class="lyrics" style="font-size:calc(18px + ${Math.max(0,fontSize-1)}px)">${esc(line.lyrics || "")}</div>
                </div>`;
            }).join("")}
        </section>`).join("");

    // Every custom-song chord is a real .chord span and all chord tokens
    // are transposed from the saved original layout.
    stage.querySelectorAll(".chord").forEach(span => {
        const original = span.dataset.originalLayout || span.textContent || "";
        span.textContent = transposeChordLayout(original, transposeSteps);
    });
...
function bindControls(){
    ...
    document.getElementById("presentationBtn")?.addEventListener("click",openCustomPresentation);
    document.getElementById("editBtn")?.addEventListener("click",openCustomEditor);
    document.getElementById("pClose")?.addEventListener("click",closeCustomPresentation);
...
function openCustomPresentation(){
    if(!song)return;
    const screen=document.getElementById("customPresentation"); if(!screen)return;
    const title=document.getElementById("pSongTitle"), artist=document.getElementById("pSongArtist"), key=document.getElementById("pSongKey"), body=document.getElementById("pBody");
    if(title)title.textContent=song.title||"Untitled Song";
    if(artist)artist.textContent=song.artist||"";
    if(key)key.textContent=transposeChord(currentKey(),transposeSteps)||"—";
    const sections=normalizeSections(song.sections);
    if(body) body.innerHTML=sections.map(section=>`
      <section class="custom-presentation-section">
        <h3>${esc(section.type)} ${esc(section.number||"")}</h3>
        ${section.lines.map(line=>{
          const layout=String(line.chordLayout||"");
          return `<div class="custom-presentation-line"><div class="custom-presentation-chord">${esc(transposeChordLayout(layout,transposeSteps))}</div><div class="custom-presentation-lyrics">${esc(line.lyrics||"")}</div></div>`;
        }).join("")}
      </section>`).join("");
    screen.classList.add("show"); screen.setAttribute("aria-hidden","false");
    screen.requestFullscreen?.().catch(()=>{});
}
function closeCustomPresentation(){
    const screen=document.getElementById("customPresentation"); if(!screen)return;
...
function openCustomEditor(){
    ...
    sections.forEach((sec,si)=>{
      ...
      (sec.lines||[]).forEach((line,li)=>{
        ...
        const chord=document.createElement("textarea");chord.className="custom-chord";chord.value=String(line.chordLayout||"");chord.placeholder="CHORDS — e.g. D          F#m      Bm";
        ...
        row.append(chord,lyric);rows.appendChild(row);
      });
    });
