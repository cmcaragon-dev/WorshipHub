...
function wrapSongLine(line, maxChars) {
    ...
    if (!lyrics || lyrics.length <= maxChars) return [{...line, lyrics, chordText}];
    const rows = [];
    let start = 0;
    while (start < lyrics.length) {
        let end = Math.min(lyrics.length, start + maxChars);
        if (end < lyrics.length) {
            const part = lyrics.slice(start, end);
            const at = Math.max(part.lastIndexOf(" "), part.lastIndexOf("\t"));
            if (at > Math.floor(maxChars * 0.55)) end = start + at + 1;
        }
        if (end <= start) end = Math.min(lyrics.length, start + maxChars);
        rows.push({
            ...line,
...
async function loadFromService(){
    ...
    try {
        ...
        if(candidate.customSong===true || String(candidate.file||"").startsWith("custom-song.html")){
            const masterSnap=await getDoc(doc(db,"songs",String(candidate.id||"")));
            if(masterSnap.exists()){
                const master={id:masterSnap.id,...masterSnap.data()};
                // Firebase /songs is authoritative for amended song content. The service
                // keeps only service-specific values such as Service Key and transpose.
                song={
                    ...candidate,
                    ...master,
...
async function loadFromFirebaseSong(id){
    if(!id) return false;
    try {
        // Always prefer the shared Firebase master copy over bundled/local data.
        const snap = await getDoc(doc(db,"songs",String(id)));
        if(snap.exists()){
            const data=snap.data()||{};
            song={id:snap.id,...data,sections:normalizeSections(data.sections)};
            return true;
        }
...
async function load(){
    ...
    // A Service Planner launches an Add Song directly into presentation mode.
    // Do this only after the service copy has been loaded so Previous/Next and
    // the Next Song Preview have the correct service context.
...
onAuthStateChanged(auth,async()=>{
    ...
    if(activeServiceId && resume){
        ...
        if(loaded){
            ...
            // Service Planner -> Add Song: automatically open Presentation.
            if(document.getElementById("customPresentationScreen") && !document.getElementById("customPresentationScreen").classList.contains("show")){
                setTimeout(() => { void startCustomPresentation(); }, 0);
            }
            return;
        }
