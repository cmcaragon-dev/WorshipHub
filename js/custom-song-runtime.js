...
function multiScreenMessage(){
    return {type:"chordio-multiscreen-state",song:song?{...song,sections:normalizeSections(song.sections)}:null,serviceName:service?.name||"",serviceIndex:index,serviceSongs:service?.songs||[],sectionIndex:multiScreenCurrentSection,modes:{...multiScreenModes},enabled:{...multiScreenEnabled},background:{...multiScreenBackground},layout:getPresentationLayout(),lyricsSettings:getMultiLyricsSettings(),lyricsOverrides:getMultiLyricsOverrides()};
}
...
function renderMultiPartLyricsPreview(){const box=document.getElementById("multiPartLyricsPreview");if(!box)return;box.innerHTML="";const visible=multiScreenVisibleSections(),entry=visible[multiScreenCurrentSection];if(!song||!entry){box.innerHTML='<div class="multi-preview-empty">Select a song and song part.</div>';return;}const settings=getMultiLyricsSettings();const wrap=document.createElement("div");wrap.className="multi-part-lyrics-inner";wrap.style.textAlign=settings.align||"center";wrap.style.lineHeight=String(settings.spacing||1.15);wrap.style.fontSize=`${Number(settings.font||100)/100}em`;wrap.style.paddingTop=`${Math.max(0,Number(settings.vertical||50)-50)/2}%`;getMultiLyricsForSection(entry.section,entry.item.id).forEach(text=>{const ly=document.createElement("div");ly.className="multi-part-lyrics-line";ly.textContent=String(text||"");wrap.appendChild(ly);});box.appendChild(wrap);}
...
function renderMultiScreenPreviews(){const box=document.getElementById("multiScreenPreviews");if(!box)return;box.innerHTML="";const visible=multiScreenVisibleSections(),selected=visible[multiScreenCurrentSection]?.section,settings=getMultiLyricsSettings();[1,2,3,4].forEach(n=>{const card=document.createElement("div");card.className="multi-screen-preview-card"+(multiScreenEnabled[n]===false?" disabled":"");const head=document.createElement("div");head.className="multi-screen-preview-card-head";head.innerHTML=`<strong>SCREEN ${n}</strong><span>${multiScreenEnabled[n]===false?"HIDDEN":multiScreenModeLabel(multiScreenModes[n])}</span>`;card.appendChild(head);const stage=document.createElement("div");stage.className="multi-screen-preview-stage";if(multiScreenEnabled[n]===false)stage.innerHTML='<span class="multi-preview-empty">Screen disabled</span>';else if(!song||!selected)stage.innerHTML='<span class="multi-preview-empty">No song selected</span>';else if(multiScreenModes[n]==="blank")stage.innerHTML='<span class="multi-preview-empty">BLACK / BLANK</span>';else if(multiScreenModes[n]==="lyrics"){stage.classList.add("lyrics-preview");stage.style.textAlign=settings.align||"center";const l=document.createElement("div");l.className="preview-selected-lyrics";l.style.lineHeight=String(settings.spacing||1.15);l.style.fontSize=`${Math.max(.65,Number(settings.font||100)/100)*.72}em`;getMultiLyricsForSection(selected,visible[multiScreenCurrentSection]?.item?.id).slice(0,7).forEach(text=>{const x=document.createElement("div");x.textContent=String(text||"");l.appendChild(x);});stage.appendChild(l);}else{stage.classList.add("chords-preview");visible.slice(0,5).forEach((entry,i)=>{const sec=document.createElement("div");sec.className="preview-chords-section"+(i===multiScreenCurrentSection?" selected":"");const label=document.createElement("b");label.textContent=presentationSectionLabel(entry.section,i);sec.appendChild(label);(entry.section.lines||[]).slice(0,3).forEach(line=>{const row=document.createElement("div");row.className="preview-chord-row";const ch=document.createElement("span");ch.className="pc-chord";ch.textContent=chordRowFromPositions(line);const ly=document.createElement("span");ly.textContent=String(line.lyrics||"");row.append(ch,ly);sec.appendChild(row);});stage.appendChild(sec);});}card.appendChild(stage);box.appendChild(card);});}
...
function renderMultiBackgroundControls(){
    const type=document.getElementById("multiBackgroundType"),url=document.getElementById("multiBackgroundUrl");if(type)type.value=multiScreenBackground.type||"none";if(url)url.value=multiScreenBackground.url||"";
}
...
function renderMultiScreenOutput(message){
    ...
    const bg=message?.background||{type:"none",url:""};
    ...
    if(bgEl){bgEl.style.display=lyricsOnly?"block":"none";bgEl.style.backgroundImage=(lyricsOnly&&bg.type==="image"&&bg.url)?`url("${String(bg.url).replace(/"/g,'\"')}")`:"none";let video=bgEl.querySelector("video");if(lyricsOnly&&bg.type==="video"&&bg.url){if(!video){video=document.createElement("video");video.autoplay=true;video.muted=true;video.loop=true;video.playsInline=true;video.setAttribute("aria-hidden","true");bgEl.appendChild(video);}if(video.src!==bg.url)video.src=bg.url;video.style.cssText="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;";void video.play().catch(()=>{});}else if(video){video.remove();}}
    ...
    output.innerHTML="";
    if(mode==="lyrics"){
        const entry=visible[idx];if(!entry)return;const sec=entry.section;output.className="multi-output-lyrics-stage";output.style.textAlign=settings.align||"center";output.style.paddingTop=`${Math.max(0,Number(settings.vertical||50)-50)/2}%`;const el=document.createElement("section");el.className="multi-output-section current lyrics-only-section";((Array.isArray(outputLyricsOverrides[String(visible[idx]?.item?.id)])?outputLyricsOverrides[String(visible[idx]?.item?.id)]:((sec.lines||[]).map(line=>String(line?.lyrics||""))))).forEach(text=>{const ly=document.createElement("div");ly.className="multi-output-lyric-only-line";ly.style.lineHeight=String(settings.spacing||1.15);ly.style.fontSize=`${Number(settings.font||100)/100}em`;ly.textContent=String(text||"");el.appendChild(ly);});output.appendChild(el);document.getElementById("multiOutputSection").textContent="";
    }else{
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        ["multiLyricsSpacing","multiLyricsVertical","multiLyricsFont","multiLyricsAlign"].forEach(id=>document.getElementById(id)?.addEventListener("input",()=>{syncMultiLyricsSettingsControls();renderMultiPartLyricsPreview();renderMultiScreenPreviews();}));
        ...
        document.getElementById("multiBackgroundApply")?.addEventListener("click",()=>{const type=document.getElementById("multiBackgroundType")?.value||"none";const url=document.getElementById("multiBackgroundUrl")?.value.trim()||"";multiScreenBackground={type,url};try{localStorage.setItem("chordioMultiScreenBackground",JSON.stringify(multiScreenBackground));}catch(_){}multiScreenBroadcast({});});
        document.getElementById("multiBackgroundClear")?.addEventListener("click",()=>{multiScreenBackground={type:"none",url:""};try{localStorage.setItem("chordioMultiScreenBackground",JSON.stringify(multiScreenBackground));}catch(_){}renderMultiBackgroundControls();multiScreenBroadcast({});});
        document.getElementById("multiBackgroundFile")?.addEventListener("change",e=>{const file=e.target.files?.[0];if(!file)return;const type=file.type.startsWith("video/")?"video":"image";const reader=new FileReader();reader.onload=()=>{multiScreenBackground={type,url:String(reader.result||"")};renderMultiBackgroundControls();try{localStorage.setItem("chordioMultiScreenBackground",JSON.stringify(multiScreenBackground));}catch(_){}multiScreenBroadcast({});};reader.readAsDataURL(file);});
    }
}
