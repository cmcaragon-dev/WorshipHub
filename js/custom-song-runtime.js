...
function render() {
    ...
    /*
     * IMPORTANT:
     * Every Add Song line is rendered as one linked .song-line:
     *
     * <div class="song-line">
     *   <span class="chord">G                       Bm7</span><br>
     *   You are the peace that guards my heart
     * </div>
     *
     * The spaces inside .chord are intentional. They are generated from the
     * character position saved by the editor, so the chord remains above the
     * exact lyric position instead of being independently positioned with CSS.
     */
    stage.innerHTML = sections.map((section, si) => `
        <section class="section song-section" data-section-index="${si}">
            <div class="section-title">${esc(section.type)} ${esc(section.number || "")}</div>
            ${section.lines.map((line, li) => {
                const chordText = chordRowFromPositions(line);
                const chordMarkup = chordText
                    ? `<span class="chord" data-position="${line.chords?.[0]?.position ?? 0}">${esc(chordText)}</span><br>`
                    : "";
                return `<div class="line song-line" data-section-index="${si}" data-line-index="${li}">${chordMarkup}${esc(line.lyrics)}</div>`;
            }).join("")}
        </section>`).join("");

    // Re-apply the current transpose to the linked chord row without changing
    // any whitespace/character positions.
    stage.querySelectorAll(".song-line .chord").forEach((span, rowIndex) => {
        const sectionIndex = Number(span.closest(".song-line")?.dataset.sectionIndex || 0);
        const lineIndex = Number(span.closest(".song-line")?.dataset.lineIndex || 0);
        const line = sections[sectionIndex]?.lines?.[lineIndex];
        if (line) span.textContent = chordRowFromPositions(line);
    });
...
function buildCustomPrintRoot() {
        ...
        <div class="print-song-header">
            <div class="print-song-meta">
                ...
                <div class="print-song-info">
                    <span><b>Artist:</b> ${esc(song.artist || "")}</span>
                    <span><b>Original Key:</b> ${esc(song.originalKey || song.key || "")}</span>
                    <span><b>Service Key:</b> ${esc(transposeChord(currentKey(), transposeSteps) || "")}</span>
                </div>
            </div>
        </div>
    ...
    const content = root.querySelector(".print-song-content");
    const source = document.createElement("div");
    source.className = "wh-print-source-content song";
    const sections = normalizeSections(song.sections);

    source.innerHTML = sections.map(section => {
        const label = `${section.type || ""} ${section.number || ""}`.trim();
        const lines = (section.lines || []).map(line => {
            const chordText = chordRowFromPositions(line);
            const chordMarkup = chordText
                ? `<span class="chord">${esc(chordText)}</span><br>`
                : "";
            return `<div class="song-line">${chordMarkup}${esc(line.lyrics || "")}</div>`;
        }).join("");
        return `<section class="song-section"><div class="section-title">${esc(label)}</div>${lines}</section>`;
    }).join("");
...
window.printSong = function printSong() {
    const root = buildCustomPrintRoot();
    if (!root) return;
...
function chordRowFromPositions(line) {
    ...
    const chars=Array.from({length:Math.max(maxEnd,1)},()=>" ");
...
function renderCustomPresentation(){
    ...
    const k=document.getElementById("customPresentationKey"); if(k)k.textContent=`Key: ${transposeChord(currentKey(),transposeSteps)||"—"}`;
...
function renderCustomNextPreview(){
    ...
    if(next){const key=document.createElement("span");key.className="custom-next-key";key.textContent=`Service Key: ${next.serviceKey||next.key||next.originalKey||"—"}`;p.appendChild(key);p.onclick=()=>redirectSong(next);}else p.onclick=null;
...
function bindControls(){
    ...
    document.getElementById("printBtn")?.addEventListener("click",window.printSong);
    ...
    const prev=()=>{if(!service||index<=0)return;index--;localStorage.setItem("currentSongIndex",String(index));redirectSong(service.songs[index]);};
    const next=()=>{if(!service||index>=service.songs.length-1)return;index++;localStorage.setItem("currentSongIndex",String(index));redirectSong(service.songs[index]);};
