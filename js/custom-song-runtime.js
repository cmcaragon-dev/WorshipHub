...
function multiScreenOpenControl(){
    ...
    renderMultiServiceSongs();renderMultiScreenSectionButtons();renderMultiPartLyricsPreview();renderMultiStructureList();renderMultiBackgroundControls();syncMultiLyricsSettingsControls();syncMultiLyricsEditor();renderMultiSlides();renderMultiScreenPreviews();multiScreenBroadcast({});
}
...
function loadOfflineBibleFile(file){
    ...
    reader.onload=()=>{try{const parsed=JSON.parse(String(reader.result||"{}"));window.CHORDIO_OFFLINE_BIBLE=parsed;try{localStorage.setItem("chordioOfflineBible",JSON.stringify(parsed));}catch(_){}searchOfflineBible();const r=document.getElementById("multiBibleResults");if(r)r.insertAdjacentHTML("afterbegin",'<span class="multi-bible-loaded">Offline Bible loaded</span>');}catch(_){alert("The local Bible file is not valid JSON.");}};
...
function renderMultiPartLyricsPreview(){
    ...
    visible.forEach((entry,i)=>{
        const part=document.createElement("button");part.type="button";part.className="multi-part-preview-section"+(i===multiScreenCurrentSection?" active":"");part.dataset.sectionIndex=String(i);
...
function renderMultiScreenPreviews(){
    ...
    [1,2,3,4].forEach(n=>{
        ...
        const head=document.createElement("div");head.className="multi-screen-preview-card-head";head.innerHTML=`<strong>SCREEN ${n}</strong><span>${multiScreenEnabled[n]===false?"HIDDEN":multiScreenModeLabel(multiScreenModes[n])}</span>`;const settingsBtn=document.createElement("button");settingsBtn.type="button";settingsBtn.className="multi-preview-settings-button";settingsBtn.textContent="⚙ SETTINGS";settingsBtn.setAttribute("data-preview-settings",String(n));settingsBtn.onclick=()=>{const panel=document.getElementById(`multiScreenSettings${n}`);if(panel)panel.classList.toggle("open");};head.appendChild(settingsBtn);card.appendChild(head);
