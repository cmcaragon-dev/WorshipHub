...
const MULTI_LYRICS_DEFAULTS={spacing:1.15,vertical:50,fontPx:42,align:"center",fontFamily:"Arial",bold:false,italic:false,underline:false,color:"#FFFFFF",letterSpacing:0};
...
function syncMultiLyricsSettingsControls(){
    ...
    if(c)c.textContent=`${Math.round(Number(settings.fontPx||42))} px`;
}
...
function initMultiScreen(){
    ...
    if(fontValue) fontValue.textContent=`${Math.round(Number(settings.fontPx||42))} px`;
