...
function multiScreenMessage(){
    return {type:"chordio-multiscreen-state",song:song?{...song,sections:normalizeSections(song.sections)}:null,serviceName:service?.name||"",serviceIndex:index,serviceSongs:service?.songs||[],sectionIndex:multiScreenCurrentSection,modes:{...multiScreenModes},enabled:{...multiScreenEnabled},background:{...multiScreenBackground}};
}
...
function multiScreenOpenControl(){
    ...
    renderMultiServiceSongs();renderMultiLivePreview();renderMultiScreenSectionButtons();renderMultiStructureList();renderMultiBackgroundControls();multiScreenBroadcast({});
}
...
function renderMultiScreenControl(){
    ...
    renderMultiServiceSongs();renderMultiLivePreview();renderMultiScreenSectionButtons();renderMultiStructureList();renderMultiBackgroundControls();
}
function renderMultiLivePreview(){
    const box=document.getElementById("multiLivePreview");if(!box)return;
    const visible=multiScreenVisibleSections();const sec=visible[multiScreenCurrentSection]?.section;box.innerHTML="";
    if(!song||!sec){box.innerHTML='<div class="multi-preview-empty">Select a song and song part.</div>';return;}
    const title=document.createElement("div");title.className="multi-preview-title";title.textContent=song.title||"Untitled Song";box.appendChild(title);
    const meta=document.createElement("div");meta.className="multi-preview-meta";meta.textContent=`${song.artist||""}${song.artist?" • ":""}Service Key: ${song.serviceKey||song.key||song.originalKey||"—"}`;box.appendChild(meta);
    const section=document.createElement("div");section.className="multi-preview-section";const st=document.createElement("div");st.className="multi-preview-section-title";st.textContent=presentationSectionLabel(sec,multiScreenCurrentSection);section.appendChild(st);
    (sec.lines||[]).forEach(line=>{const row=document.createElement("div");row.className="multi-preview-line";const ch=document.createElement("div");ch.className="multi-preview-chord";ch.textContent=chordRowFromPositions(line);const ly=document.createElement("div");ly.className="multi-preview-lyric";ly.textContent=String(line.lyrics||"");if(ch.textContent)row.appendChild(ch);row.appendChild(ly);section.appendChild(row);});box.appendChild(section);
}
...
function multiScreenSelectSection(index,label=""){
    ...
    renderMultiScreenSectionButtons();renderMultiLivePreview();
...
function renderMultiScreenOutput(message){
    ...
    if(bgEl){
        bgEl.style.backgroundImage=(bg.type==="image"&&bg.url)?`url("${String(bg.url).replace(/"/g,'\\"')}")`:"none";
    ...
    const source=normalizeSections(data.sections||[]),layout=message?.layout||getPresentationLayout();const visible=layout.map(item=>({item,section:source[Number(item.id)]})).filter(x=>x.section&&x.item.visible!==false);const idx=Math.max(0,Math.min(Number(message?.sectionIndex??0),visible.length-1));
    output.innerHTML="";const entry=visible[idx];if(!entry)return;const sec=entry.section;const el=document.createElement("section");el.className="multi-output-section current";
    const title=document.createElement("div");title.className="multi-output-section-title";title.textContent=presentationSectionLabel(sec,idx);el.appendChild(title);
    (sec.lines||[]).forEach(line=>{const row=document.createElement("div");row.className="multi-output-line";if(mode==="chords"){const ch=document.createElement("div");ch.className="multi-output-chord";ch.textContent=chordRowFromPositions(line);if(ch.textContent)row.appendChild(ch);}const ly=document.createElement("div");ly.className="multi-output-lyric";ly.textContent=String(line.lyrics||"");row.appendChild(ly);el.appendChild(row);});output.appendChild(el);
