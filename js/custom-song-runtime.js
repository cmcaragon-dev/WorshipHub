...
function renderSongPagePassingChords(){
    const box=document.getElementById("songPagePassingItems");
    if(!box) return;
    const items=customPassingChords();
    box.innerHTML=items.map(([label,value])=>`<span class="song-page-passing-item"><span class="song-page-passing-label">${esc(label)}</span><span class="song-page-passing-value">${esc(value)}</span></span>`).join("");
}

function render() {
    if (!song) return;
    renderSongPagePassingChords();
...
async function loadFromService(){
    ...
    try {
        ...
        // Every occurrence in a Service Planner receives a stable instance identity.
        // This isolates structure, formatting, lyrics overrides and backgrounds
        // when the same master song is added more than once.
        const serviceInstancesChanged=ensureServiceSongInstanceIds(service);
        if(serviceInstancesChanged){
            try{localStorage.setItem("currentServiceSnapshot",JSON.stringify(service));}catch(_){}
            if(auth.currentUser){
                try{await setDoc(doc(db,"users",auth.currentUser.uid,"services",String(serviceId)),{songs:service.songs,updatedAt:serverTimestamp()},{merge:true});}
                catch(error){console.warn("Unable to migrate Service Planner song instances:",error);}
            }
        }
...
function presentationLayoutStorageKey(){
    ...
    return PRESENTATION_LAYOUT_PREFIX + String(serviceId) + ":" + getServiceSongInstanceId(song,index);
}
...
function getServiceSongInstanceId(data=song, indexHint=index){
    if(data?.instanceId) return String(data.instanceId);
    const serviceId=localStorage.getItem("currentServiceId")||service?.id||"standalone";
    return `legacy-${serviceId}-${Number.isInteger(Number(indexHint))?Number(indexHint):0}-${String(data?.id||data?.file||data?.title||"song")}`;
}

function ensureServiceSongInstanceIds(targetService){
    if(!targetService || !Array.isArray(targetService.songs)) return false;
    const used=new Set(); let changed=false;
    targetService.songs=targetService.songs.map((item,i)=>{
        const copy={...item};
        let id=String(copy.instanceId||"").trim();
        if(!id || used.has(id)){
            id=`svc-${Date.now()}-${i}-${Math.random().toString(36).slice(2,10)}`;
            changed=true;
        }
        copy.instanceId=id; used.add(id); return copy;
    });
    return changed;
}
...
function getPresentationLayout(){
    ...
    const serviceId=localStorage.getItem("currentServiceId");
    const serviceSaved=serviceId && song?.id ? (serviceLayouts[getServiceSongInstanceId(song,index)] || ((service?.songs||[]).filter(x=>String(x?.id||"")===String(song.id)).length===1 ? serviceLayouts[String(song.id)] : null)) : null;
    try{
...
async function savePresentationLayout(layout){
    ...
    const layoutKey=getServiceSongInstanceId(song,index); const presentationLayouts={...(service.presentationLayouts||{}),[layoutKey]:layout};
...
async function selectMultiScreenQueueEntry(qIndex){
    ...
    if(q.type==="presentation"){
        ...
        if(source){
            ...
            song={...source,sections:normalizeSections(source.sections),serviceKey:source.serviceKey||source.key,transpose:Number(source.transpose||0)};
            transposeSteps=Number(song.transpose||0);
        }else{
            song={...q.item,sections:normalizeSections(q.item?.sections||[]),serviceKey:q.item?.serviceKey||q.item?.key,transpose:Number(q.item?.transpose||0)};
            index=0;
            transposeSteps=Number(song.transpose||0);
        }
...
function multiScreenSongBackgroundKey(data=song){return getServiceSongInstanceId(data,index);}
...
function multiScreenOpenControl(){
    ...
    const existingSongQueueIndex=multiScreenQueueIndexForSong(song?.id,getServiceSongInstanceId(song,index));if(existingSongQueueIndex>=0)multiScreenQueueIndex=existingSongQueueIndex;
    initMultiScreenPartKeyboard();
    panel.classList.add("show");panel.setAttribute("aria-hidden","false");
    // Force the control surface to occupy the entire viewport even when a
    // parent page/container previously constrained its size.
    panel.style.position="fixed"; panel.style.inset="0"; panel.style.width="100vw"; panel.style.height="100dvh"; panel.style.maxWidth="none"; panel.style.maxHeight="none";
...
function multiScreenQueue(){
    ...
    raw.forEach(ref=>{
        ...
        if(type==="presentation"){
            ...
            if(sourceIndex>=0&&sourceIndex<songs.length&&songs[sourceIndex]){
                const key=`song-instance:${String(songs[sourceIndex]?.instanceId||sourceIndex)}`;
                if(seenQueue.has(key))return;
                seenQueue.add(key);queue.push({type:"song",id:getServiceSongInstanceId(songs[sourceIndex],sourceIndex),songId:String(songs[sourceIndex]?.id||""),item:songs[sourceIndex],sourceIndex});
            }else{
                const id=String(ref?.songId||ref?.id||""); const instanceId=String(ref?.instanceId||""); const sourceIndex2=songs.findIndex(x=>instanceId ? String(x?.instanceId||"")===instanceId : String(x?.id||"")===id);
                if(sourceIndex2>=0){
                    const key=`song-instance:${String(songs[sourceIndex2]?.instanceId||sourceIndex2)}`;
                    if(seenQueue.has(key))return;
                    seenQueue.add(key);queue.push({type:"song",id:getServiceSongInstanceId(songs[sourceIndex2],sourceIndex2),songId:String(songs[sourceIndex2]?.id||""),item:songs[sourceIndex2],sourceIndex:sourceIndex2});
                }
            }
        }
    });
    ...
    songs.forEach((x,i)=>{
        const key=`song-instance:${String(x?.instanceId||i)}`;
        if(seenQueue.has(key))return;
        seenQueue.add(key);
        queue.push({type:"song",id:getServiceSongInstanceId(x,i),songId:String(x?.id||""),item:x,sourceIndex:i});
    });
...
function multiScreenQueueIndexForSong(songId, instanceId=getServiceSongInstanceId(song,index)){
    const iid=String(instanceId||""); const id=String(songId||"");
    return multiScreenQueue().findIndex(x=>x.type==="song" && (String(x.id)===iid || (id && String(x.songId||x.item?.id||"")===id && String(x.item?.instanceId||"")===iid)));
}
async function saveMultiScreenQueue(queue){
    if(!service)return false;
    const normalized=queue.map(x=>x.type==="presentation"?({type:"presentation",id:String(x.id||"")}):({type:"song",id:String(x.id||""),songId:String(x.songId||x.item?.id||""),instanceId:String(x.item?.instanceId||x.id||""),songIndex:Number.isInteger(Number(x.sourceIndex))?Number(x.sourceIndex):-1})).filter(x=>x.id);
...
function currentMultiQueueEntry(){
    const queue=multiScreenQueue();
    const qIndex=Number.isInteger(multiScreenQueueIndex)?multiScreenQueueIndex:multiScreenQueueIndexForSong(song?.id,getServiceSongInstanceId(song,index));
...
async function addSongToCurrentMultiService(source){
    ...
    const serviceSong={
        instanceId:`svc-${Date.now()}-${Math.random().toString(36).slice(2,10)}`,
...
function renderMultiServiceSongs(){
    ...
    queue.forEach((entry,i)=>{
        ...
        b.dataset.queueIndex=String(i); b.dataset.sourceIndex=String(entry.sourceIndex);
        ...
        b.querySelector(".multi-service-song-delete")?.addEventListener("click",async e=>{
            ...
            const nextSongs=[...(service.songs||[])];
            const songIndex=nextSongs.findIndex(x=>String(x?.instanceId||"")===String(item?.instanceId||""));
            const fallbackIndex=songIndex<0 ? Number(entry?.sourceIndex) : songIndex;
            const removeIndex=songIndex>=0?songIndex:(Number.isInteger(fallbackIndex)?fallbackIndex:-1);
            if(removeIndex<0 || removeIndex>=nextSongs.length)return;nextSongs.splice(removeIndex,1);
            const nextQueue=multiScreenQueue().filter(x=>!(x.type==="song"&&Number(x.sourceIndex)===removeIndex)).map(x=>x.type==="song"&&Number(x.sourceIndex)>removeIndex?{...x,sourceIndex:Number(x.sourceIndex)-1}:x);
            service={...service,songs:nextSongs};
            await saveServiceSongs(nextSongs);await saveMultiScreenQueue(nextQueue);
            if(!nextSongs.length){index=0;song=null;}else{index=Math.min(removeIndex,nextSongs.length-1);const current=nextSongs[index];song={...current,sections:normalizeSections(current.sections),serviceKey:current.serviceKey||current.key,transpose:Number(current.transpose||0)};transposeSteps=Number(song.transpose||0);}
...
function multiScreenOpenDisplay(displayNumber){
    ...
    const url=new URL(location.href);url.pathname=url.pathname.replace(/[^/]+$/, "multi-screen-output.html");url.search="";url.searchParams.set("display",String(n));url.searchParams.set("id",String(song.id||""));url.searchParams.set("instanceId",getServiceSongInstanceId(song,index));url.searchParams.set("mode",mode);url.searchParams.set("ts",String(Date.now()));
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        const expectedSongId=String(params.get("id")||""); const expectedInstanceId=String(params.get("instanceId")||"");
        ...
        const acceptState=state=>{
            ...
            const incomingId=String(normalized?.song?.id||""); const incomingInstanceId=String(normalized?.song?.instanceId||"");
            if(!outputHandshakeComplete&&expectedInstanceId&&incomingInstanceId&&incomingInstanceId!==expectedInstanceId)return;
            if(!outputHandshakeComplete&&!expectedInstanceId&&expectedSongId&&incomingId&&incomingId!==expectedSongId)return;
            if((expectedInstanceId&&incomingInstanceId===expectedInstanceId)||(!expectedInstanceId&&expectedSongId&&incomingId===expectedSongId))outputHandshakeComplete=true;
        ...
        try{const scoped=JSON.parse(localStorage.getItem(`chordioMultiScreenOutputState:${multiScreenDisplayId}`)||"null");if(scoped&&(!expectedInstanceId||String(scoped?.song?.instanceId||"")===expectedInstanceId)&&(!expectedSongId||String(scoped?.song?.id||"")===expectedSongId))acceptState(scoped);if(!multiScreenLastOutputState){const cached=JSON.parse(localStorage.getItem("chordioMultiScreenState")||"null");if(cached&&(!expectedInstanceId||String(cached?.song?.instanceId||"")===expectedInstanceId)&&(!expectedSongId||String(cached?.song?.id||"")===expectedSongId))acceptState(cached);}}catch(_){}
