...
function renderMultiServiceSongs(){
    ...
    songs.forEach((item,i)=>{
        const b=document.createElement("div");b.draggable=true;b.className="multi-service-song"+(i===index?" active":"");b.dataset.songIndex=String(i);
        b.innerHTML=`<span class="song-drag" title="Drag to reorder">☷</span><span class="song-num">${i+1}</span><span class="song-name">${esc(item.title||"Untitled Song")}</span><span class="song-meta">${esc(item.artist||"")} • Key: ${esc(item.serviceKey||item.key||item.originalKey||"—")}</span>${item.presentationNote?`<span class="song-note">📝 ${esc(item.presentationNote)}</span>`:""}<button type="button" class="multi-service-song-delete" title="Remove this song from the Service Planner" aria-label="Remove ${esc(item.title||"song")} from Service Planner">✕</button>`;
        b.addEventListener("click",async e=>{if(e.target.closest(".multi-service-song-delete"))return;await loadServiceIndex(i);multiScreenCurrentSection=0;multiActivePageSlide=null;multiSelectedPageSlideIndex=-1;renderMultiScreenControl();multiScreenBroadcast({sectionIndex:0,activePageSlide:null});});
        b.querySelector(".multi-service-song-delete")?.addEventListener("click",async e=>{e.preventDefault();e.stopPropagation();const title=item.title||"this song";if(!confirm(`Remove "${title}" from this Service Planner?\n\nThis removes it only from the selected Service Planner. The song remains in the main song library.`))return;const next=[...songs];next.splice(i,1);const currentId=String(song?.id||"");const ok=await saveServiceSongs(next);if(!ok){alert("Unable to remove the song from this Service Planner.");return;}if(!next.length){index=0;song=null;}else{index=Math.max(0,next.findIndex(x=>String(x?.id||"")===currentId));if(index<0)index=Math.min(i,next.length-1);const current=next[index];if(current){song={...current,sections:normalizeSections(current.sections),serviceKey:current.serviceKey||current.key,transpose:Number(current.transpose||0)};transposeSteps=Number(song.transpose||0);}}renderMultiScreenControl();renderMultiAddSongList();multiScreenBroadcast({sectionIndex:0,activePageSlide:null});setMultiSlideStatus(`"${title}" removed from Service`);});
