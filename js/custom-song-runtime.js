...
function openBibleReference(){const version=document.getElementById("multiBibleVersion")?.value||"MBBTAG";const ref=String(document.getElementById("multiBibleReference")?.value||"").trim();if(!ref)return alert("Enter a Bible reference first.");const url=`https://www.biblegateway.com/passage/?search=${encodeURIComponent(ref)}&version=${encodeURIComponent(version)}`;window.open(url,"CHORDIO_BIBLE","noopener,noreferrer,width=1100,height=800");}
async function addBibleReferenceSlide(){const version=document.getElementById("multiBibleVersion")?.value||"MBBTAG";const ref=String(document.getElementById("multiBibleReference")?.value||"").trim();const text=String(document.getElementById("multiBibleText")?.value||"").trim();if(!ref)return alert("Enter a Bible reference first.");const label=version==="NIV"?"NIV":"MBB";const slide={id:`scripture-${Date.now()}`,title:`${ref} — ${label}`,text:text||ref,scriptureReference:ref,scriptureVersion:version};const slides=[...currentServiceSongSlides(),slide];await saveServiceSongs(service.songs.map((x,i)=>i===index?{...x,pageSlides:slides}:x));multiSelectedPageSlideIndex=slides.length-1;renderMultiSlides();document.getElementById("multiSlideTitle").value=slide.title;document.getElementById("multiSlideText").value=slide.text;setMultiSlideStatus("Scripture reference added as slide");}
...
function renderMultiPartLyricsPreview(){const box=document.getElementById("multiPartLyricsPreview");if(!box)return;box.innerHTML="";const visible=multiScreenVisibleSections(),entry=visible[multiScreenCurrentSection];if(!song||!entry){box.innerHTML='<div class="multi-preview-empty">Select a song and song part.</div>';return;}const settings=getMultiLyricsSettings();const wrap=document.createElement("div");wrap.className="multi-part-lyrics-inner";wrap.style.textAlign=settings.align||"center";wrap.style.lineHeight=String(Math.max(.7,Math.min(2.4,Number(settings.spacing)||1.15)));wrap.style.fontSize=`${Math.max(10,Math.min(140,Number(settings.fontPx||42)))}px`;wrap.style.fontFamily=settings.fontFamily||"Arial";wrap.style.fontWeight=settings.bold?"800":"400";wrap.style.fontStyle=settings.italic?"italic":"normal";wrap.style.textDecoration=settings.underline?"underline":"none";wrap.style.color=settings.color||"#FFFFFF";wrap.style.letterSpacing=`${Number(settings.letterSpacing||0)}px`;const v=Math.max(0,Math.min(100,Number(settings.vertical)||50));wrap.style.paddingTop=`${Math.max(0,Math.min(65,v*0.65))}vh`;getMultiLyricsForSection(entry.section,entry.item.id).forEach(text=>{const ly=document.createElement("div");ly.className="multi-part-lyrics-line";ly.textContent=String(text||"");wrap.appendChild(ly);});box.appendChild(wrap);}
...
function initMultiScreen(){
    ...
    if(multiScreenIsOutput){
        ...
        document.getElementById("multiBibleOpen")?.addEventListener("click",openBibleReference);
