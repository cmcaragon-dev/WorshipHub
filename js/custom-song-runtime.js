...
function parseBibleGatewayReader(text,reference){
    ...
    if(!m)return "";
    const chapter=m[2],verse=m[3],verseStart=verse.split("-")[0],verseEnd=Number(verse.split("-")[1]||verseStart);
    const marker=new RegExp(String.raw`\b${chapter}\s*:\s*${verse.replace("-",String.raw`\s*-\s*`)}\b`,'i');
    const match=raw.match(marker);
    if(!match)return "";
    let body=raw.slice((match.index||0)+match[0].length);

    // Keep ONLY the requested verse/range. BibleGateway/Jina may return the rest of the chapter
    // after the requested verse, so stop at the next verse-number marker.
    const nextVerse=verseEnd+1;
    const nextVersePattern=new RegExp(String.raw`(?:^|\n)\s*${nextVerse}\s+(?=\S)`,"i");
    const nextVerseMatch=body.match(nextVersePattern);
    if(nextVerseMatch) body=body.slice(0,nextVerseMatch.index);
    ...
    body=body.replace(new RegExp(`^${chapter}\s*:\s*${verseStart}\s*`,'i'),"");
    body=body.replace(/^\s*\d+\s+/g,"").replace(/\s+/g," ").trim();
    return body;
}
function setBibleResultText(results,text){
    if(!results)return;
    results.innerHTML="";
    const verse=document.createElement("div");
    verse.className="multi-bible-result";
    verse.textContent=String(text||"").trim();
    results.appendChild(verse);
}
