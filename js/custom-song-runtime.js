...
function renderMultiScreenPreviews(){
    ...
    [1,2,3,4].forEach(n=>{
        ...
        if(multiScreenEnabled[n]===false) stage.innerHTML='<span class="multi-preview-empty">Screen disabled</span>';
            ...
            // Background is part of the actual output composition, so show it in every preview too.
            const bg=multiScreenBackground||{type:"none",url:""};
            ...
            if(multiScreenModes[n]==="lyrics"){
                ...
                const title=document.createElement("div");title.className="multi-preview-title";title.textContent=song.title||"CHORDIO";stage.appendChild(title);
                const passing=document.createElement("div");passing.className="multi-preview-passing";passing.textContent=customPassingChordsForSong(song).map(x=>`${x[0]}: ${x[1]}`).join("   |   ");stage.appendChild(passing);
...
function renderMultiScreenOutput(message){
    ...
    if(bgEl){bgEl.style.display=(bg.type!=="none"&&bg.url)?"block":"block";bgEl.style.backgroundImage=(bg.type==="image"&&bg.url)?`url("${String(bg.url).replace(/"/g,'\"')}")`:"none";let video=bgEl.querySelector("video");if(bg.type==="video"&&bg.url){if(!video){video=document.createElement("video");video.autoplay=true;video.muted=true;video.loop=true;video.playsInline=true;video.setAttribute("aria-hidden","true");bgEl.appendChild(video);}if(video.src!==bg.url)video.src=bg.url;video.style.cssText="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;";void video.play().catch(()=>{});}else if(video){video.remove();}}
    ...
    output.innerHTML="";
    if(mode==="lyrics"){
        const entry=visible[idx];if(!entry)return;const sec=entry.section;output.className="multi-output-lyrics-stage";output.style.textAlign=settings.align||"center";output.style.paddingTop=`${Math.max(0,Number(settings.vertical||50)-50)/2}%`;const baseScale=Math.max(.55,Number(settings.font||100)/100);const viewportScale=Math.max(.72,Math.min(1.35,Math.min(window.innerWidth/1280,window.innerHeight/720)));output.style.setProperty("--lyrics-scale",String(baseScale*viewportScale));const el=document.createElement("section");el.className="multi-output-section current lyrics-only-section";((Array.isArray(outputLyricsOverrides[String(visible[idx]?.item?.id)])?outputLyricsOverrides[String(visible[idx]?.item?.id)]:((sec.lines||[]).map(line=>String(line?.lyrics||""))))).forEach(text=>{const ly=document.createElement("div");ly.className="multi-output-lyric-only-line";ly.style.lineHeight=String(settings.spacing||1.15);ly.style.fontSize=`clamp(24px, 5vw, 78px)`;ly.style.setProperty("--lyrics-scale",String(Math.max(.55,Number(settings.font||100)/100)));ly.textContent=String(text||"");el.appendChild(ly);});output.appendChild(el);document.getElementById("multiOutputSection").textContent="";
    }else{
