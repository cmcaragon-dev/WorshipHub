        ...
        <section class="section" data-section-index="${si}">
            ...
            ${section.lines.map((line, li) => `
                <div class="line song-line" data-section-index="${si}" data-line-index="${li}">
                    <span class="chord" data-chord-text="true">${esc(transposeChord(line.chordText || chordRowFromPositions(line), transposeSteps))}</span><br>
                    <span class="lyrics" style="font-size:calc(22px + ${fontSize}px)">${esc(line.lyrics)}</span>
                </div>`).join("")}
        </section>`).join("");
