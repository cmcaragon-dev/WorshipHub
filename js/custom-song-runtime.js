...
function getOnlineBibleVersion(){return "NIV";}
...
async function fetchOnlineBibleVerse(){const reference=getOnlineBibleReference(),version="NIV",results=document.getElementById("multiBibleResults"),textBox=document.getElementById("multiBibleText");if(!reference){if(results)results.textContent="Enter Book, Chapter and Verse.";return null;}if(results)results.textContent="Searching…";try{const data=await fetchBibleGateway(reference,version);return populateOnlineBibleResult(reference,{...data,version});}catch(error){console.warn("Online Bible lookup failed:",error);if(results)results.textContent="Unable to retrieve the verse. Check the reference and internet connection.";if(textBox)textBox.value="";return null;}}
async function addBibleReferenceSlide(){const reference=getOnlineBibleReference(),version="NIV",label="NIV";if(!reference){alert("Enter Book, Chapter and Verse first. Example: John → 3 → 16 or John → 3 → 1-5.");return;}let text=String(document.getElementById("multiBibleText")?.value||"").trim();if(!text)text=await fetchOnlineBibleVerse();if(!text)return;if(!service?.songs?.[index]){alert("Open a Service Planner song first.");return;}const slide={id:`scripture-${Date.now()}`,title:reference,text,scriptureReference:reference,scriptureVersion:version},slides=[...currentServiceSongSlides(),slide];await saveServiceSongs(service.songs.map((x,i)=>i===index?{...x,pageSlides:slides}:x));multiSelectedPageSlideIndex=slides.length-1;renderMultiSlides();const title=document.getElementById("multiSlideTitle"),slideText=document.getElementById("multiSlideText");if(title)title.value=slide.title;if(slideText)slideText.value=slide.text;setMultiSlideStatus(`${label} scripture added as slide`);}
...
function renderMultiScreenPreviews(){
    ...
    [1,2,3,4].forEach(n=>{const b=document.createElement("button");b.type="button";b.textContent=String(n);b.className=slots.includes(n)?"active":"";b.title=`Preview Screen ${n}`;b.onclick=()=>{
        if(slots.includes(n)){if(slots.length<=1)return; multiScreenPreviewSlots=slots.filter(x=>x!==n);} else {multiScreenPreviewSlots=[...slots,n].slice(0,count);}
        multiScreenPreviewSlots=multiScreenPreviewSlots.slice(0,count); while(multiScreenPreviewSlots.length<count){const next=[1,2,3,4].find(x=>!multiScreenPreviewSlots.includes(x));if(next==null)break;multiScreenPreviewSlots.push(next);}
        try{localStorage.setItem("chordioMultiScreenPreviewSlots",JSON.stringify(multiScreenPreviewSlots));}catch(_){} multiScreenPreviewExpanded=0;renderMultiScreenPreviews();}; slotWrap.appendChild(b);});
    const hint=document.createElement("span");hint.textContent="Choose which physical screen numbers to preview.";
