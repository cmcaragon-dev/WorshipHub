...
function renderMultiScreenOutput(message){
    ...
    if(mode==="lyrics"){
        ...
        if(entry){const sec=entry.section;const el=document.createElement("section");el.className="multi-output-section current-part";el.dataset.sectionIndex=String(idx);const title=document.createElement("div");title.className="multi-output-section-title";title.textContent=presentationSectionLabel(sec,idx);el.appendChild(title);(sec.lines||[]).forEach(line=>{const row=document.createElement("div");row.className="multi-output-line";row.style.fontSize=`${Math.max(16,Math.min(140,58*(Number(settings.font||100)/100)))}px`;row.style.lineHeight=String(settings.spacing||1.15);const ch=document.createElement("div");ch.className="multi-output-chord";ch.textContent=chordRowFromPositions(line);if(ch.textContent)row.appendChild(ch);const ly=document.createElement("div");ly.className="multi-output-lyric";ly.textContent=String(line.lyrics||"");row.appendChild(ly);el.appendChild(row);});output.appendChild(el);}
        document.getElementById("multiOutputSection").textContent=entry?presentationSectionLabel(entry.section,idx):"";
    }
}
