...
function loadCustomSongs() {
    try {
        ...
        console.warn("Unable to load CHORDIO custom songs:", error);
    }
}
    ...
    try {
        ...
        for (let i = 0; i < refs.length; i += 400) {
            ...
            refs.slice(i, i + 400).forEach(ref => {
                const data = serviceSnap.docs.find(d => d.ref.path === ref.path)?.data() || {};
...
// Song titles are unique in CHORDIO. Comparison is case-insensitive and
...
function buildSectionsFromImportedText(text) {
    ...
    for(let i=0;i<rawLines.length;i++){
        ...
        // representation for CHORDIO's chord-position editor and presentation.
...
async function importChordPage() {
    ...
    try {
        ...
        alert("Unable to import this page automatically. The site may block browser access or use a layout CHORDIO cannot read. You can still paste the chord/lyric lines into the editor.");
    } finally {
...
async function saveSong() {
    ...
    // Notify the main CHORDIO app immediately. This keeps All Songs and
...
// CHORDIO V63 — drag/drop section reorder bridge
window.WorshipHubSongEditor.reorderSections = function(from, to){
    from=Number(from); to=Number(to);
    if(!Number.isInteger(from)||!Number.isInteger(to)||from<0||to<0||from>=sections.length||to>=sections.length||from===to)return;
    const [moved]=sections.splice(from,1); sections.splice(to,0,moved); renderEditor();
};
