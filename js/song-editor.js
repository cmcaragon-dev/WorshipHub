...
function guessSongType(category, text="") {
    const c = `${category} ${text}`.toLowerCase();
...
function parseImportedPage(html,url) {
    ...
    // Read metadata BEFORE removing scripts, because JSON-LD commonly contains title,
    ...
    const meta=(sel,attr="content")=>cleanImportedText(doc.querySelector(sel)?.getAttribute(attr)||"");
    const jsonAuthor=typeof jsonLd.author==="string"?jsonLd.author:(jsonLd.author?.name||"");
    const title=cleanImportedText(meta("meta[property='og:title']")||meta("meta[name='twitter:title']")||jsonLd.name||doc.querySelector("h1")?.textContent||doc.title||"");
    const author=cleanImportedText(meta("meta[name='author']")||jsonAuthor||doc.querySelector("[class*=artist i], [class*=author i], [itemprop='byArtist']")?.textContent||"");
    const fullText=cleanImportedText(doc.body?.innerText||"");
    const key=guessKey(fullText);
    const categoryMatch=fullText.match(/(?:category|genre|song\s*category)\s*[:\-]\s*([^\n]+)/i);
    const category=cleanImportedText(categoryMatch?.[1]||jsonLd.genre||(/\bhymn\b/i.test(fullText)?"Hymn":/\bpraise\b/i.test(fullText)?"Praise":"Worship")).split("\n")[0].trim();
    const main=doc.querySelector("main,article,[role='main'],.song,.song-content,.lyrics,.chords,.tab,.tabs,.content")||doc.body;
    // Strip navigation/ads from a cloned content area, while keeping chord/lyric text.
    const clone=main.cloneNode(true);
    clone.querySelectorAll("script,style,noscript,svg,nav,footer,header,form,button,[class*=comment],[class*=related],[class*=advert],[class*=social]").forEach(el=>el.remove());
    const lines=extractChordLines(clone);
    const sourceTitle=title.replace(/\s*[-|–—]\s*(chords?|lyrics?|tabs?|guitar.*)$/i,"").trim();
    return {
        title:sourceTitle||title,
        artist:author,
        category:/hymn/i.test(category)?"Hymn":/praise/i.test(category)?"Praise":/other/i.test(category)?"Other":"Worship",
        songType:guessSongType(category,fullText),
        key:key||"C",
        language:cleanImportedText(jsonLd.inLanguage||guessLanguage(fullText,doc))||"English",
        sourceUrl:url,
...
// Sidebar shortcut: open the Add Song editor and focus the chord-page URL field.
document.addEventListener("DOMContentLoaded", () => {
    const importBtn=document.getElementById("importSongBtn");
    importBtn?.addEventListener("click", () => {
        document.getElementById("addSongBtn")?.click();
        setTimeout(() => document.getElementById("editorSourceUrl")?.focus(), 100);
    });
    const source=document.getElementById("editorSourceUrl");
    source?.addEventListener("change", () => { if(source.value.trim()) importChordPage(); });
