...
function buildLinkedLyricsHtml(sourceSections) {
    const sectionsToUse = Array.isArray(sourceSections) ? sourceSections : [];
    return sectionsToUse.map(section => {
        ...
        const lines = (section.lines || []).map(line => {
            ...
            const maxEnd = Math.max(
                lyrics.length,
                ...chords.map(c => Math.max(
                    0,
                    Number(c.position) || 0
                ) + String(c.originalChord || c.chord || "").trim().length)
            );
            ...
            chords.slice().sort((a,b) => (Number(a.position)||0) - (Number(b.position)||0)).forEach(c => {
                ...
                Array.from(text).forEach((ch, i) => {
                    const at = position + i;
                    if (at >= chars.length) chars.push(ch);
                    else chars[at] = chars[at] === " " ? ch : chars[at] + ch;
                });
            });
            const chordRow = chars.join("");
            const chordPart = chords.length
                ? `<span class="chord">${esc(chordRow)}</span><br>`
                : "";
            return `<div class="song-line">${chordPart}${esc(lyrics)}</div>`;
        }).join("");
...
function renderEditor() {
    ...
    root.querySelectorAll(".editor-chord-strip").forEach(strip => {
        strip.addEventListener("click", event => addChordAtClick(event));
    });
    root.querySelectorAll(".editor-chord-chip").forEach(chip => {
        chip.addEventListener("click", event => {
            event.stopPropagation();
            const [si, li] = getLineIndexes(chip);
            const chordIndex = Number(chip.dataset.chordIndex);
            const chord = sections[si].lines[li].chords[chordIndex];
            const updated = prompt("Chord:", chord.chord);
            if (updated === null) return;
            const value = updated.trim();
            if (!value) {
                sections[si].lines[li].chords.splice(chordIndex, 1);
            } else {
                chord.chord = value;
            }
            refreshChordStrip(si, li);
...
function renderLine(line, sectionIndex, lineIndex) {
    ...
    const chordHTML = (line.chords || []).map((item, chordIndex) => {
    ...
    return `
        <div class="editor-line" data-section-index="${sectionIndex}" data-line-index="${lineIndex}">
            <div class="editor-line-main">
                <div class="editor-chord-strip" data-section-index="${sectionIndex}" data-line-index="${lineIndex}" style="min-width:${Math.max(520, width * 9.6 + 30)}px">${chordHTML}</div>
                <textarea class="editor-lyric-input" data-section-index="${sectionIndex}" data-line-index="${lineIndex}" placeholder="Type lyrics here...">${esc(line.lyrics)}</textarea>
            </div>
...
async function saveSong() {
    ...
    sections = sections.map(section => ({
        ...
        lines: (section.lines || []).map(line => ({
            ...
            chords: (line.chords || []).map((chord, chordIndex) => ({
                ...
                chord: String(chord.originalChord || chord.chord || "").trim(),
                originalChord: String(chord.originalChord || chord.chord || "").trim(),
    ...
    const song = {
        ...
        // Linked HTML is stored as a print-safe representation. The structured
        // sections remain authoritative for editing/transposition.
        lyricsHtml: buildLinkedLyricsHtml(sections),
        contentVersion: 2,
    ...
    document.getElementById("editorPreviewBody").innerHTML = sections.map(section => `
        <div class="editor-preview-section">
            ...
            ${section.lines.map(line => {
                const chordHTML = (line.chords || []).map(c => `<span style="position:absolute;left:${c.position * 15}px">${esc(c.chord)}</span>`).join("");
                return `<div class="editor-preview-line"><div class="editor-preview-chords">${chordHTML}</div><div class="editor-preview-lyrics">${esc(line.lyrics)}</div></div>`;
            }).join("")}
        </div>
    `).join("");
