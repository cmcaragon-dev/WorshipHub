...
async function findDuplicateSongTitle(title, currentId = null) {
    ...
    // First check the in-memory library, which contains the built-in songs and
    // any custom songs already loaded from Firebase/local storage.
    const localMatch = songs.find(song => {
        if (!song) return false;
        if (currentId && String(song.id) === String(currentId)) return false;
        // When editing an existing song, a bundled/base copy with the same
        // title is the same logical song, not a duplicate that should block
        // the save. Distinct custom songs with the same title are still blocked.
        if (currentId && song.customSong !== true) return false;
        return normalizeSongTitle(song.title) === normalizedTitle;
    });
    if (localMatch) return localMatch;

    // Also check Firebase directly in case another custom song was created in
    // another browser/session and has not reached this page's local library yet.
    try {
        const snap = await getDocs(collection(db, "songs"));
        for (const docSnap of snap.docs) {
            const song = docSnap.data() || {};
            ...
            if (currentId && song.customSong !== true) continue;
            if (normalizeSongTitle(song.title) === normalizedTitle) return { ...song, id: songId };
        }
    } catch (error) {
        // Do not block a valid save merely because Firebase rules prevent a
        // duplicate-check read. The local library check above still applies.
