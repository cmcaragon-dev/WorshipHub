...
import { collection, doc, setDoc, deleteDoc, serverTimestamp, getDoc } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
async function canAddSongs() {
    try {
        const email = firebase?.auth?.currentUser?.email || "";
        return email === "jfcm.s07@gmail.com" || localStorage.getItem("allowAddSongs") === "true";
...
function refreshChordStrip(sectionIndex, lineIndex) {
    ...
    strip.querySelectorAll(".editor-chord-chip").forEach(chip => {
        chip.addEventListener("click", event => {
            ...
            else line.chords[Number(chip.dataset.chordIndex)].chord = value;
...
function addChordAtClick(event) {
    ...
    line.chords.push({ chord: chord.trim(), position });
...
async function saveSong() {
    const allowed = await canAddSongs();
    if (!allowed) {
        alert("Adding songs is disabled by administrator.");
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("addSongBtn")?.addEventListener("click", () => openEditor());
    ...
    window.WorshipHubSongEditor = { open: openEditor, save: saveSong, deleteFirebaseSong: deleteSongFromFirebase };
});
