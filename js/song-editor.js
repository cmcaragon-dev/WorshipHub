...
async function deleteSong(songOrId) {
    ...
    // Persist deletion locally as well as in Firebase so bundled/migrated songs
    // do not reappear after a refresh.
    if (window.WorshipHubDeletedSongs?.remember) {
        window.WorshipHubDeletedSongs.remember(id);
    }
    saveCustomSongs();
...
async function loadCustomSongsFromFirebase() {
    try {
        ...
        snap.forEach(docSnap => {
            ...
            if (window.WorshipHubDeletedSongs?.isDeleted?.(song.id || docSnap.id)) return;
