...
import { canAddSongs, isAdmin, ADMIN_EMAIL } from "./admin-settings.js";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("addSongBtn")?.addEventListener("click", async () => {
        ...
        // Admin is always allowed to add songs, without waiting for a
        // Firestore permission document to load.
        if (isAdmin(user) || await canAddSongs(user)) {
            openEditor();
            return;
        }
        alert("Adding songs is currently disabled for your account. Ask the administrator to enable Add Song in Settings.");
    });
