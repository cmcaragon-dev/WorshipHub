    ...
    list.innerHTML = users.map(u => {
        ...
        return `<div class="user-permission-row" data-user-id="${escapeHtml(u.id)}">
            ...
            <label class="permission-toggle" title="${admin ? "Administrator always has access" : "Toggle Add Song permission"}">
                <input type="checkbox" class="user-song-toggle add-toggle" ${addAllowed ? "checked" : ""} ${admin ? "disabled" : ""}>
                <span></span><b>${addAllowed ? "Add ✓" : "Add ×"}</b>
            </label>
            <label class="permission-toggle" title="${admin ? "Administrator always has access" : "Toggle Edit Song permission"}">
                <input type="checkbox" class="user-song-toggle edit-toggle" ${editAllowed ? "checked" : ""} ${admin ? "disabled" : ""}>
                <span></span><b>${editAllowed ? "Edit ✓" : "Edit ×"}</b>
            </label>
        </div>`;
    }).join("");
    ...
    list.querySelectorAll(".add-toggle").forEach(toggle => {
        toggle.addEventListener("change", async event => {
            ...
            try {
                ...
                event.target.nextElementSibling?.nextElementSibling && (event.target.nextElementSibling.nextElementSibling.textContent = event.target.checked ? "Add ✓" : "Add ×");
    ...
    list.querySelectorAll(".edit-toggle").forEach(toggle => {
        toggle.addEventListener("change", async event => {
            ...
            try {
                ...
                event.target.nextElementSibling?.nextElementSibling && (event.target.nextElementSibling.nextElementSibling.textContent = event.target.checked ? "Edit ✓" : "Edit ×");
                window.dispatchEvent(new CustomEvent("worshiphub:permissions-updated"));
            } catch (error) {
                console.error(error);
                event.target.checked = !event.target.checked;
