...
async function loadAdminSongYoutubeLinks(){
    ...
    try{
        const [linkSnap, customSnap]=await Promise.all([
            getDocs(collection(db,"songYoutubeLinks")),
            getDocs(collection(db,"songs"))
        ]);
        adminYoutubeLinks=new Map();
        linkSnap.forEach(d=>{
            const data=d.data()||{};
            const key=String(data.songKey || d.id);
            adminYoutubeLinks.set(key,String(data.youtube||""));
        });
        const merged=new Map();
        (Array.isArray(staticSongs)?staticSongs:[]).forEach(song=>merged.set(adminSongKey(song),{...song}));
        customSnap.forEach(d=>{
            const data=d.data()||{};
            if(data.customSong===true) merged.set(adminSongKey({...data,id:data.id||d.id}),{...data,id:data.id||d.id});
        });
...
function renderAdminSongYoutubeLinks(list){
    ...
    root.querySelectorAll(".admin-youtube-save").forEach(btn=>btn.addEventListener("click",async()=>{
        ...
        btn.disabled=true;
        try{
            await setDoc(doc(db,"songYoutubeLinks",encodeURIComponent(key)),{
                songKey:key,
                youtube:link,
