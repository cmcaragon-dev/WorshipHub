...
async function loadGlobalAdminData() {
    ...
    let serviceDocs = [], playlistDocs = [];
    try {
        const [serviceSnap, playlistSnap] = await Promise.all([
            getDocs(collectionGroup(db, "services")),
            getDocs(collectionGroup(db, "playlists"))
        ]);
        serviceDocs = serviceSnap.docs;
        playlistDocs = playlistSnap.docs;
    } catch (groupError) {
        console.warn("Collection-group admin query failed; using per-user fallback.", groupError);
        const reads = await Promise.all(users.map(async u => {
            const [ss, ps] = await Promise.all([
                getDocs(collection(db, "users", u.id, "services")),
                getDocs(collection(db, "users", u.id, "playlists"))
            ]);
            return { uid:u.id, services:ss.docs, playlists:ps.docs };
        }));
        serviceDocs = reads.flatMap(x => x.services.map(d => ({doc:d,uid:x.uid})));
        playlistDocs = reads.flatMap(x => x.playlists.map(d => ({doc:d,uid:x.uid})));
    }

    const byUid = new Map(users.map(u => [String(u.id), u]));

    const services = serviceDocs.map(entry => {
        const d = entry.doc || entry;
        const ownerUid = entry.uid || d.ref.parent.parent?.id || "";
        return { id:d.id, path:d.ref.path, ownerUid, owner:byUid.get(String(ownerUid))||null, ...d.data() };
    });

    const playlists = playlistDocs.map(entry => {
        const d = entry.doc || entry;
        const ownerUid = entry.uid || d.ref.parent.parent?.id || "";
        return { id:d.id, path:d.ref.path, ownerUid, owner:byUid.get(String(ownerUid))||null, ...d.data() };
    });
...
function bindAdminDashboardActions(container) {
    container.querySelectorAll("[data-admin-action]").forEach(btn => btn.addEventListener("click", async () => {
        try {
            ...
            // If a modal is open, refresh its contents after an edit/delete.
            const modal = document.getElementById("adminDashboardDetailModal");
            if (modal && !modal.hidden) await openAdminDashboardPanel(btn.dataset.kind, true);
        } catch (error) {
...
async function openAdminDashboardPanel(kind, refresh = false) {
    const root = document.getElementById("globalAdminDashboard");
    if (!root) return;
    if (refresh) {
        // Re-read data so the modal always reflects the latest Firestore state.
        try { await loadGlobalAdminData(); } catch (_) {}
    }

    const data = globalAdminCache || { users: [], services: [], playlists: [] };
    const records = data[kind] || [];
    if (!Array.isArray(records)) return;

    let modal = document.getElementById("adminDashboardDetailModal");
    if (!modal) {
        modal = document.createElement("div");
        modal.id = "adminDashboardDetailModal";
        modal.className = "admin-detail-modal";
        modal.innerHTML = `
            <div class="admin-detail-backdrop" data-admin-detail-close="true"></div>
            <div class="admin-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="adminDetailTitle">
                <div class="admin-detail-header">
                    <div><span class="admin-eyebrow">GLOBAL DASHBOARD</span><h2 id="adminDetailTitle"></h2></div>
                    <button type="button" class="admin-detail-close" data-admin-detail-close="true" aria-label="Close">✕</button>
                </div>
                <div id="adminDetailBody" class="admin-detail-body"></div>
            </div>`;
        document.body.appendChild(modal);
        modal.querySelectorAll('[data-admin-detail-close="true"]').forEach(el => el.addEventListener("click", closeAdminDashboardPanel));
        document.addEventListener("keydown", e => { if (e.key === "Escape") closeAdminDashboardPanel(); });
    }

    const title = kind === "services" ? "All Service Planners" : kind === "playlists" ? "All Playlists" : "All Users";
    const body = modal.querySelector("#adminDetailBody");
    const content = document.createElement("div");
    content.className = "admin-detail-card-content";
    if (kind === "users") {
        content.innerHTML = `<div class="admin-user-list">${renderAdminUsers(records)}</div>`;
    } else {
        content.innerHTML = `<div class="admin-record-list">${renderAdminRecordList(kind, records)}</div>`;
    }
    body.innerHTML = "";
    body.appendChild(content);
    modal.querySelector("#adminDetailTitle").textContent = title;
    modal.hidden = false;
    document.body.classList.add("admin-detail-open");
    bindAdminDashboardActions(modal);
}
...
export async function renderGlobalAdminDashboard() {
    ...
    try {
        ...
        root.innerHTML = `
          ...
          <div class="admin-dashboard-note">Click <b>Service Planners</b>, <b>Playlists</b>, or <b>Total Users</b> above to open the full list. Details are shown in a separate window and are not displayed underneath the dashboard.</div>
        ...
        root.querySelectorAll("[data-admin-tab]").forEach(btn => btn.addEventListener("click", () => {
            openAdminDashboardPanel(btn.dataset.adminTab);
        }));
        bindAdminDashboardActions(root);
    } catch (error) {
...
onAuthStateChanged(auth, async user => {
    ...
    // Dashboard is a dedicated on-demand view. It must never appear in the
    // normal center/library view unless the admin explicitly clicks Dashboard.
    const dashboardBtn = document.getElementById("dashboardBtn");
    const dashboardView = document.getElementById("dashboardView");
    const songLibraryView = document.getElementById("songLibraryView");
    if (dashboardBtn) {
        dashboardBtn.hidden = !isAdmin(user);
        dashboardBtn.onclick = async () => {
            if (!isAdmin(auth.currentUser)) return;
            if (dashboardView) {
                dashboardView.hidden = false;
                dashboardView.dataset.openedByAdmin = "true";
            }
            if (songLibraryView) songLibraryView.hidden = true;
            await renderGlobalAdminDashboard();
            dashboardView?.scrollIntoView({behavior:"smooth", block:"start"});
        };
    }

    // Keep the dashboard closed after login and show the normal song library.
    if (dashboardView && !dashboardView.dataset.openedByAdmin) dashboardView.hidden = true;
    if (songLibraryView) songLibraryView.hidden = false;
    if (document.getElementById("adminSettingsContent")) await renderSettings();
    // Do not load/render the dashboard until the admin opens it.
    if (document.getElementById("globalAdminDashboard") && !document.getElementById("dashboardView")?.hidden) {
        await renderGlobalAdminDashboard();
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Dashboard is intentionally on-demand from the sidebar.
    if (document.getElementById("globalAdminDashboard") && !document.getElementById("dashboardView")?.hidden) {
