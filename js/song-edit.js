...
const isAdmin = u => emailOf(u) === ADMIN_EMAIL;
...
async function canEdit(user) {
    ...
    try {
        ...
        console.warn("Edit permission check failed:", e);
        return false;
    }
}
...
function openEditor() {
    ...
    sections.forEach((section, si) => {
        ...
        [...section.querySelectorAll(":scope > .song-line")].forEach((line, li) => {
            ...
            wrap.innerHTML = `<textarea class="chord-layout-input" spellcheck="false"></textarea><div class="chord-editor-lyric"></div>`;
    ...
    panel.querySelector("#saveChordEdit").onclick = async () => {
        ...
        btn.disabled = true;
        try {
            await setDoc(doc(db, "songs", slug()), {
            ...
            alert("Unable to save the song. Please check Firebase permissions.");
        } finally {
            btn.disabled = false;
        }
    };
