...
function ensureUI(){
  if(document.getElementById("importAllSelahBtn"))return; const anchor=document.getElementById("importSongBtn"); if(!anchor)return;
  const btn=document.createElement("button");btn.id="importAllSelahBtn";btn.type="button";btn.className="chordio-selah-import-button";btn.title="Automatically import or update the complete Selah song library";btn.innerHTML='<span class="quick-icon"><i class="fa-solid fa-cloud-arrow-down"></i></span><span>Import All Selah Songs</span>';anchor.insertAdjacentElement("afterend",btn);
...
document.head.appendChild(style);document.addEventListener("DOMContentLoaded",ensureUI);setTimeout(ensureUI,1000);window.CHORDIOSelahImporter={run:runImport};
