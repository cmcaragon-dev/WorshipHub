...
build() {
    ...
    console.log("BUILDING PRESENTATION — EXACT CHORD/LYRIC PAIRS");

    const output = document.getElementById("presentationLyrics");
    const lyricsSource = document.getElementById("lyrics");

    if (!output || !lyricsSource) {
        console.error("PRESENTATION ERROR: lyrics/output not found");
        return;
    }
    ...
    source.innerHTML = lyricsSource.innerHTML;

    let sections = Array.from(source.querySelectorAll(".song-section"));

    if (!sections.length) {
        const songElement = source.querySelector(".song");
        sections = songElement ? Array.from(songElement.children) : [source];
    }
    ...
    sections.forEach(section => {
        ...
        column.className = "presentation-section";
        ...
        if (sectionTitle) {
            ...
            title.textContent = sectionTitle.textContent.trim();
            column.appendChild(title);
        }

        const content = document.createElement("div");
        content.className = "presentation-section-content";

        const lines = Array.from(section.querySelectorAll(":scope > .song-line"));

        lines.forEach(line => {
            const pair = document.createElement("div");
            pair.className = "presentation-line";

            const chord = line.querySelector(":scope > .chord") ||
                          line.querySelector(".chord");

            /*
             * CRITICAL FIX:
             * Never place the chord and lyric in the same inline line.
             * The original chord text (including every space typed by
             * the administrator) is copied unchanged into its own line.
             * The lyric is then rendered on the following line.
             */
            if (chord) {
                const chordLine = document.createElement("div");
                chordLine.className = "presentation-chord-line";
                const chordText = chord.textContent.replace(/\r?\n/g, "");
                chordLine.textContent = chordText;
                pair.appendChild(chordLine);
            }

            const lyricLine = document.createElement("div");
            lyricLine.className = "presentation-lyric-line";

            const lyricClone = line.cloneNode(true);
            lyricClone.querySelectorAll(".chord").forEach(el => el.remove());

            /* Remove only formatting indentation around the lyric.
               Do NOT alter the chord whitespace above it. */
            lyricLine.textContent = lyricClone.textContent
                .replace(/\r/g, "")
                .replace(/^\s+/, "")
                .replace(/\s+$/, "");

            pair.appendChild(lyricLine);
            content.appendChild(pair);
        });

        /* Fallback for sections that contain no .song-line elements. */
        if (!lines.length) {
            const fallback = section.cloneNode(true);
            fallback.querySelectorAll(".section-title").forEach(el => el.remove());
            fallback.querySelectorAll(".chord").forEach(el => {
                const chordLine = document.createElement("div");
                chordLine.className = "presentation-chord-line";
                chordLine.textContent = el.textContent.replace(/\r/g, "");
                el.replaceWith(chordLine);
            });
    ...
    console.log("PRESENTATION BUILD COMPLETE — CHORDS ABOVE LYRICS");
},
