...
buildPresentationAfterTranspose() {
    try {
        if (
            ...
            typeof Presentation.build === "function"
        ) {
            Presentation.build();
        }

        // Support the dedicated presentation key label used
        // by the modern presentation header.
        const presentationKey =
            document.getElementById("presentationKey");

        if (presentationKey) {
            const song = window.currentSong;
            presentationKey.textContent =
                song?.serviceKey ||
                song?.originalKey ||
                song?.key ||
                "";
        ...
        console.error(
            "PRESENTATION TRANSPOSE REFRESH ERROR:",
            error
        );
    }
},
