...
/* ==========================================================
   PRINT ALL SONGS FROM SELECTED SERVICE
   EACH SONG = ONE SEPARATE PAGE
========================================================== */

function printSelectedServiceSongs() {

    /* ------------------------------------------------------
       1. GET CURRENTLY SELECTED SERVICE
    ------------------------------------------------------ */
    ...
    /* ------------------------------------------------------
       2. GET SONGS FROM THE SELECTED SERVICE
    ------------------------------------------------------ */

    const serviceSongs =
        currentService.songs ||
        currentService.songIds ||
        [];

    if (!serviceSongs.length) {
        alert("There are no songs in this service.");
        return;
    }


    /* ------------------------------------------------------
       3. FIND THE SONG DATA
       Adjust this if your global song array has another name.
    ------------------------------------------------------ */

    const selectedSongs = serviceSongs
        .map(serviceSong => {

            /* If service stores the complete song object */
            if (typeof serviceSong === "object") {
                return serviceSong;
            }

            /* If service stores only the song ID */
            return songs.find(song =>
                String(song.id) === String(serviceSong)
            );
        })
        .filter(Boolean);
    ...
    if (!selectedSongs.length) {
        alert("The songs in this service could not be found.");
        return;
    }


    /* ------------------------------------------------------
       4. CREATE TEMPORARY PRINT WINDOW
    ------------------------------------------------------ */
    ...
    if (!printWindow) {
        alert("Please allow pop-ups for this website to print the service.");
        return;
    }
    ...
    selectedSongs.forEach((song, index) => {
        ...
        /*
         * IMPORTANT:
         * Replace this with the function/property that your
         * application already uses to render a song.
         *
         * If your song content is stored in song.lyrics,
         * this will use that.
         */
        ...
        if (song.html) {
            ...
            songHTML = song.content;

        } else if (song.lyrics) {

            songHTML = song.lyrics;
        ...
        pagesHTML += `
            ...
            <section class="print-song-page">

                <div class="print-song-header">

                    <h1>
                        ${escapePrintHTML(song.title || "Untitled Song")}
                    </h1>

                    ${
                        song.artist
                            ? `
                            <div class="print-song-artist">
                                ${escapePrintHTML(song.artist)}
                            </div>
                            `
                            : ""
                    }
                    ...
                    ${
                        song.key
                            ? `
                            <div class="print-song-key">
                                Key: ${escapePrintHTML(song.key)}
                            </div>
                            `
                            : ""
                    }
...
<title>
    ${escapePrintHTML(
        currentService.name || "Service Songs"
    )}
</title>
...
/* Do not create an extra blank page after the final song */
...
/* ==========================================================
   USE 3 VERTICAL PRINT COLUMNS
========================================================== */

.print-song-content .song {
...
.print-song-content .song-section {
...
.print-song-content .section-title,
.print-song-content .song-section-title {
...
.print-song-content .song-line + .song-line {
...
.print-song-content .song-line p,
.print-song-content .song-line div,
.print-song-content .song-line span {
...
.print-song-content p,
.print-song-content div,
.print-song-content span {
...
.print-song-content #passingChords,
.print-song-content #songEnding {
