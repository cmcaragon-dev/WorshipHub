...
// ======================================================
// TRANSPOSE
// WORKS FOR SERVICE PLANNER + STANDALONE SONG
// ======================================================
...
async transpose(step) {
    ...
    if (!song) {
        ...
        try {
            ...
            if (
                ...
                console.log(
                    "TRANSPOSE: FOUND STANDALONE currentSong"
                );
            }
            ...
            console.log(
                "No global currentSong available."
            );
    ...
    document
        ...
        .forEach(chord => {
            ...
            // ------------------------------------------
            // SAVE ORIGINAL CHORD ON FIRST USE
            // ------------------------------------------
    ...
    // ==================================================
    // UPDATE SERVICE SONG IF AVAILABLE
    // ==================================================
    ...
    if (
        ...
        currentService.songs[serviceIndex]
    ) {
    ...
    if (
        ...
        activeService.songs[serviceIndex]
    ) {
    ...
    // ==================================================
    // REFRESH OPEN PRESENTATION AFTER TRANSPOSE
    // ==================================================
    // The presentation screen is a CLONE of #lyrics.
    // Updating the source .chord elements alone therefore
    // leaves the visible presentation chords unchanged.
    // Rebuild the presentation from the now-transposed
    // source so the key and every chord stay synchronized.
    const presentationScreen =
        document.getElementById("presentationScreen");

    if (
        presentationScreen &&
        presentationScreen.classList.contains("show")
    ) {
        this.buildPresentationAfterTranspose();
    }
    ...
    // ==================================================
    // SAVE ONLY IF THIS IS A SERVICE SONG
    // ==================================================
    ...
    if (
        ...
        console.log(
            "STANDALONE SONG: Firebase service save skipped."
        );
    ...
    // ==================================================
    // SAVE STANDALONE TRANSPOSE LOCALLY
    // ==================================================
    ...
    if (
        ...
        try {
            ...
            localStorage.setItem(
                "standaloneSongTranspose",
                String(App.transpose)
            );
            ...
            console.warn(
                "Unable to save standalone transpose locally:",
                error
            );
...
// ======================================================
// REFRESH PRESENTATION CHORDS AFTER TRANSPOSE
// ======================================================
buildPresentationAfterTranspose() {
    try {
        const totalSteps = Math.round(Number(App.transpose || 0) / 0.5);

        const transposeText = (value) => String(value || "").replace(
            /[A-Ga-g](#|b)?/g,
            note => {
                let noteIndex = SHARP_SCALE.indexOf(note);
                if (noteIndex === -1) noteIndex = FLAT_SCALE.indexOf(note);
                if (noteIndex === -1) return note;
                return SHARP_SCALE[((noteIndex + totalSteps) % 12 + 12) % 12];
            }
        );

        // The presentation is a clone, so update the visible clone directly.
        // Never use the currently displayed chord as the new source.
        document.querySelectorAll("#presentationScreen .chord").forEach(chord => {
            const original = chord.dataset.originalChord || chord.textContent || "";
            chord.dataset.originalChord = original;
            chord.textContent = transposeText(original);
        });

        // If the clone has not been built yet, rebuild it once.
        const presentationScreen = document.getElementById("presentationScreen");
        const presentationLyrics = document.getElementById("presentationLyrics");
        if (presentationScreen && presentationLyrics && !presentationLyrics.querySelector(".chord")) {
            if (typeof Presentation !== "undefined" && typeof Presentation.build === "function") {
                Presentation.build();
            }
        }

        const presentationKey = document.getElementById("presentationKey");
        if (presentationKey) {
            presentationKey.textContent = document.getElementById("serviceKey")?.textContent?.trim() ||
                window.currentSong?.serviceKey || window.currentSong?.originalKey || window.currentSong?.key || "";
        }

        const serviceKey = document.getElementById("serviceKey");
        if (serviceKey && window.currentSong) {
            serviceKey.textContent = window.currentSong.serviceKey || serviceKey.textContent;
        }
    } catch (error) {
        console.error("PRESENTATION TRANSPOSE REFRESH ERROR:", error);
    }
},
...
async start() {
    ...
    if (!overlay) {
        console.error("PRESENTATION ERROR: #presentationScreen NOT FOUND");
        alert("Presentation screen was not found.");
        return;
    }

    // ==================================================
    // DETERMINE SERVICE SONG
    // ==================================================
    ...
    try {
        service = await Service.getCurrent();
    }
    catch (error) {
        console.error("PRESENTATION SERVICE ERROR:", error);
    }

    if (service && Array.isArray(service.songs)) {

        serviceIndex = Service.getSongIndex();

        if (
            serviceIndex >= 0 &&
            serviceIndex < service.songs.length
        ) {
            serviceSong = service.songs[serviceIndex];
        }
    }

    // ==================================================
    // SERVICE PLANNER PRESENTATION
    // ==================================================

    if (serviceSong) {

        console.log("PRESENTATION MODE: SERVICE PLANNER");
        console.log("SERVICE:", service.name);
        console.log("SERVICE INDEX:", serviceIndex);
        console.log("SERVICE SONG:", serviceSong);

        window.currentSong = serviceSong;
        window.currentSongIndex = serviceIndex;
        ...
        console.log(
            "PRESENTATION OVERLAY DISPLAYED"
        );
    ...
    if (!song) {
        ...
        try {
            ...
            console.warn(
                "No standalone currentSong variable."
            );
        }
    }
    ...
    if (!song) {
        ...
        console.error(
            "PRESENTATION ERROR: NO CURRENT SONG"
        );
    ...
    console.log(
        "STANDALONE PRESENTATION OVERLAY DISPLAYED"
    );
...
build() {
    ...
    if (!output) {
        ...
        console.error(
            "PRESENTATION ERROR: #presentationLyrics NOT FOUND"
        );
    ...
    if (!lyricsSource) {
        ...
        console.error(
            "PRESENTATION ERROR: #lyrics NOT FOUND"
        );
    ...
    sections.forEach(section => {
        ...
        if (lines.length) {
            ...
            lines.forEach(line => {
                ...
                // Preserve the original chord value in the presentation
                // copy as well. This prevents a later refresh from using
                // an already-transposed chord as its new source.
                newLine
                    .querySelectorAll(".chord")
                    .forEach(chord => {
                        if (!chord.dataset.originalChord) {
