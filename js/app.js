...
async function startMultiScreenService(serviceId) {
    ...
    const savedMultiIndex = Number(selected.multiScreenConfig?.activeSongIndex);
    const startIndex = Number.isInteger(savedMultiIndex) && savedMultiIndex >= 0 && savedMultiIndex < selected.songs.length ? savedMultiIndex : 0;
    const initialSong = selected.songs[startIndex];
    ...
    try {
        ...
        localStorage.setItem("currentSongIndex", String(startIndex));
        localStorage.setItem("resumePresentation", "true");
    ...
    try {
        if (auth.currentUser) {
            ...
            if (freshSnap.exists()) {
                ...
                const freshSavedIndex = Number(selected.multiScreenConfig?.activeSongIndex);
                const freshIndex = Number.isInteger(freshSavedIndex) && freshSavedIndex >= 0 && freshSavedIndex < selected.songs.length ? freshSavedIndex : startIndex;
                const freshSong = selected.songs[freshIndex];
                if (freshSong?.id) {
                    try {
                        localStorage.setItem("currentServiceSnapshot", JSON.stringify(selected));
                        localStorage.setItem("currentServiceName", String(selected.name || selected.title || "Service Planner"));
                        localStorage.setItem("currentSongIndex", String(freshIndex));
                        const freshUrl = "custom-song.html?id=" + encodeURIComponent(freshSong.id) + "&multiScreenStart=1&ts=" + Date.now();
                        if (multiScreenPage && !multiScreenPage.closed && String(freshSong.id) !== String(initialSong.id)) {
