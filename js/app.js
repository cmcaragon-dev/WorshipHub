...
playlistBtn.onclick=function(){
    ...
    document.body.classList.add("playlist-manager-open");
...
<button onclick="openYoutube(${list.id})">
    ...
    <i class="fa-brands fa-youtube youtube-real-icon" aria-hidden="true"></i> YouTube
...
function updateDashboard(){
    const visibleSongs = (Array.isArray(songs) ? songs : []).filter(song => !isAnyDeletedSong(song) && hasStructuredLyrics(song));
    const artistNames = visibleSongs.map(song => String(song?.artist || "").trim()).filter(Boolean);
    const totalSongs = document.getElementById("totalSongs");
    if(totalSongs) totalSongs.textContent = String(visibleSongs.length);
    const totalArtists = document.getElementById("totalArtists");
    if(totalArtists) totalArtists.textContent = String(new Set(artistNames.map(a => a.toLocaleLowerCase())).size);
    const totalServices = document.getElementById("totalServices");
    if(totalServices) totalServices.textContent = String(Array.isArray(services) ? services.length : 0);
    const totalPlaylists = document.getElementById("totalPlaylists");
    if(totalPlaylists) totalPlaylists.textContent = String(Array.isArray(playlists) ? playlists.length : 0);
    const current = document.getElementById("currentService");
    if(current){ const active = getCurrentService(); current.textContent = active ? active.name : "None"; }
}
...
function renderSongs(songList) {
    ...
    songGrid.innerHTML = list.map(song => `
        <article class="song-card" data-song-id="${escapeHtml(song.id)}">
            <div class="song-card-topline">
                <span class="song-card-index" aria-hidden="true">♪</span>
                <span class="song-card-status">SONG</span>
            </div>
            <div class="song-card-body">
                <h3 class="song-card-title">${escapeHtml(song.title || "Untitled Song")}</h3>
                <div class="song-card-meta">
                    <span class="song-meta-item"><b>ARTIST</b><span>${escapeHtml(song.artist || "—")}</span></span>
                    <span class="song-meta-item"><b>KEY</b><span>${escapeHtml(song.key || song.originalKey || "—")}</span></span>
                    <span class="song-meta-item"><b>CATEGORY</b><span>${escapeHtml(song.category || "—")}</span></span>
                </div>
            </div>
            <div class="song-card-actions ${canManageSongs ? "song-card-actions-managed" : "song-card-actions-basic"}">
                <a class="open-song" href="${song.customSong ? `custom-song.html?id=${encodeURIComponent(song.id)}` : (song.file || "#")}" title="Open song">🎵 <span>Open</span></a>
                <a class="youtube-song-btn${song.youtube ? "" : " disabled"}" href="${song.youtube ? escapeHtml(song.youtube) : "#"}" ${song.youtube ? 'target="_blank" rel="noopener noreferrer"' : 'aria-disabled="true" onclick="return false;"'} title="${song.youtube ? "Open YouTube link" : "No YouTube link added"}"><i class="fa-brands fa-youtube youtube-real-icon" aria-hidden="true"></i><span>YouTube</span></a>
                ${canManageSongs ? `
                    <button type="button" class="song-action-btn edit" data-card-action="edit" data-song-id="${escapeHtml(song.id)}" title="Edit song">✏ <span>Edit</span></button>
                    <button type="button" class="song-action-btn delete" data-card-action="delete" data-song-id="${escapeHtml(song.id)}" title="Delete song">🗑 <span>Delete</span></button>
                ` : ""}
            </div>
        </article>
    `).join("");
...
async function() {
    ...
    try {
        ...
        // Show the newly-created service immediately after Firebase confirms
        // the write. Do not require another action, such as adding a song.
        ...
        // Confirm the list against Firebase as well, so the UI remains
        // synchronized with the actual persisted services.
        try {
            const freshServices = await loadServices(currentUser.uid);
            ...
            console.warn("Service saved, but Firebase list refresh failed:", refreshError);
        }
...
async function selectSong(file) {
    ...
    const serviceSong = {
        ...
        // Unique occurrence identity: duplicated songs get independent Multi-Screen state.
        instanceId:
...
function addSongsToPlaylist(playlistId){
    ...
    // Keep Playlist Manager open while the shared song picker is displayed.
    document.body.classList.add("playlist-manager-open");
