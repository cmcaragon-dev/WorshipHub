...
onAuthStateChanged(auth, async function(user) {
    ...
    currentUser = user;
    // Render the bundled/local song library immediately. Do not block the main
    // UI on profile, migration, or the full /songs Firestore collection.
    filterDeletedSongsFromLibrary();
    removeDuplicateSongTitles();
    songsReady = true;
    renderSongs(songs);
    ...
    // Cloud synchronization is intentionally non-blocking. The previous
    // startup chain waited for several Firestore reads before the user could
    // open a song or Service Planner, which made the UI appear stuck on
    // "Loading songs…".
    void (async()=>{
        try {
            await loadCurrentUserProfile();
            await migrateLocalDeletedTitlesToFirebase();
            void syncAllSongDocumentsIntoLibrary();
        } catch(error) {
            console.warn("Background account sync failed:", error);
        }
    })();
...
servicePlannerBtn.onclick = function(){
    ...
    // Render the current list immediately. Firestore refresh happens in the
    // background so the panel never waits on a network round trip.
    renderServices();
    if (currentUser?.uid) {
        void loadServices(currentUser.uid).then(freshServices=>{
            if (!Array.isArray(freshServices)) return;
            services = freshServices;
            services.sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")));
            window.services = services;
            if (servicePanel.classList.contains("show")) renderServices();
        }).catch(error=>console.warn("Unable to refresh Service Planner list:",error));
    }
}
...
async function selectSong(file) {
    ...
    const serviceSong = {
        ...
        // Unique identity for this occurrence inside the Service Planner.
        // Two copies of the same song must be able to have different structures.
        serviceInstanceId:
            (typeof crypto !== "undefined" && crypto.randomUUID)
                ? crypto.randomUUID()
                : `service-song-${Date.now()}-${Math.random().toString(36).slice(2,10)}`,
...
async function startMultiScreenService(serviceId) {
    ...
    // Open a blank tab first. Navigating immediately to custom-song.html can
    // race the localStorage writes below: the destination may execute its
    // loader before currentServiceSnapshot exists, leaving it on "Loading song…".
    // The blank tab is opened during the user click so popup blocking still works.
    const multiScreenPage = window.open("about:blank", "_blank");

    // Cache the complete planner BEFORE navigating the destination page.
    try {
    ...
    if (multiScreenPage && !multiScreenPage.closed) {
        try { multiScreenPage.location.replace(initialUrl); }
        catch (_) { multiScreenPage.location.href = initialUrl; }
    }
