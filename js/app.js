...
function showAdminServiceButton(){
    if(!allServicePlanBtn) return;
    const visible = isCurrentAdmin();
    allServicePlanBtn.style.display = visible ? "block" : "none";
    allServicePlanBtn.setAttribute("aria-hidden", visible ? "false" : "true");
}
...
async function loadAllServicePlans(){
    ...
    try{
        // Load users first. Some Firestore security rules allow an admin to
        // read each user's services subcollection but deny collectionGroup()
        // reads. The per-user fallback below works with that common setup.
        ...
        try {
            const serviceSnap = await getDocs(collectionGroup(db,"services"));
            ...
            console.warn("collectionGroup services read denied; using per-user service reads:", groupError);
            const userEntries=[...users.entries()];
            const results=await Promise.all(userEntries.map(async ([uid,u])=>{
                try {
                    const snap=await getDocs(collection(db,"users",uid,"services"));
                    return snap.docs.map(d=>({
                        id:d.id, ownerUid:uid,
                        ownerEmail:u.email||u.displayName||uid,
                        ...(d.data()||{})
                    }));
                } catch (userServiceError) {
                    console.warn(`Unable to read services for user ${uid}:`, userServiceError);
                    return [];
                }
            }));
            results.flat().forEach(plan=>plans.push(plan));
        }
...
function renderSongs(songList) {
    ...
    const list = (Array.isArray(songList) ? [...songList] : []).filter(hasStructuredLyrics).sort((a, b) =>
...
function renderSongPicker(list) {
    ...
    list.filter(hasStructuredLyrics).forEach(function(song) {
