...
// ==========================================
// INDEX INTERNAL DIRECTORY VIEWS
// ==========================================

const indexDirectoryPanel =
    document.getElementById("indexDirectoryPanel");

const indexDirectoryContent =
    document.getElementById("indexDirectoryContent");

const indexDirectoryTitle =
    document.getElementById("indexDirectoryTitle");

const indexDirectorySubtitle =
    document.getElementById("indexDirectorySubtitle");

const indexDirectoryEyebrow =
    document.getElementById("indexDirectoryEyebrow");

const closeIndexDirectory =
    document.getElementById("closeIndexDirectory");

function closeIndexDirectoryView(){
    if(!indexDirectoryPanel) return;
    indexDirectoryPanel.classList.remove("show");
    indexDirectoryPanel.setAttribute("aria-hidden", "true");
}

function openIndexDirectory(title, subtitle, content, eyebrow="LIBRARY"){
    if(!indexDirectoryPanel) return;

    // Close other index-only panels first.
    document.getElementById("allSongsPanel")?.classList.remove("show");
    document.getElementById("playlistPanel")?.classList.remove("show");
    document.getElementById("servicePanel")?.classList.remove("show");

    if(indexDirectoryTitle) indexDirectoryTitle.textContent = title;
    if(indexDirectorySubtitle) indexDirectorySubtitle.textContent = subtitle;
    if(indexDirectoryEyebrow) indexDirectoryEyebrow.textContent = eyebrow;
    if(indexDirectoryContent) indexDirectoryContent.innerHTML = content;

    indexDirectoryPanel.classList.add("show");
    indexDirectoryPanel.setAttribute("aria-hidden", "false");
}
...
function showAllArtists(){
    ...
    const html = artists.length
        ? `<div class="directory-grid">${artists.map((artist,index) => `
            <button class="directory-item" type="button" onclick="filterSongsByArtist(decodeURIComponent('${encodeURIComponent(artist)}'))">
                <span class="directory-item-icon">♬</span>
                <span>
                    <strong>${escapeHtml(artist)}</strong>
                    <small>${songs.filter(song => String(song.artist || "").trim() === artist).length} song(s)</small>
                </span>
                <span class="directory-arrow">→</span>
            </button>
        `).join("")}</div>`
        : `<div class="directory-empty">No artists found.</div>`;

    openIndexDirectory(
        "All Artists",
        `${artists.length} artist${artists.length === 1 ? "" : "s"} in your song library.`,
        html,
        "ARTISTS"
    );
}
...
function filterSongsByArtist(artist){
    ...
    const filtered = songs
        .filter(song => String(song.artist || "").trim() === artist)
    ...
    filtered.forEach((song,index) => {
        ...
        row.innerHTML = `
            <td>${index + 1}</td>
            <td><a href="${song.file}" class="all-song-title">🎵 ${escapeHtml(song.title || "Untitled Song")}</a></td>
    ...
    const search = document.getElementById("allSongsSearch");
    if(search) search.value = artist;
    panel.classList.add("show");
}
...
window.filterSongsByArtist = filterSongsByArtist;
window.closeIndexDirectoryView = closeIndexDirectoryView;

if(closeIndexDirectory){
    closeIndexDirectory.onclick = closeIndexDirectoryView;
}

if(indexDirectoryPanel){
    indexDirectoryPanel.addEventListener("click", function(event){
        if(event.target === indexDirectoryPanel){
            closeIndexDirectoryView();
        }
    });
}
