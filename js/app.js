...
function showAllSongs() {
    ...
    allSongs.forEach(function(song, index) {
        ...
        const safeFile = song.customSong
        ...
        row.innerHTML = `
            <td>${index + 1}</td>
            <td>
                <a href="${safeFile}" class="all-song-title">
                    🎵 ${title}
                </a>
            </td>
            <td>${artist || "—"}</td>
            ...
            <td>${language || "—"}</td>
            <td>${key || "—"}</td>
            <td class="song-actions-cell">${song.customSong ? `
              <button type="button" class="song-action-btn edit" data-song-action="edit" data-song-id="${String(song.id)}">✏ Edit</button>
              <button type="button" class="song-action-btn delete" data-song-action="delete" data-song-id="${String(song.id)}">🗑 Delete</button>` : '<span class="song-action-muted">Built-in</span>'}</td>
        `;

        if (song.customSong) {
            row.querySelector('[data-song-action="edit"]')?.addEventListener('click', e => {
                const target = songs.find(x => String(x.id) === String(e.currentTarget.dataset.songId));
                if (target) window.WorshipHubSongEditor?.open(target);
            });
            row.querySelector('[data-song-action="delete"]')?.addEventListener('click', async e => {
                await window.WorshipHubSongEditor?.deleteSong(e.currentTarget.dataset.songId);
                showAllSongs();
            });
        }
