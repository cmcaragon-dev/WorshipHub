...
function togglePlaylist(id){
    ...
    try {
        ...
        saved[String(id)] = body.classList.contains("show");
        localStorage.setItem(collapsedKey, JSON.stringify(saved));
    } catch (_) {}
        ...
        serviceList.innerHTML += `
            ...
            <div class="service-item">
                ...
                <div
                    ...
                    <div class="service-date" title="Planner date">
...
>
    ...
    // Restore each planner's collapsed/expanded state. New planners default collapsed.
    try {
        const saved = JSON.parse(localStorage.getItem("chordioCollapsedServicesV65") || "{}");
        serviceList.querySelectorAll(".service-item").forEach(item => {
            const body = item.querySelector(".service-body");
            const arrow = item.querySelector("[id^=serviceArrow]");
            const id = body?.id?.replace("serviceBody", "");
            const expanded = saved[String(id)] === true;
...
function toggleService(id) {
    const collapsedKey = "chordioCollapsedServicesV65";
