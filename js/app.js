...
function filterDeletedSongsFromLibrary(){
    ...
    for(let i=songs.length-1;i>=0;i--){
        if(deleted.has(String(songs[i]?.id))) songs.splice(i,1);
    }
}
...
onAuthStateChanged(auth, async function(user) {
    ...
    // Wait for the authenticated profile and Firebase song-link sync before
    // rendering. This prevents the temporary 75 -> 56 song count.
    await syncSongYoutubeLinksIntoLibrary();

    // Re-clean after Firebase/custom-song synchronization and render once.
    renderSongs(songs);
    if (typeof renderAllSongsTable === "function") renderAllSongsTable(songs);
...
function renderAllSongsTable(songList) {
    ...
    tableBody.innerHTML = "";
    const list = getCleanSongLibrary(songList);
...
function getCleanSongLibrary(songList) {
    // The Song Cards and All Songs table MUST use the exact same library.
    // This prevents deleted/unstructured/duplicate records from appearing
    // again in All Songs while the cards show the cleaned list.
    filterDeletedSongsFromLibrary();
    removeUnstructuredSongsFromLibrary();
    removeDuplicateSongTitles();

    const source = Array.isArray(songList) ? songList : songs;
    const deleted = getDeletedSongIds();
    const byTitle = new Map();

    for (const song of source) {
        if (!song || !song.id || deleted.has(String(song.id)) || !hasStructuredLyrics(song)) continue;
        const titleKey = String(song.title || "").trim().replace(/\s+/g, " ").toLocaleLowerCase();
        if (!titleKey) continue;
        const existing = byTitle.get(titleKey);
        if (!existing || songContentScore(song) > songContentScore(existing)) {
...
function renderSongs(songList) {
    ...
    // Always render only the final, cleaned library. Never briefly display
    // the raw bundled count before Firebase/auth synchronization finishes.
    const list = getCleanSongLibrary(songList);
...
async function() {
    ...
    try {
        ...
        // Persist the complete service directly to Firebase first.
        // Only update the visible list after the write succeeds.
        await saveService(
            currentUser.uid,
            service
        );

        // Re-read Firebase so the Service Planner list reflects exactly what
        // was persisted and survives refresh/re-login.
