...
// ==========================================
// MODERN LIBRARY NAVIGATION
// ==========================================
function showLibraryHome(show=true){
    const view=document.getElementById("libraryView");
    if(!view) return;
    view.hidden=!show;
    if(!show){ view.innerHTML=""; return; }
    view.innerHTML="";
}
...
function openLibrarySong(song){
    if(!song) return;
    if(song.customSong){
        location.href=`custom-song.html?id=${encodeURIComponent(song.id)}`;
    } else if(song.file){
        openSong(song.file);
    }
}
window.openLibrarySong=openLibrarySong;

function showLibraryView(type){
    const view=document.getElementById("libraryView");
    if(!view) return;
    view.hidden=false;
    document.querySelectorAll(".library-nav-active").forEach(el=>el.classList.remove("library-nav-active"));
    const renderHeader=(icon,title,sub)=>`<div class="library-view-head"><div><div class="eyebrow">WORSHIPHUB LIBRARY</div><h2>${icon} ${title}</h2><p>${sub}</p></div></div>`;
    if(type==='artists'){
        const map={};
        songs.forEach(s=>{const a=String(s.artist||'Unknown Artist').trim()||'Unknown Artist'; (map[a] ||= []).push(s);});
        const artists=Object.entries(map).sort((a,b)=>a[0].localeCompare(b[0]));
        view.innerHTML=renderHeader('🎤','Artists',`${artists.length} artists in your song library`)+`<div class="library-grid">${artists.map(([artist,list])=>`<article class="library-card"><div class="library-card-icon">🎤</div><div><h3>${escapeHtml(artist)}</h3><p>${list.length} ${list.length===1?'song':'songs'}</p></div><button type="button" class="library-card-action" onclick="showArtistSongs(${JSON.stringify(artist)})">View songs</button></article>`).join('')}</div>`;
    } else if(type==='services'){
        const list=Array.isArray(services)?[...services].sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))):[];
        view.innerHTML=renderHeader('📅','Services',`${list.length} service planners`)+`<div class="library-stack">${list.length?list.map(s=>`<article class="library-service-card"><div><h3>${escapeHtml(s.name||'Untitled Service')}</h3><p>${Array.isArray(s.songs)?s.songs.length:0} songs</p></div><button type="button" class="library-card-action" onclick="showServiceSongs(${JSON.stringify(String(s.id))})">View songs</button></article>`).join(''):`<div class="library-empty">No service planners found.</div>`}</div>`;
    } else if(type==='playlists'){
        const list=Array.isArray(playlists)?[...playlists].sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))):[];
        view.innerHTML=renderHeader('🎶','Playlists',`${list.length} playlists • select a playlist to see its linked songs below`)+`<div class="library-stack">${list.length?list.map(p=>`<article class="library-playlist-card"><button class="playlist-select" type="button" onclick="showPlaylistSongs(${JSON.stringify(String(p.id))})"><span class="playlist-cover">♪</span><span><strong>${escapeHtml(p.name||'Untitled Playlist')}</strong><small>${Array.isArray(p.songs)?p.songs.length:0} songs</small></span><span class="playlist-chevron">›</span></button><div id="libraryPlaylistSongs-${escapeHtml(String(p.id))}" class="library-playlist-songs" hidden></div></article>`).join(''):`<div class="library-empty">No playlists found.</div>`}</div>`;
    }
}
...
function showArtistSongs(artist){
    ...
    const list=songs.filter(s=>String(s.artist||'Unknown Artist').trim()===String(artist));
    view.innerHTML=`<div class="library-view-head"><div><button class="library-back" onclick="showLibraryView('artists')">← Artists</button><div class="eyebrow">ARTIST</div><h2>🎤 ${escapeHtml(artist)}</h2><p>${list.length} songs</p></div></div><div class="library-song-list">${list.map((s,i)=>`<button class="library-song-row" type="button" onclick="openLibrarySong(songs.find(x=>String(x.id)===${JSON.stringify(String(s.id))}))"><span class="song-row-number">${i+1}</span><span class="song-row-main"><strong>${escapeHtml(s.title||'Untitled')}</strong><small>${escapeHtml(s.key||s.originalKey||'—')}</small></span><span>›</span></button>`).join('')}</div>`;
}
window.showArtistSongs=showArtistSongs;

function showServiceSongs(id){
    const s=services.find(x=>String(x.id)===String(id)); if(!s)return;
    const list=Array.isArray(s.songs)?s.songs:[];
    const view=document.getElementById('libraryView'); if(!view)return;
    view.innerHTML=`<div class="library-view-head"><div><button class="library-back" onclick="showLibraryView('services')">← Services</button><div class="eyebrow">SERVICE PLANNER</div><h2>📅 ${escapeHtml(s.name||'Untitled Service')}</h2><p>${list.length} songs</p></div></div><div class="library-song-list">${list.map((x,i)=>`<button class="library-song-row" type="button"><span class="song-row-number">${i+1}</span><span class="song-row-main"><strong>${escapeHtml(x.title||'Untitled')}</strong><small>${escapeHtml(x.artist||'')} · Key: ${escapeHtml(x.serviceKey||x.key||'—')}</small></span></button>`).join('')}</div>`;
}
window.showServiceSongs=showServiceSongs;

function showPlaylistSongs(id){
    const p=playlists.find(x=>String(x.id)===String(id)); if(!p)return;
    const host=document.getElementById(`libraryPlaylistSongs-${String(id).replace(/[^a-zA-Z0-9_-]/g,'')}`) || document.getElementById(`libraryPlaylistSongs-${id}`);
    if(!host)return;
    host.hidden=false;
    const list=Array.isArray(p.songs)?p.songs:[];
    host.innerHTML=`<div class="playlist-linked-header"><span>SONGS IN PLAYLIST</span>${p.youtube?`<button type="button" onclick="openYoutube(${JSON.stringify(String(id))})">▶ YouTube</button>`:''}</div>${list.length?list.map((x,i)=>`<button class="library-song-row compact" type="button" onclick="openLibrarySong(songs.find(s=>String(s.id)===${JSON.stringify(String(x.id||x.songId||''))}))"><span class="song-row-number">${i+1}</span><span class="song-row-main"><strong>${escapeHtml(x.title||'Untitled')}</strong><small>${escapeHtml(x.artist||'')}</small></span><span>›</span></button>`).join(''):`<div class="library-empty small">No songs linked to this playlist yet.</div>`}`;
}
