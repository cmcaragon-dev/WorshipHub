...
function filterDeletedSongsFromLibrary(){
    ...
    for(let i=songs.length-1;i>=0;i--){
        const song = songs[i];
        if(deleted.has(String(song?.id)) || deletedTitles.has(normalizeDeletedSongTitle(song?.title))) songs.splice(i,1);
    }
}
window.WorshipHubDeletedSongs = {
    isDeleted(id){ return getDeletedSongIds().has(String(id)); },
    isDeletedTitle(title){ return isDeletedSongTitle(title); },
    remember(id){
...
async function syncAllSongDocumentsIntoLibrary(){
    try {
        ...
        snap.forEach(docSnap => {
            ...
            if (!id || (titleKey && isDeletedSongTitle(data.title))) return;
            // Firebase is the shared source of truth for song edits. Merge the
            // cloud copy over the bundled/static copy so every user sees the
            // same admin/approved-user edits after login or refresh.
            let index = songs.findIndex(song => String(song?.id || "").trim() === id);
            if (index < 0 && titleKey) {
                index = songs.findIndex(song => normalizeDeletedSongTitle(song?.title) === titleKey);
            }
            if (index >= 0) {
                songs[index] = { ...songs[index], ...data, id };
            } else if (data.customSong === true || Array.isArray(data.sections)) {
                songs.push({ ...data, id, customSong: data.customSong === true });
            }
            changed = true;
        });
        if (changed) {
        ...
        console.warn("Unable to sync shared Firebase song edits:", error);
    }
}
...
async function syncSongYoutubeLinksIntoLibrary(){
    try{
        ...
        snap.forEach(d=>{
            ...
            if(!key || !data.youtube || isDeletedSongTitle(data.title)) return;
...
async function loadCurrentUserProfile(){
    ...
    // Wait for the shared song collection after authentication. This makes
    // edits made by the administrator/approved users visible to every user.
...
function showAllSongs() {
    ...
    filterDeletedSongsFromLibrary();
    removeDuplicateSongTitles();
    renderAllSongsTable([...songs].filter(song => !isDeletedSongTitle(song?.title)).sort((a,b) => String(a.title || "").localeCompare(String(b.title || ""))));
}
...
async function selectSong(file) {
    ...
    if (getDeletedSongIds().has(String(song.id)) || isDeletedSongTitle(song.title)) {
