...
async function renamePlaylist(id) {
    ...
    playlist.name = name;
    try {
        await savePlaylist(currentUser.uid, playlist);
        renderPlaylists();
    } catch (error) {
...
function printServiceSongs() {
    ...
    if (!service || !Array.isArray(service.songs) || !service.songs.length) {
        alert("There are no songs in the active service to print.");
        return;
    }
    const rows = service.songs.map((song, i) => `
        <div class="song-row">
            <div class="song-number">${i + 1}</div>
            <div><strong>${escapeHtml(song.title || "Untitled Song")}</strong><br><span>${escapeHtml(song.artist || "")}</span></div>
            <div class="song-key">${escapeHtml(song.serviceKey || song.key || "—")}</div>
        </div>`).join("");
    const win = window.open("", "_blank", "noopener,noreferrer");
    if (!win) { alert("Please allow pop-ups to print the service."); return; }
    win.document.write(`<!doctype html><html><head><title>${escapeHtml(service.name || "Service Planner")}</title><style>body{font-family:Arial,sans-serif;margin:40px;color:#111}.song-row{display:grid;grid-template-columns:45px 1fr 80px;gap:15px;padding:12px 0;border-bottom:1px solid #ddd}.song-number,.song-key{font-weight:700}.song-row span{color:#666;font-size:12px}h1{margin-bottom:4px}p{color:#666}</style></head><body><h1>${escapeHtml(service.name || "Service Planner")}</h1><p>${service.songs.length} song(s)</p>${rows}<script>window.onload=()=>window.print()<\/script></body></html>`);
    win.document.close();
}
