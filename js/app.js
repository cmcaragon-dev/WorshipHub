...
// HOME VIEW
// ==========================================
// The Home button always returns the index to its normal library view.
// It closes on-demand panels and the admin dashboard without creating
// another HTML page.
function showHomeView() {
    [
        ...
        if (id === "dashboardView" || id === "globalAdminDashboard") el.hidden = true;
    });

    document.body.classList.remove("service-planner-open", "playlist-manager-open", "dashboard-open");

    const library = document.getElementById("songLibraryView");
    if (library) library.hidden = false;

    if (typeof renderSongs === "function" && Array.isArray(window.songs)) {
    ...
    list.forEach((song, index) => {
        ...
        row.innerHTML = `
            ...
            <td class="song-actions-cell">
                ${song.customSong ? `
                    ${isAdmin(currentUser) ? '<button type="button" class="song-action-btn edit" data-song-action="edit" data-song-id="' + escapeHtml(song.id) + '">✏ Edit</button>' : ''}
...
function openWorkspacePanel(panel, kind) {
    const content = document.querySelector(".content");
    if (content && panel && panel.parentElement !== content) content.appendChild(panel);
    document.body.classList.add("workspace-panel-open", `workspace-${kind}-open`);
    document.querySelectorAll(".playlist-panel.show").forEach(p => {
        if (p !== panel) p.classList.remove("show");
    });
    panel?.classList.add("show");
}

function closeWorkspacePanel(panel, kind) {
    panel?.classList.remove("show");
    document.body.classList.remove("workspace-panel-open", `workspace-${kind}-open`);
}

playlistBtn.onclick=function(){
    openWorkspacePanel(playlistPanel, "playlist");
    renderPlaylists();
}

closePlaylist.onclick=function(){
    closeWorkspacePanel(playlistPanel, "playlist");
}
...
async function() {
    ...
    try {
        ...
        updatePlaylistCounter();
        window.WorshipHubAdmin?.renderGlobalAdminDashboard?.();
...
servicePlannerBtn.onclick=function(){
    openWorkspacePanel(servicePanel, "service");
    renderServices();
}

closeService.onclick=function(){

    closeWorkspacePanel(servicePanel, "service");
...
/* Keep the admin dashboard in sync after Service Planner / Playlist changes. */
window.addEventListener("worshiphub:admin-data-changed", () => {
    window.WorshipHubAdmin?.renderGlobalAdminDashboard?.();
});
