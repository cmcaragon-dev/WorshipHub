...
async function loadFirebaseDeletedSongTitles(){
    ...
    try{
        ...
        // Promote cloud tombstones into the local cache so older code paths
        // also respect the permanent deletion immediately.
...
async function saveFirebaseDeletedSongTitle(title){
    const key = normalizeDeletedSongTitle(title);
    if(!key) return;
    await setDoc(doc(db, "songs", deletedSongDocId(key)), {
        deletedSong: true,
        titleKey: key,
    ...
    firebaseDeletedSongTitles.add(key);
}
...
async function syncAllSongDocumentsIntoLibrary(){
    ...
    try {
        // Read deletion markers first. They live in the existing /songs
        // collection so no new Firestore security rule is required.
        await loadFirebaseDeletedSongTitles();
        const snap = await getDocs(collection(db, "songs"));
        let changed = false;
        snap.forEach(docSnap => {
            const data = docSnap.data() || {};
        ...
        // Keep the app usable even if a temporary Firestore read fails, but do
        // not pretend the cloud sync has completed.
        songsReady = true;
        filterDeletedSongsFromLibrary();
        removeDuplicateSongTitles();
        if (typeof renderSongs === "function") renderSongs(songs);
...
onAuthStateChanged(auth, async function(user) {
    ...
    // Re-read cloud deletion markers after migrating older local deletions.
    await loadFirebaseDeletedSongTitles();
    filterDeletedSongsFromLibrary();
    removeDuplicateSongTitles();
    songsReady = true;
