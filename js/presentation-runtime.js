...
function getServiceKey(song) {
    // The Song Page's restored Service Key is the single source of truth.
    // song-runtime.js sets __worshipHubServiceKeyReady after Firebase restores
    // the selected Service Planner entry. Do not let the static HTML key or an
    // older presentation copy override it.
...
function renderPresentation() {
    ...
    if (presentationKey) {
        presentationKey.textContent = `Service Key: ${getServiceKey(currentSong)}`;
    }
...
window.startPresentation = async function startPresentation(options = {}) {
    ...
    // Wait briefly for song-runtime.js to restore the Service Planner key from
    // Firebase. This prevents the static song HTML key (for example D) from
    // being captured before the saved Service Key (for example C) is ready.
    ...
    if (service?.songs?.length) {
        ...
        if (found >= 0) {
            ...
            // If song-runtime has restored the page's Firebase Service Key, keep
