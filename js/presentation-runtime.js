...
function rebuildLine(line) {
     ...
     *
     * Every source song line is rendered as two independent, absolutely
     * positioned rows:
     *
     *     CHORDS  (row 1)
     *     LYRICS  (row 2)
     *
     * The chord layout string is NEVER trimmed or reflowed. Its whitespace
     * represents the character positions entered by the admin. Chord tokens
     * are positioned from those original character indexes using the SAME
     * monospace font metrics as the lyric row.
     *
     * This deliberately avoids putting chords and lyrics in the same inline
     * formatting context. Therefore a chord can never sit behind, inside, or
     * on top of the lyric text.
     */
    const chordNodes = [...line.querySelectorAll(":scope > .chord")];

    // Preserve every horizontal space. Only source line-break characters are
    // removed because they are formatting from the HTML source itself.
    const chordText = chordNodes
        .map(chord => String(chord.textContent || "").replace(/\r?\n/g, ""))
        .join("");

    // Only direct text nodes belong to the lyric. Do not accidentally include
    // text from chord elements or nested controls.
    ...
    // The line itself is a fixed two-row coordinate space. Row 1 and row 2
    // have exactly the same font metrics and are vertically separated by
    // exactly one line-height unit -- no extra gap.
    line.style.position = "relative";
    line.style.display = "block";
    line.style.width = "100%";
    line.style.height = chordText.trim() ? "2em" : "1em";
    line.style.minHeight = chordText.trim() ? "2em" : "1em";
    ...
    chordRow.style.position = "absolute";
    chordRow.style.left = "0";
    chordRow.style.right = "0";
    chordRow.style.top = "0";
    chordRow.style.width = "100%";
    chordRow.style.height = "1em";
    chordRow.style.margin = "0";
    chordRow.style.padding = "0";
    chordRow.style.lineHeight = "1";
    chordRow.style.fontFamily = "inherit";
    chordRow.style.fontSize = "1em";
    chordRow.style.whiteSpace = "pre";
    chordRow.style.overflow = "visible";
    chordRow.style.boxSizing = "border-box";

    const lyricRow = document.createElement("div");
    lyricRow.className = "presentation-lyric-row";
    lyricRow.style.position = "absolute";
    lyricRow.style.left = "0";
    lyricRow.style.right = "0";
    lyricRow.style.top = chordText.trim() ? "1em" : "0";
    lyricRow.style.width = "100%";
    lyricRow.style.height = "1em";
    lyricRow.style.margin = "0";
    lyricRow.style.padding = "0";
    lyricRow.style.lineHeight = "1";
    lyricRow.style.fontFamily = "inherit";
    lyricRow.style.fontSize = "1em";
    lyricRow.style.whiteSpace = "pre";
    lyricRow.style.overflow = "visible";
    lyricRow.style.boxSizing = "border-box";
    ...
    if (chordText.trim()) {
        ...
        span.className = "presentation-chord-source";
        span.textContent = chordText;
        span.dataset.chordLayout = "true";
        span.style.display = "none";
        chordRow.appendChild(span);
    }
}

function applyExactPresentationChordPositions(root = document) {
    root.querySelectorAll(".presentation-line").forEach(line => {
        const lyric = line.querySelector(":scope > .presentation-lyric-row");
        const chordRow = line.querySelector(":scope > .presentation-chord-row");
        const sourceChord = line.querySelector(":scope > .presentation-chord-row > .presentation-chord-source");
        if (!lyric || !chordRow || !sourceChord) return;
        ...
        // Measure the exact monospace character width used by the lyric row.
        // This is the coordinate grid for every chord entered in the source.
        ctx.font = `${cs.fontWeight} ${cs.fontSize} ${cs.fontFamily}`;
        const charWidth = ctx.measureText("0").width || parseFloat(cs.fontSize) * 0.6;

        const layout = String(sourceChord.textContent || "");
        const step = getTransposeSteps();

        chordRow.querySelectorAll(":scope > .presentation-chord").forEach(el => el.remove());

        // Find every chord token but retain its ORIGINAL character index.
        // Never trim, collapse, center, distribute, or otherwise reflow it.
        const tokenRe = /\S+/g;
        let match;
        while ((match = tokenRe.exec(layout))) {
            const token = document.createElement("span");
            token.className = "presentation-chord";
            token.textContent = transposeChord(match[0], step);
            token.style.position = "absolute";
            token.style.left = `${match.index * charWidth}px`;
            token.style.top = "0";
            token.style.margin = "0";
            token.style.padding = "0";
            token.style.width = "max-content";
            token.style.fontFamily = cs.fontFamily;
            token.style.fontSize = cs.fontSize;
            token.style.fontWeight = cs.fontWeight;
            token.style.lineHeight = "1";
            token.style.letterSpacing = "0";
            token.style.whiteSpace = "nowrap";
            token.style.color = "#FFD700";
            token.dataset.chordPosition = String(match.index);
            chordRow.appendChild(token);
        }
    });
}

function getTransposeSteps() {
    const song = currentSong || getSongFromPage();
    return (keyIndex(getServiceKey(song)) - keyIndex(getOriginalKey(song)) + 12) % 12;
}
...
function buildTwoColumns() {
    ...
    const createPage = () => {
        ...
        pageGrid.style.setProperty("--presentation-page-height", `${Math.max(300, target.clientHeight || window.innerHeight)}px`);
    ...
    pages.forEach((entry) => {
        entry.grid.style.height = `${availableHeight()}px`;
        entry.grid.style.minHeight = `${availableHeight()}px`;
        entry.grid.style.alignItems = "start";
    });

    // Use the lyric font's real character width for chord X positions.
    // Chords may be smaller, but their horizontal positions must remain exactly
    // where the admin entered them in Add Song.
    applyExactPresentationChordPositions(target);
}
...
function getSongUrl(song) {
    ...
    const onCustomSongPage = /(?:^|\/)custom-song\.html$/i.test(location.pathname);
    if (song.customSong || String(song.file || "").startsWith("custom-song.html")) {
        const prefix = onCustomSongPage ? "custom-song.html" : "../custom-song.html";
        return `${prefix}?id=${encodeURIComponent(song.id || "")}`;
    }
    ...
    return onCustomSongPage ? `songs/${encodeURIComponent(name)}` : `../songs/${encodeURIComponent(name)}`;
}
...
window.startPresentation = async function startPresentation() {
    ...
    // Browser fullscreen is attempted as well; if the browser blocks automatic
    // fullscreen (common when there is no direct user gesture), the overlay
    // still remains fully maximized and usable.
    document.body.classList.add("presentation-active");
    screen.classList.add("show", "presentation-auto-maximized");
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Custom songs populate their structured #lyrics source asynchronously.
    // Let custom-song-runtime start the presentation only after that source
    // has been rendered; otherwise the generic auto-start can open an empty screen.
    if (document.body.classList.contains("custom-song-page")) return;
...
onAuthStateChanged(auth, async () => {
    ...
    if (qs("presentationScreen")?.classList.contains("show")) {
        ...
        await loadService();
        renderPresentation();
    } else if (!document.body.classList.contains("custom-song-page") &&
