...
function buildTwoColumns() {
    ...
    // Two-column reading flow:
    // 1) Start at the top of column 1.
    // 2) Keep placing complete sections below one another.
    // 3) When the next complete section would exceed the visible height,
    //    move that section to the TOP of column 2.
    // 4) Column 2 continues downward and the presentation viewport scrolls.
    ...
    sections.forEach((section, index) => {
        ...
        // Append to the current column first so the browser can measure the
        // real rendered height using the actual chord/lyric font metrics.
        const currentColumn = columns[columnIndex];
        currentColumn.appendChild(clone);

        if (columnIndex === 0 && currentColumn.children.length > 1) {
            const limit = availableHeight();
            const overflows = currentColumn.scrollHeight > limit;

            if (overflows) {
                // Remove the section and put the WHOLE section at the very top
                // of column 2. This prevents Verse 2 (or any following section)
                // from appearing below an empty/offset area in column 2.
    ...
    // Force a final layout pass after all sections have been inserted. This
    // keeps both columns top-aligned even when a large monitor changes the
    // available height after the presentation header is rendered.
