...
function rebuildLine(line) {
    ...
    [...line.childNodes].forEach((node) => {
        if (node.nodeType === Node.TEXT_NODE) {
            // Source HTML uses indentation/newlines around the chord span.
            // Those formatting characters are NOT lyric spacing. Remove only
            // the leading HTML formatting whitespace so the lyric starts
            // immediately below the chord without an artificial blank line.
            const lyricText = String(node.nodeValue || "").replace(/^(?:\s*\r?\n)+\s*/, "");
            lyricRow.appendChild(document.createTextNode(lyricText));
        }
    });
    ...
    chordRow.style.lineHeight = "1.428571";
...
function applyExactPresentationChordPositions(root = document) {
    root.querySelectorAll(".presentation-line").forEach(line => {
        ...
        const chord = line.querySelector(":scope > .presentation-chord-row > .presentation-chord");
        if (!lyric || !chord) return;
        const layout = String(chord.textContent || "");
        ...
        const chordRow = chord.parentElement;
        const tokenRe = /\S+/g;
        const tokens = layout.match(tokenRe) || [];
        // Replace the single source span with separately positioned tokens.
        chordRow.innerHTML = "";
        let m;
        tokenRe.lastIndex = 0;
        while ((m = tokenRe.exec(layout))) {
            ...
            token.textContent = transposeChord(raw, ((keyIndex(getServiceKey(currentSong)) - keyIndex(getOriginalKey(currentSong)) + 12) % 12));
            token.style.left = `${m.index * charWidth}px`;
