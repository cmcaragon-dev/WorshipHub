...
function noteIndex(note) {
    const clean = String(note || "").replace("♯", "#").replace("♭", "b");
    const sharp = SHARP_KEYS.indexOf(clean);
    if (sharp >= 0) return sharp;
    const flat = FLAT_KEYS.indexOf(clean);
    return flat >= 0 ? flat : -1;
}

function transposeChord(chord, semitones = 0) {
    const value = String(chord || "").trim();
    if (!value) return "";
    const root = value.match(/^([A-Ga-g])([#b]?)/);
    if (!root) return value;
    const rootName = root[1].toUpperCase() + root[2];
    const index = noteIndex(rootName);
    if (index < 0) return value;
    const shifted = (index + Number(semitones || 0) + 120) % 12;
    const newRoot = (preferFlats(value) ? FLAT_KEYS : SHARP_KEYS)[shifted];
    let result = newRoot + value.slice(root[0].length);
    result = result.replace(/\/([A-Ga-g])([#b]?)(?=\b|$)/g, (_, n, accidental) => {
        const slash = n.toUpperCase() + accidental;
        const i = noteIndex(slash);
        if (i < 0) return "/" + slash;
        return "/" + (preferFlats(value) ? FLAT_KEYS : SHARP_KEYS)[(i + Number(semitones || 0) + 120) % 12];
    });
    return result;
}

function transposeChordLayout(layout, semitones = 0) {
    const value = String(layout ?? "");
    if (!value) return "";
    return value.replace(/(^|[\s|,;()\-])([A-Ga-g](?:#|b)?(?:m|maj7?|min7?|sus[24]?|dim7?|aug)?(?:\/[A-Ga-g](?:#|b)?)?)(?=$|[\s|,;()\-])/g,
        (_, prefix, chord) => prefix + transposeChord(chord, semitones));
}
...
function rebuildLine(line) {
    // Presentation line is ALWAYS two physical rows:
    //   row 1 = chords
    //   row 2 = lyrics
    // The chord layout is taken verbatim from the source .chord element.
    // Every token keeps its original character index.  We never trim,
    // center, distribute, or reflow the chord positions.
    const chordNodes = Array.from(line.children).filter(el => el.classList.contains("chord"));
    const chordLayout = chordNodes
        .map(el => String(el.textContent || "").replace(/\r?\n/g, ""))
        .join("");

    const lyricText = Array.from(line.childNodes)
        .filter(node => !(node.nodeType === Node.ELEMENT_NODE && node.classList?.contains("chord")))
        .map(node => node.nodeType === Node.TEXT_NODE ? String(node.nodeValue || "") : String(node.textContent || ""))
        .join("")
        .replace(/^[ \t]+/, "")
        .replace(/[ \t]+$/, "");
    ...
    chordRow.setAttribute("aria-label", "Chords above lyrics");
    chordRow.dataset.originalLayout = chordLayout;

    // Build chord spans at the exact character index from the original layout.
    // The presentation CSS uses the same monospace font for the chord and lyric
    // rows, so 1ch is the exact same character grid used by the lyrics.
    const chordTokenRe = /\b([A-Ga-g](?:#|b)?(?:m|maj7?|min7?|sus[24]?|dim7?|aug|add\d+)?(?:\/([A-Ga-g](?:#|b)?))?)\b/g;
    let match;
    while ((match = chordTokenRe.exec(chordLayout))) {
        const span = document.createElement("span");
        span.className = "presentation-chord";
        span.textContent = transposeChord(match[1], getTransposeSteps());
        span.style.setProperty("--chord-pos", String(match.index));
        span.setAttribute("data-original-position", String(match.index));
...
function applyExactPresentationChordPositions(root = document) {
    root.querySelectorAll(".presentation-line").forEach(line => {
        ...
        const sourceChord = line.querySelector(":scope > .presentation-chord-row > .presentation-chord");
        ...
        const tokenRe = /\S+/g;
        let m;
        while ((m = tokenRe.exec(layout))) {
            const token = document.createElement("span");
            token.className = "presentation-chord";
            token.textContent = transposeChord(m[0], step);
            token.style.position = "absolute";
            // Store the X coordinate separately from the chord font size.
            // CSS uses this value so the chord position remains based on the
            // lyric character grid and is never compressed by the smaller chord font.
            // The position is measured from the ORIGINAL chord layout string.
            // Do not reflow, trim, center, or distribute the chord.  The lyric
            // font's character width is the reference grid because the chord
            // and lyric now use the exact same font size.
            token.style.setProperty("--chord-x", `${m.index * charWidth}px`);
            token.style.setProperty("--chord-pos", String(m.index));
            token.style.top = "0";
