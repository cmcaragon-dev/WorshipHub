...
function rebuildLine(line) {
    /*
     * PRESENTATION LINE CONTRACT
     * --------------------------
     * Every source .song-line is rendered as TWO PHYSICAL ROWS:
     *
     *     CHORDS
     *     LYRICS
     *
     * The chord positions are taken from the exact character indexes in the
     * source <span class="chord"> text.  We do not trim, center, distribute,
     * or reflow those positions.  The chord row and lyric row use the same
     * monospace font metrics, so one character position in the chord row is
     * the same horizontal position in the lyric row.
     *
     * IMPORTANT: chord spans live ONLY inside the chord row.  They can never
     * be children of, or overlap, the lyric row vertically.
     */
    const chordNodes = Array.from(line.children)
        .filter(el => el.classList.contains("chord"));
    ...
    /*
     * Keep every chord at its ORIGINAL character index.  Transposition is
     * applied to the chord text only; it does not change the stored position.
     */
    const tokenRe = /\b([A-Ga-g](?:#|b)?(?:m|maj7?|min7?|sus[24]?|dim7?|aug|add\d+)?(?:\/([A-Ga-g](?:#|b)?))?)\b/g;
    let match;

    while ((match = tokenRe.exec(chordLayout))) {
        const span = document.createElement("span");
        span.className = "presentation-chord";
        span.textContent = transposeChord(match[1], getTransposeSteps());
        span.dataset.originalPosition = String(match.index);
...
function applyExactPresentationChordPositions(root = document) {
    /* Kept for backward compatibility with older callers. Positioning is
       already applied by rebuildLine() using each chord's original index. */
    root.querySelectorAll(".presentation-line .presentation-chord").forEach(chord => {
        const position = chord.dataset.originalPosition ?? chord.style.getPropertyValue("--chord-pos") ?? "0";
        chord.style.setProperty("--chord-pos", String(position).trim());
    });
}
