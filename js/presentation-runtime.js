...
/*
 * CHORDIO Presentation Runtime v2
 *
 * Responsibilities:
...
let presentationSectionIndex=0;
...
function buildTwoColumns() {
    ...
    // CHORDIO PRESENTATION: ONE COLUMN ONLY.
    const column = document.createElement("div");
    column.className = "presentation-column presentation-single-column";
    ...
    presentationSectionIndex = Math.max(0, Math.min(presentationSectionIndex, sections.length - 1));
    requestAnimationFrame(() => selectPresentationSection(presentationSectionIndex, false));
}

function goPresentationSection(delta) {
    const target = qs("presentationLyrics");
    if (!target) return;
    const sections = [...target.querySelectorAll(".presentation-section")];
    if (!sections.length) return;
    const next = Math.max(0, Math.min(presentationSectionIndex + delta, sections.length - 1));
    if (next === presentationSectionIndex) return;
    presentationSectionIndex = next;
    selectPresentationSection(next, true);
}
...
/* CHORDIO v26: move next-song preview into compact header */
(function(){
...
/* CHORDIO v27: next-song preview lower-right */
(function(){
