...
function normalizeSong(song) {
    ...
    // When a Service Planner entry is supplied, `key` is the visible/planner
    // key and must win over a stale `serviceKey` value. Both fields are then
    // synchronized so presentation, song page, and passing chords agree.
    const resolvedKey = song.key || song.serviceKey || originalKey;
    merged.serviceKey = resolvedKey;
    merged.key = resolvedKey;
...
function rebuildLine(line) {
    ...
    const chordTransposeSteps = getChordTransposeSteps(currentSong);
    let lyricText = "";
    ...
    chordNodes.forEach(chord => {
        ...
        const originalChordText = String(chord.textContent || "").replace(/\r/g, "").replace(/\n/g, "").replace(/\t/g, "");
        span.textContent = transposeChordText(originalChordText, chordTransposeSteps);
...
window.startPresentation = async function startPresentation(options = {}) {
    ...
    if (service?.songs?.length) {
        ...
        if (found >= 0) {
            ...
            // IMPORTANT: The Service Planner's displayed `key` is the authoritative
            // service key. Older service records may contain a stale `serviceKey`
            // while `key` has already been updated (for example: key=C,
            // serviceKey=D). Always resolve that conflict in favor of `key`.
            const serviceSong = service.songs[found];
            const resolvedServiceKey = serviceSong?.key || serviceSong?.serviceKey || serviceSong?.originalKey || pageSong?.originalKey || pageSong?.key || "C";
