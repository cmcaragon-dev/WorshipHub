...
function renderSectionNavigator(sections) {
    const header = qs("presentationHeader");
    if (!header) return;

    let nav = qs("presentationSectionNav");
    if (!nav) {
        nav = document.createElement("div");
        nav.id = "presentationSectionNav";
    ...
    sections.forEach((section, index) => {
        const title = section.querySelector(":scope > .section-title")?.textContent?.trim() || `SECTION ${index + 1}`;
        const button = document.createElement("button");
        button.type = "button";
        button.className = "presentation-section-nav-button";
        button.dataset.index = String(index);
        button.textContent = title;
        button.title = `Show ${title}`;
        button.addEventListener("click", () => {
            selectPresentationSection(index, true);
        });
        nav.appendChild(button);
    });

    nav.querySelector(".presentation-section-nav-button")?.classList.add("active");
}

function bindSectionScrollTracking() {
    const target = qs("presentationLyrics");
    const nav = qs("presentationSectionNav");
    if (!target || !nav || typeof IntersectionObserver === "undefined") return;
    if (target._sectionObserver) target._sectionObserver.disconnect();

    const buttons = [...nav.querySelectorAll(".presentation-section-nav-button")];
    target._sectionObserver = new IntersectionObserver((entries) => {
    ...
    target.querySelectorAll(".presentation-section").forEach((section, index) => {
        section.dataset.sectionIndex = String(index);
        section.id = `presentationSection-${index}`;
        target._sectionObserver.observe(section);
    });
}
...
function buildTwoColumns() {
    ...
    renderSectionNavigator(sections);
    target.innerHTML = "";

    // Presentation mode intentionally shows ONE section at a time.
    // This gives instrumentalists the largest possible readable area while
    // the section navigator above the lyrics is used to jump between Verse,
    // Chorus, Bridge, Interlude, etc.
    const selectedIndex = Math.max(0, Math.min(
        Number(target.dataset.selectedSection || 0),
        Math.max(0, sections.length - 1)
    ));
    target.dataset.selectedSection = String(selectedIndex);

    const section = sections[selectedIndex];
    if (!section) return;

    const clone = section.cloneNode(true);
    clone.classList.add("presentation-section", "presentation-single-section");
    clone.dataset.sectionIndex = String(selectedIndex);
    clone.id = `presentationSection-${selectedIndex}`;
    ...
    bindSectionScrollTracking();

    const nav = qs("presentationSectionNav");
    nav?.querySelectorAll(".presentation-section-nav-button").forEach((button, index) => {
...
function selectPresentationSection(index, smooth = true) {
    ...
    target.dataset.selectedSection = String(safeIndex);
    buildTwoColumns();
