...
function rebuildLine(line) {
    ...
    const chordText = chordNodes.map(chord => String(chord.textContent || "").replace(/\r?\n/g, "").replace(/\t/g, "")).join("");

    const chordRow = document.createElement("div");
    chordRow.className = "presentation-chord-row";
    chordRow.style.position = "relative";
    chordRow.style.height = "0.70em";
    chordRow.style.minHeight = "0.70em";
    ...
    [...line.childNodes].forEach(node => {
        if (node.nodeType === Node.TEXT_NODE) {
            // Only discard indentation/newline added by HTML formatting.
            // Preserve every actual lyric character exactly.
            lyricRow.appendChild(document.createTextNode(
                String(node.nodeValue || "").replace(/^(?:\s*\r?\n)+\s*/, "")
            ));
        }
    });
    ...
    line.style.gridTemplateRows = "0.70em 1em";
    line.style.height = "1.70em";
    line.style.minHeight = "1.70em";
    line.style.margin = "0";
