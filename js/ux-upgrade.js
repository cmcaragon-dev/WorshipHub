/* CHORDIO UX UPGRADE — V65 */
(function(){
'use strict';
const LS={fav:'chordioFavoritesV65',recent:'chordioRecentV65',tpl:'chordioServiceTemplatesV65',live:'chordioLiveModeV65',snap:'chordioUndoSnapshotV65'};
...
function enhanceDashboard(){
 ...
 const hero=document.createElement('section');hero.id='chordioHero';hero.className='chordio-dashboard-hero';const userName=$('#userName')?.textContent?.trim()||'User';hero.innerHTML=`<div class="chordio-welcome-copy"><div class="chordio-welcome-eyebrow">HELLO, WELCOME</div><h1>${escapeHtml(userName)}</h1><p>Prepare your service, manage songs, and control your screens from one place.</p></div><div class="chordio-autosave" id="chordioAutosave">Ready</div>`;
 ...
 actions.append(
  quickAction('New Service','<i class="fa-solid fa-calendar-plus"></i>',()=>{$('#servicePlannerBtn')?.click()},'Create a service plan'),
  quickAction('Add Song','<i class="fa-solid fa-music"></i>',()=>{$('#addSongBtn')?.click()},'Build a new song'),
  quickAction('Multi-Screen','<i class="fa-solid fa-display"></i>',()=>{const b=document.querySelector('#serviceList .start-multi-screen-service-btn');if(b)b.click();else $('#servicePlannerBtn')?.click()},'Open screen control from a service'),
  quickAction('Bible','<i class="fa-solid fa-book-bible"></i>',()=>{toast('Open Multi-Screen → Online Bible for Scripture slides.','warn')},'Prepare Scripture slides'),
  quickAction('Settings','<i class="fa-solid fa-sliders"></i>',()=>{$('#settingsBtn')?.click()},'Configure CHORDIO')
 );
...
function addLiveMode(){
 const shell=$('.multi-screen-shell');if(!shell||$('#chordioLiveToggle'))return;const head=shell.querySelector('.multi-screen-head');if(!head)return;const b=document.createElement('button');b.id='chordioLiveToggle';b.className='chordio-live-toggle';b.innerHTML='<span class="chordio-live-status-dot" aria-hidden="true"></span><span class="chordio-live-label">LIVE MODE</span>';b.onclick=()=>toggleLive();head.appendChild(b);if(get(LS.live,false))toggleLive(true);
}
function toggleLive(force){const on=typeof force==='boolean'?force:!document.body.classList.contains('chordio-live-mode');document.body.classList.toggle('chordio-live-mode',on);set(LS.live,on);const b=$('#chordioLiveToggle');if(b){const label=b.querySelector('.chordio-live-label');if(label)label.textContent=on?'LIVE':'LIVE MODE';b.classList.toggle('chordio-live-exit',on);b.setAttribute('aria-pressed',String(on));}$$('.multi-live-dot').forEach(dot=>dot.setAttribute('aria-label',on?'Live':'Offline'));toast(on?'Live Mode enabled — presentation controls simplified.':'Live Mode disabled.','success')}
...
function syncWelcomeName(){
 const source=$('#userName'); const heroName=$('#chordioHero h1');
 if(source&&heroName){ const value=source.textContent?.trim()||'User'; if(heroName.textContent!==value)heroName.textContent=value; }
}
...
function observe(){new MutationObserver(()=>{captureSongCards();syncWelcomeName()}).observe(document.body,{childList:true,subtree:true});}
