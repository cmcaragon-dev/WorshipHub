...
async function init() {
    ...
    const serviceSong = activeService?.songs?.[serviceIndex];
    if (serviceSong) {
        // The Service Planner copy is authoritative. Use its saved transpose
        // first, then derive it from originalKey -> serviceKey for older data.
        const savedTranspose = Number(serviceSong.transpose);
        const oi = noteIndex(serviceSong.originalKey || serviceSong.baseKey || song?.originalKey || song?.key);
        const si = noteIndex(serviceSong.serviceKey || serviceSong.key);
        ...
        // Keep service metadata separate from the master song object. The HTML
        // lyrics/chord markup remains the source for the actual song content.
        window.currentSong = {
            ...(song || {}),
            ...serviceSong,
            originalKey: serviceSong.originalKey || serviceSong.baseKey || song?.originalKey || song?.key || serviceSong.key,
            serviceKey: serviceSong.serviceKey || serviceSong.key || song?.serviceKey || song?.key,
            transpose: Number.isFinite(Number(serviceSong.transpose)) ? Number(serviceSong.transpose) : transposeSteps
        };
    }
    updateAll();
    requestAnimationFrame(layoutSongPageTwoColumns);
