...
async function restoreServiceSongFromFirebase() {
    ...
    const savedServiceKey = serviceSong.serviceKey || serviceSong.key || originalKey;

    let savedTranspose = Number(serviceSong.transpose);
    if (!Number.isFinite(savedTranspose)) {
        const oi = noteIndex(originalKey);
        const si = noteIndex(savedServiceKey);
        if (oi >= 0 && si >= 0) {
            savedTranspose = (si - oi + 12) % 12;
    ...
    window.__worshipHubServiceKeyReady = true;
    updateAll();
    window.dispatchEvent(new CustomEvent("worshiphub:service-key-ready", {
        detail: {
            serviceKey: savedServiceKey,
            transpose: savedTranspose,
            serviceId: serviceId,
