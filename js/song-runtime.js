...
function layoutSongPageTwoColumns() {
    ...
    source.classList.add("wh-v23-source");
    layout.innerHTML = "";

    // Keep two equal columns and use the available viewport height as the
    // page height. Complete sections flow down column 1 first; once the
    // next section cannot fit, it starts at the top of column 2. When both
    // columns are full, a new two-column page is created below.
    const availableHeight = Math.max(320, Math.floor(window.innerHeight - 178));
    let page = null;
    let columns = null;
    let columnIndex = 0;
    ...
    sections.forEach(section => {
        ...
        clone.classList.add("song-display-section");

        let current = columns[columnIndex];
        current.appendChild(clone);

        // If the section does not fit in the current column and that column
        // already contains content, move the COMPLETE section to the next
        // column. This prevents sections from being split between columns.
...
window.WorshipHubSongRuntime = { setTranspose, updateAll, startPresentation, exitPresentation, layoutSongPageTwoColumns };
