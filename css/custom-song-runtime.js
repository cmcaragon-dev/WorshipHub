...
function transposeChord(chord){
    return String(chord || "").replace(/[A-G](#|b)?/g,note=>{
        let i=SHARP.indexOf(note);
        if(i<0){const flats={Db:"C#",Eb:"D#",Gb:"F#",Ab:"G#",Bb:"A#"};i=SHARP.indexOf(flats[note]||note)}
        if(i<0)return note;
        return SHARP[(i+transpose+12)%12];
    });
}
function render(){
    ...
    stage.innerHTML=sections.map(section=>`<section class="section"><div class="section-title">${esc(section.type)} ${section.number||""}</div>${(section.lines||[]).map(line=>{
        const chordHTML=(line.chords||[]).map(c=>`<span class="chord" style="left:${Number(c.position||0)*15}px">${esc(transposeChord(c.chord))}</span>`).join("");
...
document.getElementById("up").onclick=()=>{transpose++;render()};
document.getElementById("down").onclick=()=>{transpose--;render()};
